...
//};
//zhb:use for search
static unsigned int rfc1738_unsafe_bits[4] = {0,1342177421,2013265920,2013265921};
...
//zhb:use for search
static unsigned int rfc1738_reserved_bits[4] = {0,2885713984UL,1,0};
...
rfc1738_do_escape(const char *url, int encode_reserved)
{
	...
	for (p = url, q = buf; *p != '\0'; p++, q++)
	{
		...
		//zhb:make it faster
		if(rfc1738_unsafe_bits[((int)(*p)) / 32] & (1 << ((int)(*p)) % 32))
			do_escape = 1;
	...
	//	}
		...
		//zhb:make it faster
		if(encode_reserved > 0 && (rfc1738_reserved_bits[((int)(*p)) / 32] & (1 << ((int)(*p)) % 32)))
			do_escape = 1;
