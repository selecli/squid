...
HttpHeaderHashField **
httpHeaderBuildHashFields(const HttpHeaderFieldAttrs * attrs, int count)
{
	int i;
	HttpHeaderHashField **table = NULL;
	assert(attrs && count);

	/* allocate space */
	table = xcalloc(137, sizeof(HttpHeaderHashField*));

	for (i = 0; i < count; ++i)
	{
		const int id = attrs[i].id;
		const char* p = attrs[i].name;
		int nLen = strlen(p);
		if(nLen < 2)
			continue;
		...
		//int nSlot = (p[0] + p[1] + p[nLen - 1] + p[nLen - 2] + p[nLen/2]) * nLen % 137;
		//int nSlot = (towlower(p[0]) + towlower(p[1]) + towlower(p[nLen - 1]) + towlower(p[nLen - 2]) + towlower(p[nLen/2])) * nLen % 137;
		int nSlot = (tolower(p[0]) + tolower(p[1]) + tolower(p[nLen - 1]) + tolower(p[nLen - 2]) + tolower(p[nLen/2])) * nLen % 137;
		...
		HttpHeaderHashField *hashNode = xcalloc(1, sizeof(HttpHeaderHashField));
		hashNode->id = id;
		if(NULL == table[nSlot])
			table[nSlot] = hashNode;
		else
		{
			HttpHeaderHashField *pHash = table[nSlot];
			while(NULL != pHash->next)
			{
				pHash = pHash->next;
			}
			pHash->next = hashNode;
	...
	for (i = 0; i < count; ++i)
	{
		...
		if(nLen < 2)
			continue;
		nSlot = (p[0] + p[1] + p[nLen - 1] + p[nLen - 2] + p[nLen/2]) * nLen % 137;
		
		assert(id >= 0 && id < count);
		assert(attrs[i].name);
		
		hashNode = xcalloc(1, sizeof(HttpHeaderHashField));
		hashNode->id = id;
		if(NULL == table[nSlot])
			table[nSlot] = hashNode;
		else
...
void
httpHeaderDestroyHashFields(HttpHeaderHashField ** table, int count)
{
	int i;
	for (i = 0; i < 137; ++i)
	{
		HttpHeaderHashField * p = table[i];
		HttpHeaderHashField * p1 = NULL;
		while(NULL != p)
		{
			p1 = p->next;
			xfree(p);
