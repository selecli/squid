...
#ifdef CC_FRAMEWORK
void fwdStartExtend(int fd, StoreEntry * e, request_t * r, fde *fd_match);
PF fwdServerClosed;
#else
...
#ifdef CC_FRAMEWORK
	if (fwdState->old_fd_params.matched_mod_params)
	{       
		int i, j;
        Array *array;
        ...
        {
            // added for memory leakgae when reload
            // chenqi noted
            array = &fwdState->old_fd_params.matched_mod_params[i];
            for (j=0; j<array->count; j++)
            {
                mod_param = array->items[j];
                mod_param->count--;
            }
            arrayClean(array);
        }
		memPoolFree(matched_mod_params_pool, fwdState->old_fd_params.matched_mod_params);
		fwdState->old_fd_params.matched_mod_params = NULL; 
	}       
	if (fwdState->old_fd_params.cc_run_state)
	{
		memPoolFree(cc_run_state_pool, fwdState->old_fd_params.cc_run_state);
		fwdState->old_fd_params.cc_run_state = NULL;
	}
	/*
	 * The free method of cc_fde_private_data is wrong. for nobody uses it,
	 * we already do not copy this struct from client side to server side.
	 * so it is not necessary to free it.
	if(NULL != fwdState->old_fd_params.cc_fde_private_data)	
	{
		memPoolFree(cc_private_data_pool, fwdState->old_fd_params.cc_fde_private_data);
		fwdState->old_fd_params.cc_fde_private_data = NULL;
	}
	*/
	cc_free_fwdState_private_data(fwdState);
...
fwdCheckRetriable(FwdState * fwdState)
{
	 ...
	 */
	/***Even if the client use the post method,we still reuse the persistent connections***/
#ifdef CC_FRAMEWORK

	if( cc_call_hook_func_private_post_use_persistent_connection(fwdState) )
	{
		return 1;
	}
#endif
...
#ifdef CC_FRAMEWORK
    cc_call_hook_func_private_preload_change_hostport(fwdState, &host, &port);
	int ret = 0;
	ret = cc_call_hook_func_private_set_name_as_ip(fwdState,&name);
	cc_call_hook_func_private_UserAgent_change_port(fwdState->client_fd, fwdState, &port);
#endif
	if (ftimeout < 0)
		ftimeout = 5;
...
#ifdef CC_FRAMEWORK       
			cc_copy_fd_params(&fwdState->old_fd_params, &fd_table[fwdState->server_fd], 1);
#endif
			fwdState->n_tries++;
...
#ifdef CC_FRAMEWORK
		fd=pconnPop(name,port,NULL,NULL,0,&idle);
		if(ret != 0)	
		{
			cc_call_hook_func_private_openIdleConn(idle,fwdState,name,domain,ctimeout);
		}
		if(fd == -1)
        {
...
//for dst ip
	...
	ret = cc_call_hook_func_private_dst_ip(host, port, ctimeout, outgoing, tos, fwdState);
	if(ret == 1)
		return;
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_comm_set_timeout(fwdState,&ctimeout);
#endif
	...
	else
	{
		...
		if (fwdState->request->flags.tproxy)
		{
...
#ifdef CC_FRAMEWORK
			int ret = cc_call_hook_func_private_set_tproxy_ip(fwdState);
			if(ret!=0)
				itp.v.addr.faddr.s_addr = inet_addr(httpHeaderGetByName(&fwdState->request->header,"X-Forwarded-For").buf);
			else
#endif
			...
			{
				debug(20, 1) ("tproxy ip=%s,0x%x,port=%d ERROR ASSIGN %s\n",
							  inet_ntoa(itp.v.addr.faddr),
							  itp.v.addr.faddr.s_addr,
							  itp.v.addr.fport,strerror(errno));
			}
			else
			{
				itp.op = TPROXY_FLAGS;
...
#ifdef CC_FRAMEWORK
		if(cc_call_hook_func_private_add_host_ip_header_for_ca(fwdState->client_fd, fwdState))
			return;
#endif
...
#ifdef CC_FRAMEWORK
	//cc_copy_client_fd_params_server(fwdState->client_fd, server_fd);
#endif
...
#ifdef CC_FRAMEWORK
	/* add by xin.yao: for reforwardable
	 * if 0: go on
	 * if 1: return 1
	 */
	int ret = cc_call_hook_func_private_reforwardable(fwdState);
	if (ret)
	{
		return 1;
	}
...
fwdStartPeer(peer * p, StoreEntry * e, request_t * r)
{
	...
	fwdState->server_fd = -1;
#ifdef CC_FRAMEWORK
	// memset(&fwdState->old_fd_params, 0, sizeof(fde));
	/*
	int i;
	for (i=0;i<Config.cc_all_mod_num ;i++)
	{       
		fwdState->old_fd_params.matched_mod_params[i] = NULL;
	} 
	*/
#endif
...
void
fwdStart2(int fd, StoreEntry * e, request_t * r,clientHttpRequest* http)
{
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_private_change_request(r,http);
#endif
	FwdState *fwdState;
	int answer;
	ErrorState *err;
	...
	for (i=0;i<Config.cc_all_mod_num ;i++)
	{       
		fwdState->old_fd_params.matched_mod_params->items[i] = NULL;
	}     
	cc_copy_fd_params(&fd_table[fwdState->client_fd], &fwdState->old_fd_params, 0);
#endif
	fwdState->server_fd = -1;
	fwdState->request = requestLink(r);
	fwdState->start = squid_curtime;
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_preload_change_fwdState(r, fwdState);
#endif
...
#ifdef CC_FRAMEWORK
{
    fwdStartExtend(fd, e, r, NULL);
}

void
fwdStartExtend(int fd, StoreEntry * e, request_t * r, fde *fd_match)
#endif
{
	FwdState *fwdState;
	int answer;
	...
	if (r->client_addr.s_addr != no_addr.s_addr && r->protocol != PROTO_INTERNAL && r->protocol != PROTO_CACHEOBJ)
	{
		 ...
		 */
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token_and_host("miss_access",r->host);

	answer = aclCheckFastRequest(acl,r);
#else
		answer = aclCheckFastRequest(Config.accessList.miss, r);
#endif
		if (answer == 0)
...
#ifdef CC_FRAMEWORK
	if(r->protocol == PROTO_CACHEOBJ)
		goto CCGO;
	int ret = cc_call_hook_func_private_query_direct(fd, r, e);
	debug(17, 5) ("fwdStart: cc_call_hook_func_private_query_direct return %d\n", ret);
	if(ret > 0)
		return;
CCGO:
#endif
...
#ifdef CC_FRAMEWORK
	memset(&fwdState->old_fd_params, 0, sizeof(fde));
    if (fd >= 0 && fd_table[fd].cc_run_state)
        cc_copy_fd_params(&fd_table[fd], &fwdState->old_fd_params, 0);
    else if (fd_match && fd_match->cc_run_state)
        cc_copy_fd_params(fd_match, &fwdState->old_fd_params, 0);

	cc_call_hook_func_fwd_start(fwdState);
#endif
	fwdState->server_fd = -1;
	fwdState->request = requestLink(r);
	fwdState->start = squid_curtime;
...
fwdFail(FwdState * fwdState, ErrorState * errorState)
{
	debug(17, 3) ("fwdFail: %s \"%s\"\n\t%s\n",
				  ...
				  storeUrl(fwdState->entry));
#ifdef CC_FRAMEWORK
	fwdLogReplyStatus(1, 1);
#endif
#ifdef CC_FRAMEWORK
	cc_call_hook_func_store_no_cache_reason(fwdState->entry, NCR_FWD_FAIL);
#endif
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_fwd_complete_clean(fwdState);
#endif
		/*
...
fwdStats(StoreEntry * s)
{
	...
	for (i = 0; i <= (int) HTTP_INVALID_HEADER; i++)
	{
		...
		if(i == 1)
		{
			storeAppendPrintf(s, "Fail");
		}
		else if(i == 2)
		{

			storeAppendPrintf(s, "Abort");
		}
		else
		{
...
void _fwdInitiateSSL(FwdState * fwdState)
{
	fwdInitiateSSL(fwdState);
}

void _fwdDispatch(FwdState * fwdState)
{
	fwdDispatch(fwdState);
}

void _fwdConnectStart(void *data)
{
	fwdConnectStart(data);
}

void _fwdServerClosed(int fd, void *data)
{
	fwdServerClosed(fd, data);
}
