...
httpHdrRangeSpecParseCreate(const char *field, int flen)
{
	...
	if (*field == '-')
	{
        if (!httpHeaderParseSize(field + 1, &spec.length) || range_spec_unknown == spec.length)
			return NULL;
	}
	else
    ...
    if (known_spec(spec.length) && !spec.length || (range_spec_unknown == spec.length && range_spec_unknown == spec.offset))
	{
...
httpHdrRangeBoundaryStr(clientHttpRequest * http)
{
	...
	assert(http);
#ifdef CC_FRAMEWORK
	int length = cc_call_hook_func_support_complex_range(http);
	if (length)
	{
		stringAppend(&b,full_appname_string, length);
	}
	else
#endif
...
#ifdef CC_FRAMEWORK
	int ret = 1;
	cc_call_hook_func_httpHdrRangeOffsetLimit(&ret);
	if(ret == 0)
	{
		return 1;
	}
...
#ifdef CC_FRAMEWORK
int
httpHdrRangeOffsetLimit2(request_t *request, HttpHdrRange * range)
{
	if (NULL == range)
		/* not a range request */
		return 0;
#ifdef CC_FRAMEWORK
    int ret = -1;   //Must Set ret = -1 in mod_async_load
	cc_call_hook_func_httpHdrRangeOffsetLimit(request,&ret);
	if(ret == 0)
	{
		return 1;
	}
