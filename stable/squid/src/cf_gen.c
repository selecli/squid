...
next_alias:
		...
		{
#ifdef CC_FRAMEWORK
			if(strcmp(entry->type,"acl_access") == 0)
			{
				fprintf(fp,
						"\t\tcc_parse_%s(\"%s\"%s);\n",
						entry->type,entry->name,entry->array_flag ? "[0]": ""
						);
			}
			else
#endif
...
gen_free(Entry * head, FILE * fp) 
{
	...
	for (entry = head; entry != NULL; entry = entry->next)
	{
		...
		if (entry->ifdef)
			fprintf(fp, "#if %s\n", entry->ifdef);
		if (strcmp(entry->name, "http_port") == 0 
		    || strcmp(entry->name, "https_port") == 0)
		{
			fprintf(fp, "#ifdef CC_FRAMEWORK\n\tif(reconfiguring)\n\t{\n\t\tif(http_port_cfg_changed)\n");
