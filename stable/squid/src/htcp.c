...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token_and_host("htcp_access",s->request->host);
	
	if (!htcpAccessCheck(acl, s, from))
#else
...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token_and_host("htcp_clr_access",s->request->host);
	if (!htcpAccessCheck(acl, s, from))
#else
