...
#ifdef CC_FRAMEWORK
int httpCachableReply(HttpStateData *, NOCACHE_REASON *);
#else
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_httpStateFree(httpState);
#endif
	/* added end*/
...
#ifdef CC_FRAMEWORK
int
httpCachableReply(HttpStateData * httpState, NOCACHE_REASON *reason)
#else
static int
...
#ifdef CC_FRAMEWORK
	int ret = 0;
	ret = cc_call_hook_func_http_repl_cachable_check(httpState->fd, httpState);	
	if(ret == 5)
	{
		*reason = NCR_MOD_NEGATIVE_TTL;
		return 0;
	}
	else if(ret == 4)
		return 1;
...
#define REFRESH_OVERRIDE(field)	0
	...
	case HTTP_GONE:
		...
		{
#ifdef CC_FRAMEWORK
			*reason = NCR_REFRESH_IS_CACHABLE;
#endif
			return 0;
		}
...
#ifdef CC_FRAMEWORK
		ret = cc_call_hook_func_private_store_partial_content(httpState);
		if(ret)
		{
			return 1;
			break;
...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token_and_host("broken_vary_encoding",request->host);
			if (acl && aclCheckFast(acl, &checklist))
#else
...
httpProcessReplyHeader(HttpStateData * httpState, const char *buf, int size)
{
	...
	ret2 = cc_call_hook_func_is_active_compress(httpState->fd, httpState, hdr_size, &done);
	debug(11, 3)("httpProcessReplyHeader: mod_active_compress has been taken over storeAppend [%d]\n", ret);
	
	ret = cc_call_hook_func_http_repl_read_start(httpState->fd, httpState);	
	if(ret == HTTP_REPL_READ_START_NO_CACHE)
		goto no_cache;
	if (ret == 6)
		goto skip_append_rep_hdr;

	// end 
	int ret1 = 1;
	ret1 = cc_call_hook_func_store_append_check(httpState);
...
skip_append_rep_hdr:
...
#else
#endif
		httpHeaderHas(&reply->header, HDR_VARY)
#if X_ACCELERATOR_VARY
...
#ifdef CC_FRAMEWORK
	NOCACHE_REASON ncr = NCR_UNKNOWN;
#endif
	switch (httpCachableReply(httpState, &ncr))
	{
	case 1:
		httpMakePublic(entry);

#ifdef CC_FRAMEWORK
		cc_call_hook_func_http_repl_refresh_start(httpState->fd, httpState);	
#endif

		break;
	case 0:
#ifdef CC_FRAMEWORK
		cc_call_hook_func_httprep_no_cache_reason(httpState->fd, entry->mem_obj, ncr);
#endif

#ifdef CC_FRAMEWORK
		if(httpState->entry->mem_obj->reply->sline.status == HTTP_NOT_MODIFIED)
			cc_call_hook_func_http_repl_refresh_start(httpState->fd, httpState);
#endif
		httpMakePrivate(entry);
		break;
	case -1:
#ifdef CC_FRAMEWORK
		cc_call_hook_func_httprep_no_cache_reason(httpState->fd, entry->mem_obj, NCR_NEG_CACHED);
		if (!cc_call_hook_func_private_negative_ttl(httpState))
		{
			goto no_cache;
		}
		else
...
httpAppendBody(HttpStateData * httpState, const char *buf, ssize_t len, int buffer_filled)
{
	...
	{
		cc_call_hook_func_http_repl_read_end(fd, httpState);
	...
	while (len > 0)
	{
...
#ifdef CC_FRAMEWORK
			int ret = cc_call_hook_func_buf_recv_from_s_body(httpState, (char *)buf, size, 0);
			if(ret == 1 )
				return;

			size_t comp_size = 0;
			LOCAL_ARRAY(char, comp_buf, 2*SQUID_TCP_SO_RCVBUF + 1);
			
			int before_complete = httpState->eof;
			int ret1 = cc_call_hook_func_private_compress_response_body(httpState, (char *)buf, size, comp_buf, 2*SQUID_TCP_SO_RCVBUF + 1, &comp_size, 0);
			if(ret1 == 1 ){
				complete = httpState->eof;
				httpState->eof = before_complete;
				storeAppend(httpState->entry, comp_buf, comp_size);
				//buf += size;
                        	//len -= size;
				//continue;
			}
		...
		else if (httpState->chunk_size < 0)
		{
...
#ifdef CC_FRAMEWORK
			int ret = cc_call_hook_func_buf_recv_from_s_body(httpState, (char *)buf, len, 0);
			if(ret == 1 )
				return;
			...
			int before_complete = httpState->eof;
			int ret1 = cc_call_hook_func_private_compress_response_body(httpState, (char *)buf, len,comp_buf,2*SQUID_TCP_SO_RCVBUF + 1, &comp_size, 0);
			if(ret1 == 1 ){
				complete = httpState->eof;
				httpState->eof = before_complete;
			...
			if (eol)
			{
				...
				else
				{
					...
					if (*p == '\n')
					{
						complete = 1;
#ifdef CC_FRAMEWORK
						cc_call_hook_func_private_end_compress_response_body(httpState, (char *)buf, len);
#endif
		...
		else
		{
			...
			debug(11,2)("httpAppendBody:Don't know what to do with this data. Bail out\n ");
			break;
		}
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_360_write_receive_warning_log(httpState,len);
#endif
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_store_no_cache_reason(entry, NCR_SERVER_CLOSE);
#endif
		comm_close(fd);
		return;
	}
	...
	if (len > 0)
	{
		...
		("httpReadReply: Excess data from \"%s %s\"\n",
		 ...
		 storeUrl(entry));
#ifdef CC_FRAMEWORK
		 cc_call_hook_func_private_process_excess_data(httpState);
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_repl_read_end(fd, httpState);
		cc_call_hook_func_private_http_repl_read_end(fd, httpState,&keep_alive);
#endif
	...
	if (keep_alive)
	{
		...
		else
		{
		    ...
		    cc_call_hook_func_private_set_host_as_ip(fd,&host);
//	        pconnPush(fd,fd_table[fd].ipaddr,request->port,NULL,client_addr,client_port);
//			pconnPush(fd,host,request->port,NULL,client_addr,client_port);
			pconnPush(fd,host,request->port,NULL,NULL,0);
		    cc_call_hook_func_private_set_upperlayer_persist_connection_timeout(httpState->fwd,fd); 
#else
...
httpReadReply(int fd, void *data)
{
	...
	debug(146, 5) ("httpReadReply: FD %d: len %d., buffer_filled == %d\n", fd, (int) len,buffer_filled);
	if (len > 0)
	{
...
#ifdef CC_FRAMEWORK
    len = cc_call_hook_func_private_reply_modify_body(httpState, buf, len, SQUID_TCP_SO_RCVBUF);
#endif

	if (len < 0)
	{
		debug(146, 2) ("httpReadReply: FD %d: read failure: %s.\n",
					  fd, xstrerror());
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_buf_recv_from_s(fd, buf, len);
#endif
	...
	else
	{
		if (httpState->reply_hdr_state < 2)
		{
			...
			{
#ifdef CC_FRAMEWORK
				int ret = cc_call_hook_func_buf_recv_from_s_header(httpState,buf,len);
				if(ret == 1)
					return;
#endif
	...
	/*
#ifdef CC_FRAMEWORK
		int ret = cc_call_hook_func_buf_recv_from_s_body(httpState, buf + done, len - done,buffer_filled);
		if(ret == 1 )
			return;
#endif
*/
...
#define BBUF_SZ (MAX_URL+32)
	...
	if (NULL == orig_request->range)
		...
		we_do_ranges = 0;
#ifdef CC_FRAMEWORK
	else if(httpHdrRangeOffsetLimit2(orig_request,orig_request->range))
		we_do_ranges = 0;
#else
	...
	while ((e = httpHeaderGetEntry(hdr_in, &pos)))
	{
		...
		case HDR_REQUEST_RANGE:
			...
			{
				int ret = 0;
				ret = cc_call_hook_func_private_process_range_header(request,hdr_out,(StoreEntry* )e);
				if(ret == 0)
#endif
				httpHeaderAddClone(hdr_out, e);
#ifdef CC_FRAMEWORK
					   ...
					   http_state_flags flags)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_buf_send_to_s(httpState, mb);
	if(!mb->size)
	{
		char * str = NULL;
		char * urlpath_tmp = xmalloc(MAX_URL);
		memset(urlpath_tmp, 0, MAX_URL * sizeof(char));	
		xstrncpy(urlpath_tmp, strBuf(request->urlpath), MAX_URL);
		if (-1 == httpState->fwd->client_fd && NULL != (str = strstr(urlpath_tmp, "@@@")))
		{
			*str = '\0';	
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_req_send_exact(httpState, &hdr);
#endif
	...
	//add by haobo.zhang
#ifdef CC_FRAMEWORK
	//cc_call_hook_func_http_check_request_change(mb, httpState);
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_req_send(httpState);
#endif

	memBufDefInit(&mb);
#ifdef CC_FRAMEWORK
	httpBuildRequestPrefix(httpState, req,
...
httpStart(FwdState * fwd)
{
	...
	HttpStateData *httpState;
	request_t *proxy_req;
	request_t *orig_req = fwd->request;   /*This request Is the client request store in clientHttpRequest*/ 
	...
	httpState->fwd = fwd;
	httpState->entry = fwd->entry; /*This entry is which we Create in clientProcessMiss*/
	httpState->fd = fd;      /*This FD It's fc----->origin_server'*/
	if (fwd->servers)
...
#ifdef CC_FRAMEWORK
    cc_call_hook_func_private_assign_dir_set_private_data(httpState);
#endif
	/*
...
#ifdef CC_FRAMEWORK
		acl_access * acl = cc_get_acl_access_by_token_and_host("broken_posts",httpState->request->host);
	if(!acl)
#else
...
#ifdef CC_FRAMEWORK
	else if (!aclCheckFastRequest(acl, httpState->request))
#else
...
httpSendRequestEntry(int fd, char *bufnotused, size_t size, int errflag, void *data)
{
	...
	if (size > 0)
	{
		/* no write and read op on fd, so delete the fd_bytes. or, the statistics will be incorrect */
//		fd_bytes(fd, size, FD_WRITE);
		kb_incr(&statCounter.server.all.kbytes_out, size);
		kb_incr(&statCounter.server.http.kbytes_out, size);
	}
