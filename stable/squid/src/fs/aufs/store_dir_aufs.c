...
#ifdef CC_FRAMEWORK
static STLIMITOBJ storeAufsLimitObj;
#endif
...
storeAufsDirMapBitReset(SwapDir * SD, sfileno filn)
{
	 ...
	 */
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_put_suggest(SD,filn);
#endif
...
storeAufsDirMapBitAllocate(SwapDir * SD)
{
	...
	int fn = -1;
#ifdef CC_FRAMEWORK
	fn = cc_call_hook_func_private_get_suggest(SD);
#endif
	if(fn<0)
	{
...
#ifdef CC_FRAMEWORK
	    storeAufsDirUnlinkFile(SD, filn, 0);
#else
...
storeAufsDirRebuildFromSwapLog(void *data)
{
	...
	/* fuke.li fix for startup_delete_file */	
#ifdef CC_FRAMEWORK
	for (count = 0; 1 ; count++) {
#else
    for (count = 0; count < rb->speed; count++) {
...
/* fuke.li fix for startup_delete_file */		
#ifdef CC_FRAMEWORK
		storeAufsDirRebuildFromSwapLog(rb);
#else
	    eventAdd("storeRebuild", storeAufsDirRebuildFromSwapLog, rb, 0.0, 1);
#endif
...
storeAufsDirRebuild(SwapDir * sd)
{
	...
	/* fuke.li for startup_delete_file */
#ifdef CC_FRAMEWORK
...
#else
    {
	    fd = file_open(state->cln, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY);
	    if(fd >= 0)
		    file_close(fd);
	    else
        {
            debug(50, 0) ("Can't open %s ;Reason : %s\n",   state->cln, xstrerror());
            fatalf("%s: Can't open normally. Check your disk!\n", state->cln);
        }
...
storeAufsLimitObj(SwapDir* SD)
{
	...
	f = (double)(SD->cur_obj_cnt - SD->snapshot); 
    ...
    debug(47, 3) ("storeAufsLimitObj: f=%f, max_scan=%d, max_remove=%d\n",
    ...
    walker->Done(walker);
    debug(47, (removed ? 2 : 3)) ("storeAufsLimitObj: %s removed %d/%d f=%.03f max_scan=%d\n",
...
#ifdef CC_FRAMEWORK
void
storeAufsDirUnlinkFile(SwapDir * SD, sfileno f, int disk_error_flag)
#else
void
...
#else
#ifdef CC_FRAMEWORK
    aioUnlink(storeAufsDirFullPath(SD, f, NULL), NULL, NULL, INDEX_OF_SD(SD), disk_error_flag);
#else
    aioUnlink(storeAufsDirFullPath(SD, f, NULL), NULL, NULL, INDEX_OF_SD(SD));
#endif
#endif
}
...
storeAufsDirParse(SwapDir * sd, int index, char *path)
{
...
#ifdef CC_FRAMEWORK
    sd->limitobj = storeAufsLimitObj;
    sd->obj.zcopy = storeAufsZCopy;
#endif
	...
	if(reconfigure_in_thread)
		sd->repl = createRemovalPolicy(Config_r.replPolicy);
	else
...
#ifdef CC_FRAMEWORK
    memPoolDestroy(aufs_qzcopy_pool);
#endif
...
storeFsSetup_aufs(storefs_entry_t * storefs)
{
    ...
    aufs_qread_pool = memPoolCreate("AUFS Queued read data", sizeof(queued_read));
#ifdef CC_FRAMEWORK
    aufs_qzcopy_pool = memPoolCreate("AUFS Queued zcopy data", sizeof(queued_zcopy));
#endif
