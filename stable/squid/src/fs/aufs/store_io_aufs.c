...
#ifdef CC_FRAMEWORK
static AIOCB storeAufsZCopyDone;
#endif
...
#ifdef CC_FRAMEWORK
    int disk_error_flag = cc_call_hook_func_private_getDiskErrorFlag(e);
    storeAufsDirUnlinkFile(SD, e->swap_filen, disk_error_flag);
#else
...
#ifdef CC_FRAMEWORK
static void
storeAufsZCopyDone(int fd, void *my_data, const char *buf, int len, int errflag)
{
    storeIOState *sio = my_data;
    squidaiostate_t *aiostate = (squidaiostate_t *) sio->fsstate;
