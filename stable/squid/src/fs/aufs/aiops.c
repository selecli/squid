...
#ifdef CC_FRAMEWORK
static void squidaio_queue_request(squidaio_request_t *, int);
#else
...
#ifdef CC_FRAMEWORK
static void squidaio_do_real_unlink(squidaio_request_t * requestp);
static void squidaio_do_zcopy(squidaio_request_t *);
#endif
#if USE_TRUNCATE
...
#ifdef CC_FRAMEWORK
static int request_queue_cnt = -1;
static squidaio_request_queue_t *request_queues = NULL;
typedef struct {
	squidaio_request_t *head, **tailp;
} squidaio_request_queue2_t;
static squidaio_request_queue2_t *request_queue2s = NULL;
#else
...
#endif
{
    ...
    {
	    request_queues = xcalloc(Config.cacheSwap.n_configured, sizeof(squidaio_request_queue_t));
	    request_queue2s = xcalloc(Config.cacheSwap.n_configured, sizeof(squidaio_request_queue2_t));
    }
    request_queue_cnt = index;
#endif
    ...
    if (pthread_mutex_init(&(request_queues[request_queue_cnt].mutex), NULL))
	fatal("Failed to create mutex");
    if (pthread_cond_init(&(request_queues[request_queue_cnt].cond), NULL))
	fatal("Failed to create condition variable");
    request_queues[request_queue_cnt].head = NULL;
    request_queues[request_queue_cnt].tailp = &request_queues[request_queue_cnt].head;
    request_queues[request_queue_cnt].requests = 0;
    request_queues[request_queue_cnt].blocked = 0;

    request_queue2s[request_queue_cnt].head = NULL;
    request_queue2s[request_queue_cnt].tailp = &request_queue2s[request_queue_cnt].head;
#else
    /* Initialize request queue */
...
#ifdef CC_MULTI_CORE
    for (i = 0; i < 20 / opt_total_squids; i++) {
#else
    #ifdef CC_FRAMEWORK
    for (i = 0; i < 2; i++) {
    #else
...
#ifdef CC_FRAMEWORK
	threadp->sd_index = request_queue_cnt;
#endif
	threads = threadp;
...
#ifdef CC_FRAMEWORK
	pthread_mutex_lock(&request_queues[threadp->sd_index].mutex);
	while (!request_queues[threadp->sd_index].head) {
	    pthread_cond_wait(&request_queues[threadp->sd_index].cond, &request_queues[threadp->sd_index].mutex);
	}
	request = request_queues[threadp->sd_index].head;
	if (request)
	    request_queues[threadp->sd_index].head = request->next;
	if (!request_queues[threadp->sd_index].head)
	    request_queues[threadp->sd_index].tailp = &request_queues[threadp->sd_index].head;
	pthread_mutex_unlock(&request_queues[threadp->sd_index].mutex);
#else
	...
	if (!request->cancelled) {
		switch (request->request_type) {
			...
			case _AIO_OP_UNLINK:
				...
				break;
#ifdef CC_FRAMEWORK
			case _AIO_OP_REAL_UNLINK:
				squidaio_do_real_unlink(request);
				break;  
#endif
#if USE_TRUNCATE
...
#ifdef CC_FRAMEWORK
	    case _AIO_OP_ZCOPY:
		debug(110, 3)("Copying data with sendfile Yeah!\n");
		squidaio_do_zcopy(request);
		break;
#endif
...
#ifdef CC_FRAMEWORK
static void
squidaio_queue_request(squidaio_request_t * request, int sd_index)
#else
static void
	...
	
{
    ...
    request->next = NULL;
#ifdef CC_FRAMEWORK
    if (!request_queue2s[sd_index].head) {
	    if (pthread_mutex_trylock(&request_queues[sd_index].mutex) == 0) {
		    /* Normal path */
		    *request_queues[sd_index].tailp = request;
		    request_queues[sd_index].tailp = &request->next;
		    pthread_cond_signal(&request_queues[sd_index].cond);
		    pthread_mutex_unlock(&request_queues[sd_index].mutex);
	    } else {
		    /* Oops, the request queue is blocked, use request_queue2 */
		    *request_queue2s[sd_index].tailp = request;
		    request_queue2s[sd_index].tailp = &request->next;
	    }
    } else {
	    ...
	    *request_queue2s[sd_index].tailp = request;
	    if (pthread_mutex_trylock(&request_queues[sd_index].mutex) == 0) {
		    ...
		    *request_queues[sd_index].tailp = request_queue2s[sd_index].head;
		    request_queues[sd_index].tailp = &request->next;
		    pthread_cond_signal(&request_queues[sd_index].cond);
		    pthread_mutex_unlock(&request_queues[sd_index].mutex);
		    request_queue2s[sd_index].head = NULL;
		    request_queue2s[sd_index].tailp = &request_queue2s[sd_index].head;
	    } else {
...
#ifdef CC_FRAMEWORK
int
squidaio_open(const char *path, int oflag, mode_t mode, squidaio_result_t * resultp, int sd_index)
#else
int
...
#ifdef CC_FRAMEWORK
    squidaio_queue_request(requestp, sd_index);
#else
...
#ifdef CC_FRAMEWORK
int
squidaio_zcopy(int fd, int targetfd, int bufs, off_t offset, squidaio_result_t * resultp, int sd_index)
{
    squidaio_request_t *requestp;

    requestp = memPoolAlloc(squidaio_request_pool);
    requestp->fd = fd;
    requestp->zc_targetfd = targetfd;
    requestp->buflen = bufs;
    requestp->offset = offset;
    requestp->resultp = resultp;
    requestp->request_type = _AIO_OP_ZCOPY;
    requestp->cancelled = 0;
...
static void squidaio_do_zcopy(squidaio_request_t * requestp)
{
	requestp->ret = sendfile(requestp->zc_targetfd, requestp->fd, &requestp->offset, requestp->buflen);
	requestp->err = errno;
	//fd_bytes(requestp->zc_targetfd, requestp->ret, FD_WRITE);
...
#ifdef CC_FRAMEWORK
int
squidaio_read(int fd, char *bufp, int bufs, off_t offset, int whence, squidaio_result_t * resultp, int sd_index)
...
#ifdef CC_FRAMEWORK
int
squidaio_write(int fd, char *bufp, int bufs, off_t offset, int whence, squidaio_result_t * resultp, int sd_index)
#else
int
...
#ifdef CC_FRAMEWORK
int
squidaio_close(int fd, squidaio_result_t * resultp, int sd_index)
#else
int
...
#ifdef CC_FRAMEWORK
int
squidaio_stat(const char *path, struct stat *sb, squidaio_result_t * resultp, int sd_index)
#else
int
...
#ifdef CC_FRAMEWORK
int
squidaio_unlink(const char *path, squidaio_result_t * resultp, int sd_index, int disk_error_flag)
#else
int
...
#endif
{
	...
	ret = cc_call_hook_func_private_needUnlink();
	if(ret)
	{
		requestp->request_type = _AIO_OP_REAL_UNLINK;
	}
	else
	{
	...
	if (disk_error_flag)
	{
		debug(20, 1)("make a real unlink for disk error\n");
		requestp->request_type = _AIO_OP_REAL_UNLINK;
	}
#else
	requestp->request_type = _AIO_OP_UNLINK;
...
#ifdef CC_FRAMEWORK
static void
squidaio_do_real_unlink(squidaio_request_t * requestp)
{
	requestp->ret = unlink(requestp->path);
	//    requestp->ret = unlink(requestp->path);
	requestp->err = errno;
...
#ifdef CC_FRAMEWORK
int
squidaio_truncate(const char *path, off_t length, squidaio_result_t * resultp, int sd_index)
#else
int
...
squidaio_poll_queues(void)
{
	...
	int i;
	for(i = 0; i <= request_queue_cnt ; i ++)
	{
		if (request_queue2s[i].head &&
				pthread_mutex_trylock(&request_queues[i].mutex) == 0) {
			*request_queues[i].tailp = request_queue2s[i].head;
			request_queues[i].tailp = request_queue2s[i].tailp;
			pthread_cond_signal(&request_queues[i].cond);
			pthread_mutex_unlock(&request_queues[i].mutex);
			request_queue2s[i].head = NULL;
			request_queue2s[i].tailp = &request_queue2s[i].head;
		}
	}
#else
    /* kick "overflow" request queue */
...
squidaio_stats(StoreEntry * sentry)
{
    ...
    threadp = threads;
#ifdef CC_FRAMEWORK
    for (i = 0; i < cc_squidaio_nthreads; i++) {
#else
    for (i = 0; i < squidaio_nthreads; i++) {
#endif
