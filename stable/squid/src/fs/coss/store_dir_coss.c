...
#ifdef CC_FRAMEWORK
static STLIMITOBJ storeCossLimitObj;
#endif
...
void
storeCossAdd2(SwapDir * sd, StoreEntry * e)
{
    CossInfo *cs = (CossInfo *) sd->fsdata;
    int curstripe = storeCossFilenoToStripe(cs, e->swap_filen);
    ...
    cs->count++;
    if(curstripe > cs->curstripe)
        cs->curstripe = curstripe + 1;
}
...
#ifdef CC_FRAMEWORK
static void
storeCossRebuildComplete2(RebuildState *data)
{
    RebuildState *rb = data;
    SwapDir *SD = rb->sd;
    CossInfo *cs = SD->fsdata;
    ...
    debug(47, 1) ("COSS: %s: Begin to start Membuf\n", stripePath(SD));
    storeCossStartMembuf2(SD);
    store_dirs_rebuilding--;
    storeCossDirCloseTmpSwapLog(SD);
    storeRebuildComplete(&rb->counts);
    debug(47, 1) ("COSS: %s: Rebuild Completed\n", stripePath(SD));
...
/*
   static void
   storeCoss_AddStoreEntry2(RebuildState * rb, const cache_key * key, StoreEntry * e)
   {
   StoreEntry *ne;
   SwapDir *SD = rb->sd;
//CossInfo *cs = SD->fsdata;
...
EBIT_SET(ne->flags, ENTRY_VALIDATED);
storeHashInsert(ne, key);	
storeCossAdd2(SD, ne);

StoreEntry* te;
te = (StoreEntry *) hash_lookup(store_table, key);
...
static StoreEntry *
storeCossDirAddDiskRestore(RebuildState* rb, SwapDir * SD, const cache_key * key,
        sfileno file_number,
        ...
        int clean)
{
    ...
    storeSetMemStatus(ne, NOT_IN_MEMORY);
    ne->swap_status = SWAPOUT_DONE;
    ne->swap_filen = file_number;
    ne->swap_dirn = SD->index;
    ne->swap_file_sz = swap_file_sz;
    ne->lock_count = 0;
    ne->lastref = lastref;
    ne->timestamp = timestamp;
    ne->expires = expires;
    ne->lastmod = lastmod;
    ne->refcount = refcount;
    ne->flags = flags;
    EBIT_SET(ne->flags, ENTRY_CACHABLE);
    EBIT_CLR(ne->flags, RELEASE_REQUEST);
    EBIT_CLR(ne->flags, KEY_PRIVATE);
...
static int
storeCossDirValidFileno(SwapDir * SD, sfileno filn, int flag)
{
    CossInfo *cs = (CossInfo *) SD->fsdata;
    debug(20,3)("s.swap_filen = %d, cs->max_disk_nf = %d\n", filn, cs->max_disk_nf);
    if(filn >= cs->max_disk_nf || filn < 0)
        return 0;
    else
        return 1;
}
...
static void
storeCossDirRebuildFromSwapLog(void *data)
{
    RebuildState *rb = data;
    SwapDir *SD = rb->sd;
    StoreEntry *e = NULL;
    ...
    for (count = 0; 1 ; count++) {
        ...
        if (!storeCossDirValidFileno(SD, s.swap_filen, 0))
        {
            rb->counts.invalid++;
            continue;
        }
        ...
        if(e && s.lastmod < e->lastmod)
            continue;
        else if(e && e->swap_filen == s.swap_filen && e->swap_dirn == SD->index)
        {
            if (e->store_status == STORE_OK) {
                ...
                debug_trap("storeCossDirRebuildFromSwapLog: bad condition");
                debug(47, 1) ("\tSee %s:%d\n", __FILE__, __LINE__);
            }
            continue;
        }
        ...
        ne = storeCossDirAddDiskRestore(rb, SD, s.key,
                s.swap_filen,
                s.swap_file_sz,
                s.expires,
                s.timestamp,
                ...
                s.flags,
                (int) rb->flags.clean);
        if(ne && ne->swap_filen > -1 && ne->swap_status == SWAPOUT_DONE && EBIT_TEST(ne->flags, ENTRY_VALIDATED))
        {
            storeDirUpdateSwapSize(&Config.cacheSwap.swapDirs[ne->swap_dirn], ne->swap_file_sz, 1);
        }
        storeDirSwapLog(ne, SWAP_LOG_ADD);
...
static void
storeCossDirRebuildFromSwapLogCheckVersion(void *data)
{
    RebuildState *rb = data;
    storeSwapLogHeader hdr;
    hdr.record_size = hdr.version = hdr.op = 0;
	if (fread(&hdr, sizeof(storeSwapLogHeader), 1, rb->log) != 1
	    || fseek(rb->log, hdr.record_size, SEEK_SET) != 0
       ) 
		goto err;
	...
	{
		storeCossDirRebuildFromSwapLog(rb);
		return;
	}

err:
    debug(50,1)("Coss swap.state fail reason  OP : %d  Version : %d  RecordSize : %d  storeSwapLogData : %d\n",hdr.op,  hdr.version, hdr.record_size, sizeof(storeSwapLogData));
	storeCossRebuildComplete2(rb);
	//fatal("storeCossDirRebuildFromSwapLogCheckVersion: I dont know this swap.stat.");
}
#endif
...
storeCossDirRebuild(SwapDir * sd)
{
    ...
    if(NULL == fp)
    {
        store_dirs_rebuilding++;
        storeCossRebuildComplete2(rb);
        debug(47, 1) ("Rebuilding storage in %s: cant open swap file correctly, rebuild abort\n",sd->path);
        return;
    }
    rb->log = fp;
    rb->flags.clean = (unsigned int) clean;
...
#ifdef CC_FRAMEWORK
    /*
     *Dont use event! for case 0001992. It will case assert fail. jyp fix it at 2011/4/14
     */
    //eventAdd("storeCossRebuild", storeCossDirRebuildFromSwapLogCheckVersion, rb, 0.0, 0);
    storeCossDirRebuildFromSwapLogCheckVersion(rb);
#else
...
static void
storeCossWriteSwapLogheader(int fd)
{
...
storeCossDirOpenTmpSwapLog(SwapDir * sd, int *clean_flag, int *zero_flag)
{
    ...
    *zero_flag = log_sb.st_size == 0 ? 1 : 0;
    ...
    cs->swaplog_fd = fd;
#ifdef CC_FRAMEWORK
    storeCossWriteSwapLogheader(fd);
#endif
...
storeCossDirWriteCleanStart(SwapDir * sd)
{
    ...
    state->strip_cur = cs->curstripe;
    state->offset_cur = -1;
#ifndef CC_FRAMEWORK
    sd->log.clean.write = NULL;
    sd->log.clean.state = NULL;
#else
    storeCossWriteSwapLogheader(state->fd);
    state->strip_cur = cs->numstripes;
#endif
...
storeCossDirCleanLogNextEntry(SwapDir * sd)
{
    ...
    loop = state->strip_cur;
    offset = state->offset_cur;

find:
    for(state->current=NULL;; loop--,offset=-1)
    {
        if(loop < 0)
            return NULL;
        cstripe = &cs->stripes[loop];
        state->current = cstripe->objlist.head;
        if(state->current)
            break; 
    }
    ...
    state->offset_cur = offset + 1;
    state->strip_cur = loop;
    return entry;
#endif
}
...
storeCossCreateStripe(SwapDir * SD, const char *path)
{
    ...
    int result = 0;
    result = storeDirGetUFSStats(SD->path, &totl_kb, &free_kb, &totl_in, &free_in);
    if(result)
    {
        fatalf("Can not check filesystem space before creating coss%s\n", xstrerror());
    }
#ifdef CC_MULTI_CORE
	if(free_kb * 1024 < (off_t)cs->numstripes * COSS_MEMBUF_SZ * opt_total_squids)
	{
		fatalf("Failed to create COSS stripe %s, free=%" PRIu64 "KB, coss size=%" PRIu64 "KB\n", path, free_kb, (off_t)cs->numstripes * COSS_MEMBUF_SZ / 1024 * opt_total_squids);
	}
	else
	{

		debug(47,1)("Space is ok to create COSS stripe %s, free=%" PRIu64 "KB, coss size=%" PRIu64 "KB\n", path, free_kb, (off_t)cs->numstripes * COSS_MEMBUF_SZ / 1024 * opt_total_squids);
	}
#else
	if(free_kb * 1024 < (off_t)cs->numstripes * COSS_MEMBUF_SZ)
	{    
		fatalf("Failed to create COSS stripe %s, free=%" PRIu64 "KB, coss size=%" PRIu64 "KB\n", path, free_kb, (off_t)cs->numstripes * COSS_MEMBUF_SZ / 1024);
	}    
	else    
	{    

		debug(47,1)("Space is ok to create COSS stripe %s, free=%" PRIu64 "KB, coss size=%" PRIu64 "KB\n", path, free_kb
				, (off_t)cs->numstripes * COSS_MEMBUF_SZ / 1024);
	}
#endif
    lseek(swap, ((off_t)cs->numstripes - 1) * COSS_MEMBUF_SZ, SEEK_SET);
    if (write(swap, block, COSS_MEMBUF_SZ) < COSS_MEMBUF_SZ) {
        fatalf("Failed to create COSS stripe %s\n", path);
    }
...
{
	struct stat st;
	if (stat(SD->path, &st) == 0)       // coss exits
	{
		if (S_ISDIR(st.st_mode)) 
		{
            struct stat st1;
			if (stat(stripePath(SD), &st1) != 0)        // stripe does not exist
				storeCossCreateStripe(SD, stripePath(SD));
		}
#ifdef CC_FRAMEWORK		// added by chenqi, case 3059
        else if (S_ISREG(st.st_mode)) 
		{
			if (NULL == Config.Log.swap)
				fatalf("storeCossDirNewfs: %s is a file, cache_swap_log is needed in your COSS configuration.\n", SD->path);
		}
#endif					// added end
	} 
	else                // coss does not exist
		{
#ifdef CC_FRAMEWORK		// added by chenqi, case 3059
			if (0 == mkdir(SD->path, 0755))
				debug(47,1)("COSS: creating directory %s\n", SD->path);
			else
				fatalf("storeCossDirNewfs: cannot creating derectory %s\n", SD->path);
...
#ifdef CC_FRAMEWORK
void
storeCossLimitObj(SwapDir* SD)
{
    CossInfo *cs = (CossInfo *) SD->fsdata;

    if( cs->curstripe > cs->rmark )
    {
        //cs->mark = cs->current_offset;
...
#ifdef CC_FRAMEWORK
    cs->blksz_bits = 12;	/* default block size = 4096 */
#else
...
#endif /* CC_FRAMEWORK */
	//cs->nummemstripes = (int)(membufs/opt_total_squids);
...
#ifdef CC_FRAMEWORK
    aioRead(cs->fd, (off_t) cs->rebuild.curstripe * COSS_MEMBUF_SZ, COSS_MEMBUF_SZ, storeDirCoss_ReadStripeComplete, rb, SD->index);
#else
