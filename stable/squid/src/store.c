...
#ifdef CC_FRAMEWORK
void storeHashDelete(StoreEntry *);
#else
...
#ifdef CC_FRAMEWORK
	cc_free_MemObject_private_data(mem);
#endif
...
#ifdef CC_FRAMEWORK
	safe_free(mem->store_url);/*zhoudshu fix one bug for store_url memory leak problem at 2011-02-24*/
#endif
...
#ifdef CC_FRAMEWORK
	cc_free_StoreEntry_private_data(e);
#endif
...
storeAppend(StoreEntry * e, const char *buf, int len)
{
	...
	if (len)
	{
		debug(146, 5) ("storeAppend: appending %d bytes for '%s'\n",
					  len,
					  storeKeyText(e->hash.key));
...
storeCheckCachable(StoreEntry * e)
{
	...
	if (e->mem_obj->method != METHOD_GET && e->mem_obj->method != METHOD_HEAD)
	{
		debug(20, 2) ("storeCheckCachable: NO: non-GET method\n");
#ifdef CC_FRAMEWORK
		cc_call_hook_func_store_no_cache_reason(e, NCR_NOT_METHOD_CACHABLE);
#endif
		...
		if (e->store_status == STORE_OK && EBIT_TEST(e->flags, ENTRY_BAD_LENGTH))
		{
			...
			cc_call_hook_func_store_no_cache_reason(e, NCR_BAD_LENGTH);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_RELEASE_REQUEST);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_NOT_ENTRY_CACHABLE);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_NEG_CACHED);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_TOO_BIG);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_TOO_SMALL);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_KEY_PRIVATE);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_TOO_MANY_DISK_FILES_OPEN);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_TOO_MANY_FD_OPEN);
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_store_no_cache_reason(e, NCR_NONE);
#endif
			return 1;
		}
	storeReleaseRequest(e);
...
storeComplete(StoreEntry * e)
{
	...
	InvokeHandlers(e);
#ifdef CC_FRAMEWORK
        cc_call_hook_func_store_complete(e);
#endif
}
...
storeMaintainSwapSpace(void *datanotused)
{
	...
	if (store_swap_size > Config.Swap.maxSize)
	{
		...
		{
#ifdef CC_FRAMEWORK
			debug(20, 0) ("WARNING: Disk space over limit: %" PRINTF_OFF_T " KB > %" PRINTF_OFF_T " KB\n", store_swap_size, Config.Swap.maxSize);
#else
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_setDiskErrorFlag(e, 0);
#endif
...
storeEntryValidLength(const StoreEntry * e)
{
	...
	debug(20, 5) ("storeEntryValidLength: content_length = %" PRINTF_OFF_T "\n",
				  clen);
#ifdef CC_FRAMEWORK
	int ret = call_hook_func_private_store_entry_valid_length((StoreEntry*)e);
	if(ret > 0 ){
		return 0;
	}
#endif
...
#ifdef CC_FRAMEWORK
	debug(20, 1) ("Swap maxSize %llu + %llu KB, estimated %ld objects\n",
				  Config.Swap.maxSize, (Config.memMaxSize >> 10), i);
#else
...
#ifdef CC_FRAMEWORK
	debug(20, 1) ("Max Swap size: %llu KB\n", Config.Swap.maxSize);
#else
...
storeTimestampsSet(StoreEntry * entry)
{
...
#ifdef CC_FRAMEWORK
	int ret;
	ret = cc_call_hook_func_private_modify_entry_timestamp(entry);
	if(1 == ret || 0 == ret)
		served_date = squid_curtime;
	else if (2 == ret)
		served_date = squid_curtime - age;
	...
	if (reply->expires > 0 && reply->date > -1)
	{
		...
		debug(22,2)("XIAOSI set entry->expires1=%d+(%d-%d)\n",served_date,
				reply->expires,
				reply->date);
	}
	else
	{
		...
		debug(22,2)("XIAOSI set entry->expires2=%d\n",reply->expires);
	}
