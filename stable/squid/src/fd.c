...
fd_close(int fd)
{
    ...
    {    
        for (i = 0; i < Config.cc_all_mod_num; i++) 
        {    
            p_data = &(F->cc_fde_private_data[i]);
            if (p_data && p_data->private_data && p_data->call_back)
                p_data->call_back(p_data->private_data);
        }   
        memPoolFree(cc_private_data_pool, F->cc_fde_private_data);
        F->cc_fde_private_data = NULL;
    }    
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_fd_bytes(fd, len, type);
#endif
	if (type == FD_READ)
		F->bytes_read += len;
	else
