...
{
#ifdef CC_FRAMEWORK
	return cc_find_refresh(url);
	//return cc_call_hook_func_private_refresh_limits(url);
#else
	const refresh_t *R;
...
refreshStaleness(const StoreEntry * entry, time_t check_time, time_t age, const refresh_t * R, stale_flags * sf)
{
	...
	assert(age >= 0);
#ifdef CC_FRAMEWORK
	cc_call_hook_func_refresh_staleness((StoreEntry *)entry);
#endif
	/*
...
refreshCheck(const StoreEntry * entry, request_t * request, time_t delta)
{
	...
	debug(22,2)("XIAOSI age=%d\n",age);
#ifdef CC_FRAMEWORK
	int hook_ret;
	time_t cc_age;
	if((hook_ret = cc_call_hook_func_refresh_check_pre_return(age, delta, request, (StoreEntry *)entry, &cc_age)) >= 0)
	{
		return hook_ret;
	}
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_refresh_check_set_expires((StoreEntry *)entry, cc_age, R);
#endif
