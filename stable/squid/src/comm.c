...
#ifdef CC_FRAMEWORK
	if (addr.s_addr != no_addr.s_addr && (addr.s_addr != INADDR_ANY || port != 0))
#else
...
commConnectDnsHandle(const ipcache_addrs * ia, void *data)
{
...
#ifdef CC_FRAMEWORK
    cc_call_hook_func_private_comm_connect_polling_serverIP(ia, cs->fd, cs); 
#endif
	...
	/*
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_dup_persist_fd(cs->fd,cs);
#endif
*/
...
comm_close(int fd)
{
	...
	if (shutting_down && (!F->flags.open || F->type == FD_FILE))
		return;
	/*add by fangfang.yang for case 4806*/
#ifdef CC_FRAMEWORK
	if (shutting_down && (!F->flags.open || F->type == FD_PIPE))
	{
		close(fd);
		return;
	}
#endif
	/*add by fangfang.yang end*/
	assert(F->flags.open);
...
commHandleWrite(int fd, void *data)
{
	...
	debug(146, 5) ("commHandleWrite: FD %d: off %ld, hd %ld, sz %ld.\n",
...
commHandleWrite(int fd, void *data)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_individual_limit_speed(fd,data,&nleft,commHandleWrite);
	if(nleft==0) {
		return;
	}
#endif
	...
	else
	{
		...
		else
		{
			...
			CommWriteStateCallbackAndFree(fd, COMM_OK);
			#ifdef CC_FRAMEWORK
				cc_call_hook_func_private_mp4_read_moov_body(fd,state);
			#endif
...
#ifdef CC_FRAMEWORK
static void
commHandleZWrite(int fd, void *data)
{
	int len = 0;
	int nleft;
	CommWriteStateData *state = &fd_table[fd].rwstate;
	...
	debug(5, 5) ("commHandleZWrite: FD %d: off %ld, hd %ld, sz %ld.\n",
				 fd, (long int) state->offset, (long int) state->header_size, (long int) state->size);

	nleft = state->size + state->header_size - state->offset;
	debug(113,3)("nleft=%d state->size=%d, state->header_size=%d state->offset=%d\n",
			nleft,state->size,state->header_size,state->offset);
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_individual_limit_speed(fd,data,&nleft, commHandleZWrite);
	if(nleft==0) {
		return;
	}
#endif
	...
	else
	{
		//len = FD_WRITE_METHOD(fd, state->buf + state->offset - state->header_size, nleft);
		len = state->size;
		//printf("len = state->size = %d\n", len);
	}
	debug(5, 5) ("commHandleZWrite: write() returns %d\n", len);
	//fd_bytes(fd, len, FD_WRITE);
	statCounter.syscalls.sock.writes++;
	...
	if (len == 0)
	{
		...
		/* We're done */
		if (nleft != 0)
			debug(5, 1) ("commHandleZWrite: FD %d: write failure: connection closed with %d bytes remaining.\n", fd, nleft);
		CommWriteStateCallbackAndFree(fd, nleft ? COMM_ERROR : COMM_OK);
	}
	else if (len < 0)
	{
		...
		if (fd_table[fd].flags.socket_eof)
		{
			debug(5, 2) ("commHandleZWrite: FD %d: write failure: %s.\n",
						 fd, xstrerror());
			CommWriteStateCallbackAndFree(fd, COMM_ERROR);
		}
		else if (ignoreErrno(errno))
		{
			debug(5, 10) ("commHandleZWrite: FD %d: write failure: %s.\n",
						  fd, xstrerror());
			commSetSelect(fd,
						  COMM_SELECT_WRITE,
						  commHandleZWrite,
	...
	else
	{
		...
		//else
		//{
		//printf("is_zcopy = %d before CommWriteStateCallbackAndFree\n",((store_client *)fd_table[fd].rwstate.handler_data)->is_zcopy);
			CommWriteStateCallbackAndFree(fd, COMM_OK);
		//}
	}
}
...
#ifdef CC_FRAMEWORK
/**
 * callback is commHandleZWrite,for zerocopy
 */
void
comm_z_write(int fd, int size, CWCB * handler, void *handler_data, FREE * free_func)
{
	CommWriteStateData *state = &fd_table[fd].rwstate;
	debug(5, 5) ("comm_z_write: FD %d: sz %d: hndl %p: data %p.\n",
				 fd, size, handler, handler_data);
	if (state->valid)
	{
		debug(5, 1) ("comm_z_write: fd_table[%d].rwstate.valid == true!\n", fd);
		fd_table[fd].rwstate.valid = 0;
	}
	state->size = size;
	state->header_size = 0;
	...
	state->valid = 1;
	cbdataLock(handler_data);
	commSetSelect(fd, COMM_SELECT_WRITE, commHandleZWrite, NULL, 0);
}
#endif
...
#ifdef CC_FRAMEWORK
void
comm_z_write_mbuf(int fd, MemBuf mb, CWCB * handler, void *handler_data)
{
	comm_z_write(fd, mb.size, handler, handler_data, memBufFreeFunc(&mb));
}
#endif
...
#ifdef CC_FRAMEWORK
void commEnhanceSndBuf(int fd)
{
	int sndbuflen;
	int sizeofint = sizeof(int);
	if(getsockopt(fd, SOL_SOCKET, SO_SNDBUF, (void *) &sndbuflen, &sizeofint) < 0)
	{
		debug(5, 0)("get SO_SNDBUF failed! %s\n", strerror(errno));
	}
	sndbuflen = sndbuflen * 10;
	if(setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sndbuflen, sizeofint) < 0)
	{
		debug(5, 0)("set SO_SNDBUF failed! %s\n", strerror(errno));
		
	}

}

void commSetCork(int fd)
{

	int off = 0;
	if(setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &off, sizeof(off)) < 0)
	{
		debug(5, 0)("failed to set TCP_NODELAY before set CORK\n");
	}
	int on = 1;
	if(setsockopt(fd, SOL_TCP, TCP_CORK, &on, sizeof (on)) < 0)
	{
		debug(5, 1)("set TCP_CORK failed!\n");
	}
}

void commSetNoCork(int fd)
{
	int off = 0;
	if(setsockopt(fd, SOL_TCP, TCP_CORK, &off, sizeof (off)) < 0)
	{
		debug(5, 1)("unset TCP_CORK failed!\n");
	}
}
#endif
