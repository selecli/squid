...
clientStoreURLRewriteStart(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token("storeurl_access",http);
		if ( acl != NULL)
#else
	...
	{
#ifdef CC_FRAMEWORK
		http->acl_checklist = cc_clientAclCheckListCreate("storeurl_access",http);
#else
...
clientStoreURLRewriteDone(void *data, char *result)
{
	...
	{
#ifdef CC_FRAMEWORK		
		safe_free(http->request->store_url);//zhoudshu fix one bug of store_url memory leak at 2011-02-24
#endif
...
#ifdef CC_FRAMEWORK		
	cc_call_hook_func_http_after_client_store_rewrite(http);
#endif
