...
accessLogCustom(AccessLogEntry * al, customlog * log)
{
	/*
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_record_trend_access_log(al);
#endif
*/
	logformat *lf;
	Logfile *logfile;
	...
	for (fmt = lf->format; fmt != NULL; fmt = fmt->next)  	/* for each token */
	{
		...
		case LFT_REPLY_SIZE_HEADERS: 
			outint = al->reply->hdr_sz;
			doint = 1;
			break;
			
		case LFT_REPLY_SIZE_BODY: 
			outint = al->cache.size - al->reply->hdr_sz;
			doint = 1;
			break;
...
//#ifdef CC_FRAMEWORK
#if 0
		char *daemonstr = "daemon:";
		
		if(strncmp(log->filename, daemonstr, strlen(daemonstr)) != 0){
			if(strncmp(log->filename, "/", 1) == 0){
				char newfilename[1024];
				memset(newfilename,0,1024);
				strcpy(newfilename,daemonstr);
				strcat(newfilename,log->filename);
				log->logfile = logfileOpen(newfilename, MAX_URL << 2, 1);
