...
#endif
#ifdef CC_FRAMEWORK
ipcache_entry *ipcache_get(const char *);
void ipcacheRelease(ipcache_entry *);
int ipcacheExpiredEntry(ipcache_entry *);
#else
...
// added by chenqi @ 2012/03/21
// return 0 represents fail, 1 represents success
// this function takes place of ipcache_testname, 
// now, we test dns_nameservers, not /etc/resolv.conf
#ifdef CC_FRAMEWORK
#include <resolv.h>
...
ipcache_testdns()
{
	debug(14, 1)("Performing DNS Name Server Tests...\n");
	wordlist *w = Config.dns_nameservers;
	if (NULL == w)	// still failed 
		return 0;
	...
	int i;
	_res.nscount = 0;
	_res.retry = 3;	    // retry times when fail
	for (i=0; w; w=w->next, i++) 
	{
		_res.nsaddr_list[i].sin_family = AF_INET;
		_res.nsaddr_list[i].sin_port = htons(53);	// well known port 53 is used by dns
		inet_pton(AF_INET, w->key, &_res.nsaddr_list[i].sin_addr);
		_res.nscount++;
	}
	for (i=_res.nscount; i<MAXNS; i++) 
		memset(&_res.nsaddr_list[i], 0, sizeof(struct sockaddr_in));

	ns_msg ns;
	ns_rr rr;
	unsigned char ans[65536];
	int ans_len;
	wordlist *name = Config.dns_testname_list;
	for (; name; name = name->next)
	{
		if (ans_len = (res_search(name->key, C_IN, T_A, ans, 65536)))
			if (ns_initparse(ans, ans_len, &ns) == 0)
				for (i=0; i<ns_msg_count(ns, ns_s_an); i++)
				{
					if (ns_parserr(&ns, ns_s_an, i, &rr) == 0)
						if (ns_rr_type(rr) == ns_t_a)	    // A record find
							return 1;
				}
	}
	return 0;
...
#ifndef CC_FRAMEWORK	    // rpm incompetible
	else if (!ipcache_testdns())	    // test dns_nameservers
#else
	else if (!ipcache_testname())	    // test /etc/resolv.conf
#endif 
	{
