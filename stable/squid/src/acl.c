...
#ifdef CC_FRAMEWORK
void aclLookupProxyAuthStart(aclCheck_t * checklist);
#else
static void aclLookupProxyAuthStart(aclCheck_t * checklist);
#endif
#ifdef CC_FRAMEWORK
void aclLookupProxyAuthDone(void *data, char *result);
#else
...
aclFindByName(const char *name)
{
	...
	{
		for (a = Config_r.aclList; a; a = a->next)
	...
	while ((t = strtokFile()))
	{
		...
		if ((errcode = regcomp(&comp, t, flags)) != 0)
		{
			...
			debug(28, 0) ("aclParseRegexList: Invalid regular expression '%s': %s\n",
						  t, errbuf);
#ifdef CC_FRAMEWORK
            cc_framework_check_start("Invalid Regex");
#endif
            regfree(&comp);						  
			continue;
		}
...
aclParseAclLine(acl ** head)
{
	...
	{
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Empty ACL");
#endif
...
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Missing error page");
#endif
		return;
	}
...
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Not contains ACL");
#endif
...
aclParseAccessLine(acl_access ** head)
{
	...
	{
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Not found Switch");
#endif
		debug(28, 0) ("%s line %d: %s\n",
					  cfg_filename, config_lineno, config_input_line);
...
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Not match Switch");
#endif
		cbdataFree(A);
		return;
	}
	...
	if (A->acl_list == NULL)
	{
		debug(28, 0) ("%s line %d: %s\n",
					  cfg_filename, config_lineno, config_input_line);
#ifdef CC_FRAMEWORK
        cc_framework_check_start("Not found acl_name");
#endif
...
//		rfc1738_unescape(esc_buf);
#ifdef CC_FRAMEWORK
		int ret = cc_call_hook_func_private_aclMatchRegex(ae,r);
		if(ret==1)
		{
			safe_free(esc_buf);
			return ret;
...
aclCheck(aclCheck_t * checklist)
{
	allow_t allow = ACCESS_DENIED;
#ifdef CC_FRAMEWORK
	acl_access* aa = (acl_access *)checklist->access_list;
	int ret = cc_call_hook_func_private_before_loop(&allow,checklist,aa,NULL,1,NULL);
	if(ret ==1)
	{
	...
	while ((A = checklist->access_list) != NULL)
	{
		...
		if (match)
		{
			...
			cbdataUnlock(A);
			//acl_access* cl=checklist->access_list;
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_before_acl_callback(allow ,checklist,aa);
#endif
			checklist->access_list = NULL;
			aclCheckCallback(checklist, allow);
			//checklist->access_list=NULL;
	...
	/*
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_before_acl_callback(allow != ACCESS_DENIED ? ACCESS_DENIED : ACCESS_ALLOWED ,checklist,aa);
#endif
*/
	allow = ACCESS_DENIED;
