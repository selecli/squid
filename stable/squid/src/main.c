...
#ifdef CC_FRAMEWORK
static volatile int do_swaplog = 0;
extern void redirectReloadStart( void *unused);
#endif
#ifndef CC_FRAMEWORK
...
#endif
			"       -k reconfigure|rotate|shutdown|interrupt|kill|debug|check|parse|swaplog\n"
...
mainParseOptions(int argc, char *argv[])
{
	...
	{
#elif defined CC_MULTI_CORE
	while ((c = getopt(argc, argv, "CDFINRSYXd:f:hk:m::sl:u:VvzA:a:?")) != -1)
	{
#elif defined CC_FRAMEWORK
	while ((c = getopt(argc, argv, "CDFINRSYXd:f:hk:m::sl:u:Vvz?")) != -1)
	{
#else 
...
#endif
#ifdef CC_FRAMEWORK
			else if (!strncmp(optarg, "swaplog", strlen(optarg)))
				opt_send_signal = SIGRTMIN+6;
#endif
		...
		case 'v':
			{
				...
				printf("FC Cache: Version %s\nconfigure options: %s\n", version ? version : version_string, SQUID_CONFIGURE_OPTIONS);
				xfree(version);
				cc_print_mod_ver(0);
#if defined(USE_WIN32_SERVICE) && defined(_SQUID_WIN32_)
				printf("Compiled as Windows System Service.\n");
...
#ifdef CC_MULTI_CORE
		case 'A':
			opt_total_squids = atoi(optarg);
			assert(opt_total_squids < 1000);
			break;
		case 'a':
			opt_squid_id = atoi(optarg);
			assert(opt_squid_id < 1000);
			break;
#endif
		case '?':
		default:
...
#ifdef CC_FRAMEWORK
int rotate_equal_one(void)
{
	do_rotate = 1;
	debug(1, 5)("Execution do_rotate equal %d\n", do_rotate);
	return do_rotate;
}
#endif
extern int parseConfigFile_r(const char* file_name);
#ifdef CC_FRAMEWORK
int reconfiguring_in_thread = 0;
#endif
...
serverConnectionsOpen(void)
{
	...
	if (reconfiguring)
	{
		if(http_port_cfg_changed)
		{
			clientOpenListenSockets();
			http_port_cfg_changed = 0;
		}
	}
	else
#endif 
...
serverConnectionsClose(void)
{
	...
	if (reconfiguring)
	{
		if(http_port_cfg_changed)
			...
			debug(0, 1)("Hold client connections when reconfiguring for the \"http_port\" and \"https_port\" config is not motified\n");
	}
	else
#endif
		clientHttpConnectionsClose();
...
extern void mem_dump_debug(const char *info);
void mainReconfigure_done()
{
	do_reconfigure = 0;

	if(1 == reconfigure_in_thread)/*no more RELOAD signal reached during reconfiguring*/
	{
	    reconfigure_in_thread = 0;    
	}

	/*if more RELOAD signal reached during reconfiguring, should reconfigure_in_thread > 1*/
	
    debug(1, 1) ("Reconfiguring Squid Cache (version %s) done and reconfigure_in_thread = %d\n", version_string, reconfigure_in_thread);
	return;
}
#ifdef CC_FRAMEWORK
void
...
#ifdef CC_FRAMEWORK
	int redirect_should_reload = redirectShouldReload();
	int redirect_should_restart = redirectShouldRestart();
	//commented by fangfang.yang for case 4818
#endif 
	redirectShutdown();
	storeurlShutdown();
...
//	errorClean();
    ...
    {
        debug(3,0)("%s: cc_call_hook_func_module_reconfigure beg\n", __func__);
    }
    cc_call_hook_func_module_reconfigure(redirect_should_reload);
    if(Config.onoff.debug_80_log)
    {
        debug(3,0)("%s: cc_call_hook_func_module_reconfigure end\n", __func__);
    }

    //mem_dump_debug("after_parseConfigFile_r beg\n");
	after_parseConfigFile_r(ConfigFile); // copy and clean 
	//mem_dump_debug("after_parseConfigFile_r end\n");
	
	cc_call_hook_func_sys_after_parse_param();
#else
	...
	eventCleanup();
										//After eventCleanup(), we just add new redirect reload event.
	if (redirect_should_reload && !redirect_should_restart)
	{
		debug(1, 1) ("mainReconfigure: Begin to add reload task by eventAdd()!\n");
		eventAdd("redirectReloadStart", redirectReloadStart, NULL, 0, 0);	
	}
...
#ifdef CC_FRAMEWORK
    if (0 == eventFind(cc_free_mod_param_r, NULL))
    {
        cc_free_mod_param_r();      // eventCleanup is invalid to this 
    }
	cc_call_hook_func_before_sys_init();
	cc_call_hook_func_sys_init();       // this hook should only called once for each module
	//do_reconfigure = 0;
	//reconfigure_in_thread = 0;
#endif
}
...
setEffectiveUser(void)
{
	...
	return;
#endif
/* permit squid running as privilegied user */
#ifdef CC_FRAMEWORK
	if (strcmp(Config.effectiveGroup, "root") 
	    && strcmp(Config.effectiveUser, "root") )
	{
		if (geteuid() == 0)
		{
...
#endif
#ifdef CC_FRAMEWORK
	squid_signal(SIGRTMIN+6, swap_logs, SA_RESTART);
#endif
...
#endif
{
	...
	/*reduce the mmap threshold in glibc from default 128k to 32k*/
...
#ifdef CC_FRAMEWORK
	cc_init_hook_to_module();
	sigemptyset(&not_parse_config);
	sigaddset(&not_parse_config, SIGHUP);
    cc_framewrok_check_clean();
#endif
...
#ifdef CC_FRAMEWORK
        /* just send signal, do not parseConfigFile when reload, added by chenqi */
        if (1 == sigismember(&not_parse_config, opt_send_signal))
        {
            sendSignal();
            debug(0,0)("reload failed!!\n");
            assert(0);
        }
        /* just send signal, end */

		/* only xcalloc cc_modules when squid -D */
		if ( -1 == cc_create_modules()) 
		// if ( !opt_parse_cfg_only && !opt_create_swap_dirs && (-1 == cc_create_modules())) 
			return 0;
#endif
	...
	{
#ifdef CC_FRAMEWORK
        if (do_reconfigure == 0 && reconfigure_in_thread >0)
        { 
            reconfigure_in_thread = do_reconfigure = 1;
            parseConfigFile_r(ConfigFile);
        }
#else
		if (do_reconfigure)
		{
		...
		{
			storeDisDirWriteCleanLogs(NULL);
			do_swaplog = 0;
		}
#endif
		else if (do_shutdown)
...
#ifdef CC_FRAMEWORK
	if (1 == sigismember(&not_parse_config, opt_send_signal))
        pid = readRunningPidFile();
    else
#endif
        pid = readPidFile();
	if(0 == opt_send_signal)
	{    
		/*when execute 'squid -k check', just check, don`t send signal*/
		exit(0);     
	} 
    if (pid > 1)
    {
...
{
#ifdef CC_FRAMEWORK
	/* framework and module cleanup */
	//cc_modules_cleanup();
	cc_destroy_hook_to_module();
	cc_destroy_hook_to_module_r();
#endif
