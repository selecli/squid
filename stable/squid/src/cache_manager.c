...
#ifdef CC_FRAMEWORK
static OBJH cachemgrrotate;
extern int rotate_equal_one();
#endif
...
#ifdef CC_FRAMEWORK
	if(!strncasecmp(mgr->action, "rotate", 6) && strncasecmp("127.0.0.1", fd_table[fd].ipaddr, 9))
	{
			storeAppendPrintf(entry,"it isn't allowed  Remote login");
			return; 
	}
#endif
	mgr->entry = entry;
...
#ifdef CC_FRAMEWORK
static void
cachemgrrotate(StoreEntry * sentry)
{
	int flag = 0;
	flag = rotate_equal_one();
	debug(16, 5)("do_rotate equal %d\n",flag);
	storeAppendPrintf(sentry, "rotate log success\n");
}
#endif
static void
...
#ifdef CC_FRAMEWORK
	cachemgrRegister("rotate",
					"Execution do_rotate equal",
					cachemgrrotate, 0, 1);
#endif
}
