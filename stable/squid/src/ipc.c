...
ipcCreate(int type, const char *prog, const char *const args[], const char *name, int *rfd, int *wfd, void **hIpc)
{
    ...
    {
        debug(54, 1) ("[DEBUG] ipcCreate: try to check path existing\n");
    }
	...
	else if (type == IPC_UNIX_STREAM)
	{
		...
		if(Config.onoff.debug_80_log)
		{
    		debug(54, 1) ("[DEBUG] ipcCreate: try to create socketpair\n");
		}
	...
	if(Config.onoff.debug_80_log)
	{
    	debug(54, 1) ("[DEBUG] ipcCreate: try to logFlush\n");
	}
	/* flush or else we get dup data if unbuffered_logs is set */
	logsFlush();
	if(Config.onoff.debug_80_log)
	{
    	debug(54, 1) ("[DEBUG] ipcCreate: try to fork\n");
	}
	if ((pid = fork()) < 0)
	{
	...
	if (pid > 0)  		/* parent */
	{
		...
		if(Config.onoff.debug_80_log)
		{
    		debug(54, 1) ("[DEBUG] ipcCreate: PARENT: try to read from child pid %u, fd %d\n", pid, prfd);
		}
	    ...
	    if(Config.onoff.debug_80_log)
	    {
            debug(54, 1) ("[DEBUG] ipcCreate: PARENT: read from child pid %u, fd %d done\n", pid, prfd);
        }
		if (x < 0)
		{
		...
		if(Config.onoff.debug_80_log)
		{
    		debug(54, 1) ("[DEBUG] ipcCreate: PARENT: Config.sleep_after_fork = %d\n", Config.sleep_after_fork);			
		}
