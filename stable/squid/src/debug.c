...
__debugLogTime_msec()
{
    ...
    tv_msec = (int)(tv.tv_usec / 1000);

    len = snprintf(buf, 127, "%4d/%02d/%02d %02d:%02d:%02d.%03d",
                    tm.tm_year + 1900, 
                    tm.tm_mon + 1, 
                    tm.tm_mday, 
                    tm.tm_hour, 
...
{
    if(0 == Config.debugLogTsSwitch)/*in sec*/
    {
        return __debugLogTime_sec(t);
    }
    return __debugLogTime_msec();
}
