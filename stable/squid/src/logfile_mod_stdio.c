...
/*    s = write(ll->fd, (char const *) buf, len);
    if (s < len && errno == EINTR)
    {
        char *new_buf = (char*) buf;
        new_buf += s;
        len -= s;
        s = write(ll->fd, new_buf, len);        // give another chance 
    }
*/
	char *new_buf = (char*) buf;
	size_t  nbw = len;
	//FIX case3310
	while((s = write(ll->fd, new_buf,nbw)) < nbw) {
		if(s >0) {
			new_buf += s;
			nbw -= s;
		}
		else if (errno != EINTR) {
			debug(50, 0) ("logfileRotate (stdio): %s: %s\n", lf->path, xstrerror());
			lf->flags.fatal = 0;
			break;
		}
	}
#else
    s = FD_WRITE_METHOD(ll->fd, (char const *) buf, len);       // this called may be interrupted by rotate signal
#endif

#ifdef CC_FRAMEWORK 
    fd_bytes(ll->fd, len, FD_WRITE);
    if (s == nbw)
        return;
#else
    ...
    fatalf("logfileWrite (stdio): %s buf_len = %d, write_len = %d\n", lf->path, len, s);
}
...
logfile_mod_stdio_open(Logfile * lf, const char *path, size_t bufsz, int fatal_flag)
{
	...
	lf->data = ll;
#ifdef CC_FRAMEWORK
	ll->fd = file_open(path, O_WRONLY | O_CREAT | O_TEXT | O_APPEND);
#else
