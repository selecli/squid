...
typedef struct _lenovo_download_authStateData
{
	void * data;
	RH * handler;
} lenovo_download_authStateData;

static cc_module *mod = NULL; 
static helper * lenovo_download_auth = NULL; 
static MemPool * mod_config_pool = NULL; 
CBDATA_TYPE(lenovo_download_authStateData);

static void * mod_config_pool_alloc(void)
{
	void * obj = NULL; 
	if (NULL == mod_config_pool)
		mod_config_pool = memPoolCreate("mod_lenovo_download_auth config_struct", sizeof(mod_config));
...
{
	debug(222, 3)("mod_lenovo_download_auth: config is %s\n", cfg_line);
	mod_config * cfg = NULL;
	char *token = NULL;
	char cmd[256];
	wordlist * line  = NULL;
	...
	*p = 0;
	...
	if (NULL == (token = strtok(cfg_line, w_space)))
		goto err;
	else if (0 != strcmp("mod_lenovo_download_auth", token))
		goto err;
	...
	if (NULL != line)
		...
		goto err;
	debug(222, 1)("mod_lenovo_download_auth: param hlpchildren = %d hlpconcurrency = %d  hlpcmdline = %s\n", cfg->hlpchildren, cfg->hlpconcurrency, cfg->hlpcmdline->key);
	cc_register_mod_param(mod, cfg, free_cfg);
	return 0;
err:
	if (NULL != cfg)
		free_cfg(cfg);
	debug(222, 1)("mod_lenovo_download_auth: error occured when parsing cfg line\n");
	return -1;
}

static void func_lenovo_download_auth_shutdown()
{
	if (NULL != lenovo_download_auth)
	{
		helperShutdown(lenovo_download_auth);
		helperFree(lenovo_download_auth);
		lenovo_download_auth = NULL;
		debug(222, 3)("mod_lenovo_download_auth: clean up previous lenovo_download_auth when reconfiguring\n");
	}
}

static void lenovo_download_authInit(clientHttpRequest * http)
{
	mod_config *cfg = (mod_config*)cc_get_mod_param(http->conn->fd, mod);

	if (NULL == lenovo_download_auth)
	{
		lenovo_download_auth = helperCreate("lenovo_download_auth");
		lenovo_download_auth->cmdline = cfg->hlpcmdline;
		lenovo_download_auth->n_to_start = cfg->hlpchildren;
		lenovo_download_auth->concurrency = cfg->hlpconcurrency;
		lenovo_download_auth->ipc_type = IPC_STREAM;
		helperOpenServers(lenovo_download_auth);
		CBDATA_INIT_TYPE(lenovo_download_authStateData);
		debug(222, 3)("Init lenovo_download_auth with the config params as the first request coming\n");
	}
}

static void lenovo_download_authHandleReply(void *data, char *reply)
{
	lenovo_download_authStateData * state = data;
	int valid;
	debug(222, 5) ("lenovo_download_authHandleReply: {%s}\n", reply ? reply : "<NULL>");
	if (reply) 
	{       
		if (*reply == '\0')
			reply = NULL; 
	...
	if (valid) {
		...
		debug(222, 3)("mod_lenovo_dowload_auth: reply --> %s\n", reply);
	} else {
		debug(222, 3)("mod_lenovo_dowload_auth: valid is NULL\n");
	}
	cbdataFree(state);
	return;
}

static void lenovo_download_authDone(void * data, char *reply)
{
	debug(222, 3)("mod_lenovo_dowload_auth: lenovo_download_authDone reply --> %s\n", reply);
	...
	if (NULL == (p = strstr(reply, "200")))
	{
		...
		debug(222, 3)("mod_lenovo_dowload_auth: do not auth successfully, so forbid!\n");
	}
	else {
		p += 3;
		while (' ' == *p || '\t' == *p) {
		...
		debug(222, 3)("mod_lenovo_dowload_auth: p--> %s!\n", p);
...
static int func_urlSubmitForAuth(clientHttpRequest * http)
{
	...
	if (NULL == strstr(httpuri, "fileserver/fileman/DownLoadServlet"))
	{
		debug(222, 5)("do not be a vip request to AUTH\n");
		return 4;
	}
	char * t = strstr(httpuri, "t=");
	char * s = strstr(httpuri, "s=");
	if (NULL == t || NULL == s)
	{
		ErrorState *err = errorCon(ERR_ACCESS_DENIED, HTTP_UNAUTHORIZED, http->request);
		http->log_type = LOG_TCP_MISS;
	...
	lenovo_download_authInit(http);
	lenovo_download_authStateData * state;
	state = cbdataAlloc(lenovo_download_authStateData);
	state->data = http;
	state->handler = lenovo_download_authDone;
	cbdataLock(state->data);
	int	valid = cbdataValid(state->data);
	debug(222, 3)("valid after cbdatalock %d\n",valid);
	snprintf(buf, 24584, "%s\n", httpuri);
	helperSubmit(lenovo_download_auth, buf, lenovo_download_authHandleReply, state);
	valid = cbdataValid(state->data);
...
{
	debug(222, 1)("(mod_lenovo_download_auth) ->     hook_cleanup:\n");
	if (NULL != lenovo_download_auth)
	{
		helperShutdown(lenovo_download_auth);
		helperFree(lenovo_download_auth);
...
{
	debug(222, 1)("(mod_lenovo_download_auth) ->      mod_register:\n");
