...
typedef struct _request_param
{
	...
	char *uri;
	int  succ;
	media_type is_real_mp4;
	//int http_miss_strategy;
	//int miss_strategy;
	int  http_miss_wait;
} request_param;
...
static MemPool * fde_data_pool = NULL;
static MemPool * mp4WriteStateDataPool = NULL;
#define MOOV_PID_FILE	"/var/run/moov_generator.pid"
...
static void * request_param_pool_alloc(void)
{
	...
	{
		request_param_pool = memPoolCreate("mod_mp4 private_data request_param", sizeof(request_param));
	}
...
static void * mod_config_pool_alloc(void)
{
	...
	{
		mod_config_pool = memPoolCreate("mod_mp4 config_struct", sizeof(mod_config));
	}
    g_helper_refernce_count++;
	return obj = memPoolAlloc(mod_config_pool);
}

static void * fde_data_pool_alloc(void)
{
	void * obj = NULL;
	if (NULL == fde_data_pool)
	{
		fde_data_pool = memPoolCreate("mod_mp4 private_data fde_data", sizeof(fde_data));
	}
	return obj = memPoolAlloc(fde_data_pool);
}

static void * mp4WriteStateDataPoolAlloc(void)
{
	void * obj = NULL;
	if (NULL == mp4WriteStateDataPool)
	{
		mp4WriteStateDataPool = memPoolCreate("mod_mp4 other_data mp4WriteStateData", sizeof(mp4WriteStateData));
	}
	return obj = memPoolAlloc(mp4WriteStateDataPool);
}	
...
static int httpHeaderCheckExistByName(HttpHeader *h, char *name)
{
	...
	while ((curHE = httpHeaderGetEntry(h, &pos)))
	{
		...
		{
			debug(121,3)("Internal X-CC-Media-Prefetch request!\n");
			return 1;
		}
	}

	debug(121,3)("Not internal X-CC-Media-Prefetch request!\n");
	return 0;
}
...
static int free_cfg(void *data)
{
	...
	if(NULL != cfg)
	{
        ...
        if(g_helper_refernce_count <= 0 && media_prefetchers)
        {
            ...
            debug(121,1)("mod_mp4  shutdown helper %d\n", g_helper_refernce_count);
...
static int free_fde_data(void *data){
	...
	if(fd_data != NULL){
		...
		memPoolFree(fde_data_pool, fd_data);
		fd_data = NULL;
	}
	return 0;
}
...
static void store_sohu_key_url(clientHttpRequest *http){
	...
	char *rsturi = xstrdup(uri);

	char* str2 = strrchr(tmpuri, '/');
	if(NULL == str2)
	{
		safe_free(uri);
		safe_free(rsturi);
	...
	*str3 = '-';
	...
	http->request->store_url = xstrdup(rsturi+(str4-tmpuri));
	safe_free(uri);
	safe_free(rsturi);

	debug(121, 5)("request store_url is %s \n", http->request->store_url);
...
static int close_mp4_fd(int mp4_sock,int client_fd){
         ...
         if(fd_data == NULL){
               ...
               debug(121,5)("mod_mp4: when close mp4 fd fde_data param is null\n");
               return 1;
          }
...
{
	debug(121,3)("(mod_mp4) -> read request finished, remove url start: \n");


	//参数不完整，忽略该请求
	if( !http || !http->uri || !http->request ) {
		debug(121,3)("mod_mp4: http = NULL, return \n");
		return 0;
	}
	...
	if( (tmp = strchr(http->uri, '?')) == NULL ) {
		debug(121,3)("mod_mp4: <%s> not find ?, return \n", http->uri);
		return 0;
	}
	...
	if( (tmp = strstr(tmp, cfg->start)) == NULL ) {
		debug(121,5) ("mod_mp4: not find start return %s %s %s \n",http->uri,tmp,cfg->start);
		return 0;
	}

	tmp--;
	if( *tmp != '?' && *tmp != '&' ) {
		debug(121,5)("mod_mp4: <%s> has not start variable, return \n", http->uri);
		return 0;
	}
	tmp++;
	...
	char *store_url = xstrdup(http->uri);
	drop_startend(cfg->start, cfg->end, store_url);
	
	safe_free(http->request->store_url);
	http->request->store_url = store_url;
	return 0;
}
*/
...
static char *store_qq_key_url(char* url){
	...
	strncpy(hostname,p,tmp1-p);
        ...
        debug(121, 3)("mod_mp4 :hostname =%s \n",hostname);
	...
	char *result_url = xstrdup(tmp3);
        debug(121, 3)("mod_mp4: store_url=%s \n",result_url);
	safe_free(new_url);
        return result_url;
...
static void modify_request(clientHttpRequest * http,char *new_uri){
	...
	debug(121, 5)("mod_mp4: start, uri=[%s]\n", http->uri);
	request_t* old_request = http->request;
	
	request_t* new_request = urlParse(old_request->method, new_uri);

	debug(121, 5)("mod_mp4: start, new_uri=[%s]\n", new_uri);
	if (new_request)
	{
		safe_free(http->uri);
...
static int clientHandleMp4(clientHttpRequest *http)
{
	...
	if(cfg->website == STANDARD)
	{
		safe_free(http->request->store_url);
		http->request->store_url = store_key_url(http->uri);
	}
	else if(cfg->website == SOHU)
	{
		store_sohu_key_url(http);
	}

	//ch_mp4 = strstr(request->urlpath.buf, cfg->type);
        if(cfg->website == QQ){

		if((strstr(http->uri, "type=mp4")) == NULL ) {
			debug(121, 3) ("mod_mp4: not find type=mp4 return %s \n",http->uri);
			return 0;
		}

		char *tmp_header = strstr(http->uri, "head=1");
		if( tmp_header != NULL ) {
			debug(121, 3) ("mod_mp4: find key head=1 and return \n");
			return 0;
		}
	...
	//if (ch_mp4) {
		ch_mp4_start = strstr(request->urlpath.buf, cfg->start);
		...
		if(ch_mp4_start){
			...
			debug(121,3)("mod_mp4: request param start= %" PRINTF_OFF_T "\n",req_param->mp4_start);
			debug(121,3)("mod_mp4: request param end  = %" PRINTF_OFF_T "\n",req_param->mp4_end  );
			debug(121,3)("mod_mp4: request param flag=%d\n",req_param->mp4_flag);
			debug(121,3)("mod_mp4: request url=%s\n",http->uri);
			debug(121,3)("mod_mp4: request store url=%s\n",http->request->store_url);
		}
///	}
	return 0;
}
		...
		if(written_bytes < 0) {
			if(errno == EINTR) {
				...
				if (retry_times < 5) {
					...
					debug(121,2)("mod_mp4: fwrite error(%d) for safe_write\n", errno);
					return -1;
				}
			}
		}
...
static bool is_moov_alive(){
	...
	if(pidnum != 0){
		...
		debug(121, 3) ("cannot get moov_generator pidnum!\n");
	}
...
{
	if(!is_moov_alive()){
		debug(121,3)("moov_generator dont run,now starting it!\n");
		enter_suid();

		int cid = fork();
		if (cid == 0)   //child
		{
			int ret = execl("/usr/local/squid/bin/moov_generator", "moov_generator","-D",(char *)0);
			if (ret < 0)
			{
				debug(121,3)("(mp4) --> execl error : %s\n",xstrerror());
			}
...
static void mp4ReadMoovWrite(int fd, const char *buf, int size, CWCB * handler, void *handler_data, FREE * free_func, mp4WriteStateData *mp4_data)
{
	...
	debug(121, 3) ("mp4ReadMoovWrite: FD %d: sz %d: hndl %p: data %p.\n",
			fd, size, handler, handler_data);
	
	
	fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
		debug(121,5)("mod_mp4: when mp4 read moov write, fd param is null\n");
		free_func((void*)buf);
		return;
	}
	...
	if (state->valid){
		debug(121, 1) ("mod_mp4: fd_table[%d].rwstate.valid == true!\n", fd);
...
static void mp4ReadMoovStart(int mp4_sock, void *data)
{
	...
	int valid = cbdataValid(mp4Data->handler_data);
        ...
        //cbdataUnlock(mp4Data);
	
	if(!valid){
		
		debug(121,3)("mod_mp4: mp4 header callback valid is %d \n",valid);
		goto free_data;
	}
	...
	debug(121,3)("mod_mp4: request mp4 data from moov_generor is %s \n",Rbuf);
/*
	fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
		
		debug(121,3)("mod_mp4: when read moov header, fd param is null\n");
		goto free_data;
	}
*/
	if (Rsize > 0) {
		...
		if( req_param == NULL){
			debug(121,3)("mod_mp4: when read moov header, request param is null\n");
		...
		debug(121,3)("mod_mp4: mp4 rep size  = %d http out offset = %" PRINTF_OFF_T " \n",req_param->mp4_rep_sz,http->out.offset);
	
	        char new_buf[4096];
		memset(new_buf,0 ,4096);

		debug(121,3)("ori header buf: %s\n", buf);
		debug(121,3)("ori header size: %d\n", size);
		...
		char *chunk = NULL;
                int ck_len = 0;
                chunk = strstr(buf, "Transfer-Encoding");

                if(chunk)
                {
                        debug(121, 3)("mod_mp4: find Transfer-Encoding\n");
                        char *ck = chunk;
                        while((*ck) && (*ck != '\r'))
                        {
                                ck_len++;
		...
		if (con_len) {
			...
			size = size+strlen(tmp)-len;
                ...
                {
                        strncpy(new_buf, buf, chunk - buf);
                        strncpy(tmpbuf, chunk + ck_len, 4095);
                        snprintf(tmp, 128, "Content-Length: %" PRINTF_OFF_T, moov_size);
                        strncat(new_buf,tmp,strlen(tmp));
			strncat(new_buf,tmpbuf,strlen(tmpbuf));

			size = size+strlen(tmp)-ck_len;

			if(http->request->flags.chunked_response == 1)
			{
				http->request->flags.chunked_response = 0;
				debug(121,3)("mod_mp4: http->request->flags.chunked_response: %d\n", http->request->flags.chunked_response);
			}
		}
		else{
			debug(121,0)("mod_mp4: Can not find Content-Length or Transfer-Encoding FATAL ... ");
			assert(0);
		}
		...
		if (F->flags.open){
			size_t temp;
			char *mod_buf = memAllocBuf(size, &temp);
			strncpy(mod_buf,new_buf,size);
			FREE * free_func = memFreeBufFunc(temp);
	 		mp4ReadMoovWrite(fd, mod_buf, size, handler, handler_data, free_func, NULL); //FIXME  for leak out
			mp4Data->free_func(mp4Data->buf);
			return;
		}
		else 
		{
			debug(121,3)("mod_mp4: close mp4_sock for client close connection\n");
			goto free_data;
		}
	}else if(Rsize == 0) {
		debug(121,3)("mod_mp4: recv header Rsize = 0 and close client connection\n");
		close_client_fd(fd);
		goto free_data;
	
	}else {
		debug(121,2)("mod_mp4: recv header Rsize < 0 and close client connection %d\n",Rsize);
		close_client_fd(fd);
		goto free_data;
	}
	...
	free_data:
		...
		{
			debug(121,3)("mod_mp4: free mp4 buffer in recving header %s \n",mp4Data->buf);
			mp4Data->free_func(mp4Data->buf);
			mp4Data->free_func = NULL;
			memPoolFree(mp4WriteStateDataPool, mp4Data);
			mp4Data = NULL;
		}
} 
...
static int mp4_connect(){
	...
	*/
	...
	fd_open(mp4_fd, FD_SOCKET, "connect to moov_generator");
...
static int mp4_write_mbuf(int fd, MemBuf *mb, CWCB * handler, void *handler_data)
{
	...
	if(!entry || entry->swap_dirn == -1){
		// fix case 2927 for partial file: if client request a *.mp4?start=N,
		// when cache expire and server file has been changed.
		if (entry) {
		...
		debug(121,1)("mod_mp4: entry->swap_dirn is null and close client connection\n");
	...
	if(mp4_fd <= -1){
		debug(121,3)("mod_mp4: mp4 moov_generor is not connected, please check moov_generor \n");
		//clientWriteComplete(fd, NULL, 0, COMM_OK, handler_data);
		return 0;
	}
	...
	//register the fde private data of mp4 server fd
        fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
		fd_data = fde_data_pool_alloc();
		cc_register_mod_private_data(FDE_PRIVATE_DATA, &fd, fd_data, free_fde_data, mod);
	}

	fd_data->mp4_fd = mp4_fd;
	debug(121,3)("mod_mp4: connected mp4 moov's fd param is %d \n",fd_data->mp4_fd);
	//get http private data
	
	request_param* req_param = cc_get_mod_private_data(REQUEST_PRIVATE_DATA, http, mod);
	if( req_param == NULL){
		debug(121,5)("mod_mp4: when mp4_write mbuf ,request param is null\n");
	...
	debug(121, 3)("mod_mp4: mp4 offset in Fc Cache file: %d\n", local_mp4_req.mp4_offset);
	...
	switch (req_param->mp4_flag) {
		case 1:
			...
			local_mp4_req.type  = 1;
			break;
		case 2:
			local_mp4_req.start = req_param->mp4_start;
			local_mp4_req.end   = 0;
			local_mp4_req.type  = 2;
			break;
		case 3:
			local_mp4_req.start = req_param->mp4_start;
			local_mp4_req.end   = 0;
			local_mp4_req.type  = 3;
			break;
	}
	...
	if(cwrite != sizeof(mp4_req)){
		...
		debug(121,3)("mod_mp4: size which write to moov_generor is wrong,request all cache object content \n");
		
		close_mp4_fd(mp4_fd,fd);
		return 0; 
	}
	//注册读事件
	mp4WriteStateData *mp4Data = mp4WriteStateDataPoolAlloc();
	mp4Data->fd = fd;

	size_t temp;
	mp4Data->buf = memAllocBuf(mb->size, &temp);
	strncpy(mp4Data->buf,mb->buf,mb->size);
	mp4Data->size = mb->size;
	mp4Data->handler = handler;
	mp4Data->handler_data = handler_data;
	mp4Data->free_func = (FREE*) memFreeBufFunc(temp);
...
static void mp4ReadMoovBody(int mp4_sock, void *data)
{
	...
	if(!valid){
		debug(121,3)("mod_mp4: mp4 body callback valid is %d \n",valid);
		goto free_buf;
	}

	StoreEntry *entry = http->entry;
	size_t Rbuf_size;	
	Rbuf = (char *)memAllocBuf(4096, &Rbuf_size);
	free_func = memFreeBufFunc(Rbuf_size);
	Rsize = read(mp4_sock, Rbuf, 4095);

	request_param* req_param = cc_get_mod_private_data(REQUEST_PRIVATE_DATA, http, mod);
	if( req_param == NULL){
		debug(121,3)("mod_mp4: when read mp4 moov body ,request param is null\n");
		close_mp4_fd(mp4_sock,fd);
		goto free_buf;
	}
	...
	if( Rsize > 0) {
		...
		mp4ReadMoovWrite(fd, Rbuf, Rsize, handler, handler_data, free_func, NULL);
	}
	else if (Rsize == 0) {
		debug(121,5)("closed mp4_sock %d, sending clientfd %d, http->mp4_rep_sz %d\n", mp4_sock, fd, req_param->mp4_rep_sz);
		close_mp4_fd(mp4_sock,fd);
		...
		debug(121,5)("receive moov data exception and close the connection clientfd = %d \n", fd);
		close_mp4_fd(mp4_sock,fd);
	...
	free_buf:
		...
		debug(121,5)("mod_mp4: free body buf \n");
		if(Rbuf)
		{
			free_func(Rbuf);
			Rbuf = NULL;
		...
		if ((mp4Data != NULL)&& mp4Data->free_func)
		{
			debug(121,3)("mod_mp4: free mp4 buffer\n");
			mp4Data->free_func = NULL;
			memPoolFree(mp4WriteStateDataPool, mp4Data);
			mp4Data = NULL;
		}	
...
static void hookMp4ModuleReconfigure(int should_redirect_reload)
{
    debug(121, 1)("mod_mp4 %s DeleteMoovEvent\n", __func__);
	eventDelete(mp4Event, NULL);
}

static int hook_cleanup(cc_module *module)
{
	debug(121, 1)("(mod_mp4) ->	hook_cleanup:\n");
    hookMp4ModuleReconfigure(1);
	if(NULL != request_param_pool)
		memPoolDestroy(request_param_pool);
	if(NULL != fde_data_pool)
		memPoolDestroy(fde_data_pool);
	if(NULL != mp4WriteStateDataPool)
		memPoolDestroy(mp4WriteStateDataPool);
	if(NULL != mod_config_pool)
...
//fomat : mod_mp4 start end [1/0 proc_num miss_start/miss_wait/query_start/query_whole] allow/deny acl

static int func_sys_parse_param(char *cfg_line)
{
	debug(121,3)("mod_mp4 config=[%s]\n", cfg_line);
	if( strstr(cfg_line, "allow") == NULL )
		return 0;

	if( strstr(cfg_line, "mod_mp4") == NULL )
		return 0;
	...
	int miss_strategy = 0;
 	...
 	store_type website_type = STANDARD;
        ...
        else if( strstr(cfg_line, "sohu2") != NULL )
        {
                 website_type = SOHU2;
        }
	...
	if(!isConfKey(token)){
		...
		if(paravalue == 0 || paravalue == 1){
			...
			{
				debug(121,3)("mp4:change helper numbers --by w.yuan for debug\n");
				helper_proc_num = paravalue;
				is_load_helper = 1;
			}
		}
	...
	if(cfg->website == SOHU2)
	{
		token = strtok(NULL, w_space);
		if((token != NULL) && ((strcasecmp(token,"allow")!=0) && ( strcasecmp(token,"deny")!=0 ))){
			strncpy(cfg->url_prefix,token ,255);
		}
		else if ((strcasecmp(token,"allow")==0) || (strcasecmp(token,"deny") ==0 ))
		{
			debug(121,0)("mod_mp4: error conf\n");
		}

		//		token = strtok(NULL, w_space);
		//		if((token != NULL) && ((strcasecmp(token,"allow")!=0) && ( strcasecmp(token,"deny")!=0 ))){
		//			strncpy(cfg->dstdomain,token ,255);
		//	    }
		//		else if ((strcasecmp(token,"allow")==0) || ( strcasecmp(token,"deny") ===0 ))
		//		{
		//			debug(121,0)("mod_mp4: error conf\n");
	...
	cfg->miss_strategy = miss_strategy;
    	debug(121, 3) ("mod_mp4: paramter start= %s end= %s type= %s flag= %d miss_strategy=%d website= %d \n", cfg->start, cfg->end, cfg->type,cfg->flag, cfg->miss_strategy,cfg->website);
...
static int mod_close_mp4_server(int fd){
	...
	fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
		//debug(121,5)("mod_mp4: fd param is null\n");
		return 0;
	}

	fde *F = &fd_table[fd];
        if(F->cctype == 1 ){
                debug(121,2)("mod_mp4: this fd is server fd and not free the mp4Data %d \n",fd);
                return 0;
        }

	if(fd_data->mp4_moov_ok && fd_data->mp4_fd) {
		debug(121,3)("mod_mp4 : colse mp4_fd in comm.c for client closed fd: %d mp4_fd %d \n",fd,fd_data->mp4_fd);
		...
		if(NULL != fd_data->mp4_data)
		{
			memPoolFree(mp4WriteStateDataPool, fd_data->mp4_data);
			fd_data->mp4_data = NULL;
		}
...
static int mod_mp4_read_moov_body(int fd, CommWriteStateData *state)
{
	fde *F = &fd_table[fd];
	fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
...
static int mod_mp4_set_header_size(clientHttpRequest * http)
{
	request_param* req_param = cc_get_mod_private_data(REQUEST_PRIVATE_DATA, http, mod);
	if( req_param == NULL){
		//debug(121,5)("mod_mp4: when set header size ,request param is null\n");
		return 0;
	}

	if(req_param->is_real_mp4 != HIT_MP4 || req_param->mp4_flag == 0 ){
		return 0;
	}
...
static int mod_mp4_client_send_more_data(clientHttpRequest * http, MemBuf *mb,char *buf, ssize_t size){
...
static int mod_mp4_client_write_complete(int fd, char *bufnotused, size_t size, int errflag, void *data){
	
	fde_data* fd_data = cc_get_mod_private_data(FDE_PRIVATE_DATA, &fd, mod);
	if(fd_data == NULL){
		//debug(121,5)("mod_mp4: when write complete, fd param is null\n");
		return 0;
	}
...
static int change_server_uri(clientHttpRequest *http)
{
	//static char *uri_prefix = "http://220.181.61.229/rip?nid=1003&path=";
	char * uri = xstrdup(http->uri);
	char *tmpuri = strstr(uri,"?");
	if(tmpuri != NULL)
		*tmpuri = 0;
	...
	char new_uri[1024];
	memset(new_uri,0,1024);

	mod_config *cfg = cc_get_mod_param(http->conn->fd, mod);
	assert(cfg);


	snprintf(new_uri,1024,"%s%s",cfg->url_prefix,e);
	debug(121, 3)("request to web server url is %s \n", new_uri);
...
static int mod_mp4_http_client_cache_hit(clientHttpRequest *http, int stale)
{
	mod_config *cfg = cc_get_mod_param(http->conn->fd, mod);
	if(cfg == NULL)
		return stale;
	if((cfg->website == SOHU2)&&(strstr(http->uri,"newflv.sohu.ccgslb.net") != NULL )){
		if(stale != 0){
			change_server_uri(http);
			stale = 0;
		}
	}
	return stale;
...
static void mediaPrefetchHandleReplyForMissStart(void *data, char *reply)
{
	...
	if(strcmp(reply, "OK"))
	{
		debug(121,1)("mod_mp4 can not prefetch media! reply=[%s] \n",reply);
	}
	else{
		debug(121,1)("mod_mp4 quering whole object is success reply=[%s] \n",reply);
	}
...
static void mediaPrefetchHandleReply(void *data, char *reply)
{
	...
	if( req_param == NULL){
		//debug(121,5)("mod_mp4: request param is null\n");
		return ;
	}
...
/*	
	if(strcmp(reply, "OK"))
	{
		debug(121,1)("mod_mp4 can not prefetch media!\n");
...
*/
	...
	debug(121,3)("mod_mp4 prefetched media, and it should be a hit!\n");
...
static int mod_mp4_http_req_process(clientHttpRequest *http){
	...
	if(req_param->is_real_mp4 == HIT_OTHER){

		return 0;
	}
	
	if(req_param->is_real_mp4 != HIT_MP4){
		return 0;
	}
	...
	if (http->log_type == LOG_TCP_MISS && req_param->succ == 1){
		debug(121,3)("mod_mp4: curl reqeust is fail and fetch all whole mp4 file \n");
		req_param->mp4_flag = 0;
		
	}
	else if (http->log_type == LOG_TCP_MISS && req_param->mp4_flag != 0 && cfg->miss_strategy == 0){
		debug(121,3)("mod_mp4: request mp4 url is miss and query whole object to web server \n");
		req_param->mp4_flag = 0;
	...
	else if( http->log_type == LOG_TCP_MISS && !has_header && cfg->miss_strategy == 1 )
	{
		debug(121,3)("sending data to helper miss_wait because http->log_type=%d, cfg->miss_strategy=%d\n ", http->log_type, cfg->miss_strategy);
		debug(121,3)("client address : %s,url : %s\n", inet_ntoa(http->request->client_addr),http->uri);
	...
	else if( http->log_type == LOG_TCP_MISS && cfg->miss_strategy == 2 &&!has_header)
    {
        debug(121,3)("mod_mp4: request miss_start object because http->log_type=%d, cfg->miss_strategy=%d\n ", http->log_type, cfg->miss_strategy);
        StoreEntry *e = NULL;
		if(http->request->store_url == NULL)
		{
			req_param->is_real_mp4 = HIT_OTHER;
			debug(121,1)("mod_mp4: request [%s] has no store_url, return now !!!\n", http->uri);
			return 0;
		}
		...
		if(e == NULL){//query whole object
			...
			drop_startend(cfg->start, cfg->end, new_uri);
                        ...
                        debug(121, 2) ("mod_mp4: mp4 miss and redirect to upper layer to query whole data %s \n",new_uri);
			static char buf[4096];
			snprintf(buf, 4096, "%s\n", new_uri);
			safe_free(new_uri);
			helperSubmit(media_prefetchers, buf, mediaPrefetchHandleReplyForMissStart, state);
                ...
                if(e == NULL|| e->store_status != STORE_OK || e->swap_status != SWAPOUT_DONE ){
                        ...
                        debug(121, 2) ("mod_mp4: mp4 pending and redirect to upper layer to query start data %s \n",http->uri);
			...
			safe_free(http->request->store_url);
                        ...
                        debug(121, 3) ("mp4 hit request http->uri = %s\n", http->uri);
                }

               return 0;
        }
	else if( http->log_type == LOG_TCP_HIT && cfg->miss_strategy == 2&& !has_header)
    {
        debug(121,3)("mod_mp4: request miss_start hit object because http->log_type=%d, cfg->miss_strategy=%d\n", http->log_type, cfg->miss_strategy);
        StoreEntry *e = NULL;
		if(http->request->store_url == NULL)
        {
			req_param->is_real_mp4 = HIT_OTHER;
            debug(121,1)("Warnning !!! mod_mp4: request [%s] has no store_url, return now !!!\n", http->uri);
            return 0;
        }
        e = storeGetPublic(http->request->store_url,METHOD_GET);

        if(e == NULL){
            debug(121,2) ("mod_mp4: flv hit but has not store entry  %s \n",http->uri);
            return 0;
        }

        if(e->swap_status != SWAPOUT_DONE){

            debug(121, 2) ("mod_mp4: f4v pending and redirect to upper layer to query start data %s\n",http->uri);
            http->request->flags.cachable = 0;//no cache
            req_param->mp4_flag = 0;
            safe_free(http->request->store_url);
	...
	else if( http->log_type == LOG_TCP_MISS && cfg->miss_strategy == 3&& !has_header)
        {
                debug(121,3)("mod_mp4: query start for http->log_type=%d, cfg->miss_wait=%d\n ", http->log_type,cfg->miss_strategy);
                req_param->mp4_flag = 0;
                http->request->flags.cachable = 0;//no cache
                safe_free(http->request->store_url);
		req_param->is_real_mp4 = HIT_OTHER;
	...
	{
		debug(121,3)("can not enter helper because http->log_type=%d, req_param->mp4_flag=%d,cfg->miss_strategy=%d\n ", http->log_type, req_param->mp4_flag, cfg->miss_strategy);
	}
...
static int checkHasMp4(clientHttpRequest *http)
{
	...
	if(cfg->website != QQ){
	 ...
	 //get store_entry and path
        ...
        debug(121,3)("mod_mp4: check has mp4:, modify uri=[%s]\n" ,uri);
        ...
        if(e == NULL){
                ...
                *tmp_uri = 'l';
                tmp_uri++;
                *tmp_uri = 'v';
                debug(121, 3) ("mod_mp4: modify request http->uri = %s\n", new_uri);
...
static void hook_before_sys_init()
{
    ...
    int        reload_idx;
    debug(121, 3)("mod_mp4 hook_before_sys_init eventAdd mp4Event\n");
    eventAdd("mp4Event", mp4Event, NULL, 30, 0); 
	if(is_start_helper == 0)
	{
		debug(121,3)("mod_mp4: module cfg is_start_helper=%d and don't start helper program\n",is_start_helper);
	...
	debug(121,3)("mod_mp4: module cfg is_start_helper=%d and start helper program\n",is_start_helper);
	debug(121,3)("mod_mp4: helper process number  =%d \n",helper_proc_num);

	stat("/usr/local/squid/bin/media_prefetch.pl", &statbuf_of_this_load);

    reload_idx = reload_count;
    reload_count ++;
	...
	if(is_load_helper)
	{
	    ...
	    if(media_prefetchers) /*reload*/
	    {
	        ...
	        helperReloadServers(media_prefetchers);
	    }
	    else/*open*/
	    {	
    		debug(121,3)("mp4:start helper --by w.yuan for debug\n");
    		media_prefetchers = helperCreate("media_prefetcher");
    	
    		media_prefetchers->cmdline     = pwl;
    		media_prefetchers->n_to_start  = helper_proc_num;
    		media_prefetchers->n_running   = 0;
    		media_prefetchers->concurrency = 0;
    		media_prefetchers->ipc_type    = IPC_STREAM;
    		...
    		if(0 == reload_idx)
    		{   
    		    /*puke! when conf changing path is: no prfecher -> some prefetchers*/
    		    /*helperOpenServers will be triggered due to reload_count change from 0 to 1*/
    		    /*although the expection is to trigger helperServerOpenAllStart because */
    		    /*squid is already in server when that happens*/
    		    helperOpenServers(media_prefetchers);	    
    		}
    		else
    		{
    		    helperServerOpenAllStart(media_prefetchers);
    		}   
#endif    	
#if 1
            helperServerOpenAllStart(media_prefetchers);
...
{
	debug(121, 1)("(mod_mp4) ->	mod_register:\n");
	...
	strcpy(module->version, "7.0.R.16488.i686");
        ...
        //module->hook_func_private_mp4_read_moov_body = mod_mp4_read_moov_body;
        cc_register_hook_handler(HPIDX_hook_func_private_mp4_read_moov_body,
                        ...
                        mod_mp4_read_moov_body);

        //module->hook_func_http_repl_send_start = mod_mp4_set_header_size;
        ...
        cc_register_hook_handler(HPIDX_hook_func_private_mp4_send_more_data,
                        ...
                        mod_mp4_client_send_more_data);

        //module->hook_func_private_client_write_complete       = mod_mp4_client_write_complete;
