                 ...
                 atom_t* atom)
{
  ...
  if(atom->size_ < ATOM_PREAMBLE_SIZE)
  {
    ...
    errlog("Error: invalid atom size\n");
    return 0;
  }
                       ...
                       unsigned char* buffer, uint64_t size)
{
  ...
  if(size < 12)
  {
    ...
    errlog("Error: not enough bytes for stsz atom\n");
    return 0;
  }
...
//    atom->entries_ = 0;
  ...
  if(!atom->sample_size_)
  {
    if(size < 12 + atom->entries_ * sizeof(uint32_t))
    {
      ...
      errlog("Error: stsz.entries don't match with size\n");
      stsz_exit(atom);
      return 0;
    }
                     ...
                     unsigned char* buffer, uint64_t size)
{
  ...
  if(tag == MP4_ELEMENTARY_STREAM_DESCRIPTOR_TAG)
  {
    ...
    debug("Elementary Stream Descriptor: len=%u\n", len);
...
#endif
  debug("Decoder Configuration Descriptor:\n");
  debug("object_type_id=$%02x\n", object_type_id);
  debug("stream_type=%u\n", stream_type);
  debug("buffer_size_db=%u\n", buffer_size_db);
  debug("max_bitrate=%u\n", sample_entry->max_bitrate_);
  debug("avg_bitrate=%u\n", sample_entry->avg_bitrate_);
  ...
  if(tag == MP4_DECODER_SPECIFIC_DESCRIPTOR_TAG)
  {
    ...
    debug("Decoder Specific Info Descriptor: len=%u\n", len);
                ...
                sample_entry_t* sample_entry)
{
  ...
  if(size < 78)
  {
    ...
    errlog("invalid stsd video size\n");
    return 0;
  }
                ...
                sample_entry_t* sample_entry)
{
  ...
  debug("Sample Entry:\n");
  debug("data_reference_index=%u\n", data_reference_index);
  debug("version=%u\n", version);
  debug("revision=%u\n", revision);
  debug("vendor=%08x\n", vendor);
  debug("channel_count=%u\n", sample_entry->nChannels);
  debug("sample_size=%u\n", sample_entry->wBitsPerSample);
  debug("compression_id=%u\n", compression_id);
  debug("packet_size=%u\n", packet_size);
  debug("samplerate_hi=%u\n", sample_entry->samplerate_hi_);
  debug("samplerate_lo=%u\n", sample_entry->samplerate_lo_);
...
#endif
    debug("  samples_per_packet=%u\n", samples_per_packet);
    debug("  bytes_per_packet=%u\n", bytes_per_packet);
    debug("  bytes_per_frame=%u\n", bytes_per_frame);
    debug("  bytes_per_sample=%u\n", bytes_per_sample);
                       ...
                       unsigned char* buffer, uint64_t size)
{
  ...
  if(!atom->stsd_)
  {
    ...
    errlog("stbl: missing mandatory stsd\n");
    result = 0;
  }
  ...
  if(!atom->stts_)
  {
    ...
    errlog("stbl: missing mandatory stts\n");
    result = 0;
  }
  ...
  if(!atom->stsc_)
  {
    ...
    errlog("stbl: missing mandatory stsc\n");
//    result = 0;
  }
  ...
  if(!atom->stsz_)
  {
    ...
    errlog("stbl: missing mandatory stsz\n");
//    result = 0;
  }
  ...
  if(!atom->stco_)
  {
    ...
    errlog("stbl: missing mandatory stco\n");
//    result = 0;
  }
                       ...
                       unsigned char* buffer, uint64_t size)
{
  ...
  if(result && !stsd_parse(mp4_context, atom, atom->mdia_->minf_->stbl_->stsd_))
  {
    ...
    errlog("trak: error parsing stsd\n");
    result = 0;
  }
                         ...
                         void* parent, void* child)
{
     ...
     trak->mdia_->hdlr_->handler_type_ != FOURCC('s', 'o', 'u', 'n'))
  {
    ...
    debug("Trak ignored (handler_type=%c%c%c%c, name=%s)\n",
                                  ...
                                  traf_t const* traf)
{
  ...
  if(track == moov->tracks_)
  {
    ...
    errlog("add_fragmented_samples: trak not found\n");
    return 0;
  }
...
//  }
  ...
  if(trex == NULL)
  {
    ...
    errlog("tfhd: trex not found (track_id=%u)\n", atom->track_id_);
    return 0;
  }
      ...
      for(j = 0; j != entries; j++)
      {
        ...
        for(i = 0; i < sample_count; i++)
        {
          if(s == trak->samples_size_)
          {
            ...
            warn("Warning: ctts_get_samples=%u, should be %u\n",
                   ctts_get_samples(ctts), trak->samples_size_);
            break;
          }
    ...
    for(j = 0; j != trak->chunks_size_; j++)
    {
      ...
      for(i = 0; i != trak->chunks_[j].size_; i++)
      {
        if(s == trak->samples_size_)
        {
          ...
          warn("Warning: stco_get_samples=%u, should be %u\n",
                 stco_samples, trak->samples_size_);
          break;
        }
