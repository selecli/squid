...
// find pid_filename config line
pid_t readRunningPidFile(void)
{
    
    FILE *fp = NULL;
    if (NULL == (fp = fopen(ConfigFile, "r")))
    {
        fatalf("Unable to open configuration file: %s: %s", ConfigFile, xstrerror());
    }
    
    memset(config_input_line, '\0', BUFSIZ);
    while (fgets(config_input_line, BUFSIZ, fp))
    {
        if (strstr(config_input_line, "pid_filename"))
        {
            strtok(config_input_line, " \t\r\n");
            parse_string_with_id(&Config.pidFilename);
            break;
        }
    }
...
#ifdef CC_FRAMEWORK
int percent_64(squid_off_t a, squid_off_t b)
{
	return b ? ((int) (100.0 * a / b + 0.5)) : 0;
}
#endif
...
xusleep(unsigned int usec)
{
	...
	{
    	debug(1, 1)("[DEBUG] %s: xusleep %u usec\n", __func__, usec);
	}
