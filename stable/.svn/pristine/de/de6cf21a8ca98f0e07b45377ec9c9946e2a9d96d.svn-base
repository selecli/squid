...
#		a['CanonicalizedAmzHeaders'] = 'x-amz-meta-tag:test\nx-sina-info:\xe4\xb8\xad\xe6\x96\x87\n' 
	...
	def get_sub_host(self,host,father_domain):
		...
		host = host.replace('chinacache.net','com')
		return host
	...
	def get_uri_and_subresource(self,url):
		...
		if uri.find('?')!= -1:
			...
			ignore_part = ['Expires=','ssig=','KID=','start=','ts=','end=','te=']
			for item in res_list:
				if item.find(ignore_part[0])==-1 and item.find(ignore_part[1])==-1 and item.find(ignore_part[2])== -1 and item.find(ignore_part[3])==-1 and item.find(ignore_part[4])==-1 and item.find(ignore_part[5])==-1 and item.find(ignore_part[6])==-1:
					if count_res == 0 :
						res_calu = '?'+item
...
#		password = 'uV3F3YluFJax1cknvbcGwgjvx4QpvB+leU8dUj2o'
		password = "VxH+NHPzvtr5RcKUq34U7XDtr4tA9bLHiszhyyoQ"
