...
//                   "http process request line");
    ...
    if (rev->timedout) {
//        ngx_log_error(NGX_LOG_ERR, c->log, NGX_ETIMEDOUT, "LSCS_error:client timed out");
        c->timedout = 1;
        ngx_http_close_request(r, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }
    ...
    for ( ;; ) {
        ...
        n = c->recv(c, r->header_in->start, r->header_in->end - r->header_in->start);
        if (n == NGX_AGAIN ) {
            return;
        } else if( n == 0 ) {//close by client
/*            ngx_log_error(NGX_LOG_INFO, 
                          c->log, 0, 
                          "LSCS_error::closed by client");
*/

            c->error = 1;
            ngx_http_close_request(r, NGX_HTTP_BAD_REQUEST);
            return;
        } else if(n == NGX_ERROR){ //error
/*            ngx_log_error(NGX_LOG_INFO, 
                          c->log, ngx_socket_errno, 
                          "LSCS_error:Socket error in peek client connection");
*/
        ...
        r->header_in->last = r->header_in->start + n;
        ...
        if (rc == NGX_OK) {
            ...
            if (r->args_start) { /**if get ?arg1=value**/
...
*/
                    ...
                    != NGX_OK)
            {
                ...
                ngx_log_error(NGX_LOG_ERR, 
                          c->log, 0, 
                          "LSCS_error:ngx_list_init(r->headers_in.headers) error");
                ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
            ...
            {
                ngx_log_error(NGX_LOG_ERR, c->log, 0, 
                          "LSCS_error:ngx_list_init(r->headers_in.cookies) error");
                ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
        ...
        if (r->header_in->pos == r->header_in->end) { /**Need alloc big buffer**/
            if (hc->nbusy == 0) {
                hc->nbusy++;
            } else if (hc->nbusy > cscf->large_client_header_buffers.num + 1 ) {
                ngx_log_error(NGX_LOG_ERR, c->log, 0, 
                              "LSCS_error: Bad request with such big Header over 32KB");
                ngx_http_close_request(r, NGX_HTTP_BAD_REQUEST);
                return;
            }
            ...
            r->header_in->start = ngx_pcalloc(r->pool, hc->nbusy * 8192); 
            r->header_in->last = r->header_in->start;
            r->header_in->pos = r->header_in->start;
            r->header_in->end = r->header_in->start + hc->nbusy * 8192;
            hc->nbusy++;
            continue;
        } else {
            r->state = 0;
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
        ngx_log_error(NGX_LOG_INFO, c->log, NGX_ETIMEDOUT, "LSCS_error:client timed out");
        c->timedout = 1;
        ngx_http_close_request(r, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            if (h == NULL) {
                ngx_log_error(NGX_LOG_ERR, c->log, 0,
                        "LSCS_error:nginx has big error,ngx_list_push(r->headers_in.headers)");
                ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
            ...
            if (h->lowcase_key == NULL) {
                ngx_log_error(NGX_LOG_ERR, c->log, 0,
                        "LSCS_error:nginx has big error,ngx_pnalloc(r->pool) error");
                ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
            ...
            if (hh && hh->handler(r, h, hh->offset) != NGX_OK) {
                ngx_log_error(NGX_LOG_ERR, c->log, 0,
                        "LSCS_error:hh->handler failed, Bad Request with invalid header");
                return;
            }
...
//                           &h->key, &h->value);
            ...
            if( r->headers_in.host != NULL && (r->headers_in.host)->value.data != NULL ) { 
...
//                        "http header done");
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
            ...
            if (r->header_in->pos == r->header_in->end) { /**Need alloc big buffer**/
                ...
                u_char   *old, *new;
                ngx_int_t len = r->header_in->pos - r->header_in->start;
                ngx_int_t header_name_start_offset = r->header_name_start - r->header_in->start;

                r->header_in->start = ngx_pcalloc(r->pool, hc->nbusy * 8192); 
                r->header_in->pos = r->header_in->start + len;
                r->header_in->last = r->header_in->pos;
                r->header_in->end = r->header_in->start + hc->nbusy * 8192;
                hc->nbusy++;
              
                old = r->header_name_start;
                new = r->header_in->start + header_name_start_offset;
             ...
             * If header error, just forward to backend squid*/
            goto lscs_forward; 
        }
    }
lscs_forward:
...
//                           "http header done");
	...
	{
        if(ngx_http_find_virtual_server(r, r->headers_in.host->value.data, r->headers_in.host->value.len)
                                    == NGX_ERROR)
        {
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    ngx_http_close_request(r, clcf->handler(r));
   
    /*Fix a big bug, for fd used*/ 
    c->read->active = 0;
    /*
    ngx_del_event(c->read, NGX_READ_EVENT, NGX_CLOSE_EVENT);

    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, 
                  "LSCS_error(%s): finished request(%d) process\n", 
                  __FUNCTION__, r->connection->fd);
    */
    return;
}
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (n == NGX_AGAIN ||  (n == (r->header_in->last - r->header_in->start) )) {
        if (!rev->timer_set) {
            cscf = ngx_http_get_module_srv_conf(r, ngx_http_core_module);
...
ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, 
                  "LSCS_error(%s): request(%d), request_count:%d\n", 
                  __FUNCTION__, r->connection->fd, r->count);
