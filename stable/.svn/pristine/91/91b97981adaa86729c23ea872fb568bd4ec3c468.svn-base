...
// pass = 0 && i = 0: video track
// pass = 1 && i = 1: audio track
// get_keyframe_flag: 0,get the frame prev keyframe;
//		      1,get the frame next keyframe.
static int get_aligned_start_and_end(struct mp4_context_t const* mp4_context,
				     ...
				     unsigned int get_keyframe_flag)
{
  unsigned int pass;
  ...
  {
    unsigned int i;
    unsigned int next_keyframe = 0;//QQVIDEO
    for(i = 0; i != moov->tracks_; ++i)
    {
      ...
      else
      {
		if(stbl->stss_)
		{
			debug("pass=%d,i=%d ( video track )\n\n",pass, i);
		}
		else
		{
			debug("pass=%d,i=%d ( audio track )\n\n",pass, i);
		}	

        //MP4_INFO("start0=%u (trac time)\n", start);
        start = stts_get_sample(stbl->stts_,
          moov_time_to_trak_time(start, moov_time_scale, trak_time_scale));

        debug("start=%u (trac time)\n", start);
        debug("start=%.2f (seconds)\n",
          stts_get_time(stbl->stts_, start) / (float)trak_time_scale);

#if 1//QQVIDEO
	if (get_keyframe_flag == 1)
		start = stbl_get_nearest_keyframe_next(stbl, start + 1) - 1;
	else
		start = stbl_get_nearest_keyframe_prev(stbl, start + 1, &next_keyframe) - 1;
#else
        start = stbl_get_nearest_keyframe(stbl, start + 1) - 1;
#endif
        //MP4_INFO("start=%u (zero based keyframe)\n", start);
        debug("start=%u (zero based keyframe)\n", start);
        trak_sample_start[i] = start;
      ...
      else
      {
        ...
        debug("end=%u (trac time)\n", end);
        //MP4_INFO("end=%.2f (seconds)\n",
        debug("end=%.2f (seconds)\n",
...
#if 1//QQVIDEO
	if (get_keyframe_flag == 1)
		end = stbl_get_nearest_keyframe_next(stbl, end + 1) - 1;
	else
		end = stbl_get_nearest_keyframe_prev(stbl, end + 1, &next_keyframe) - 1;
#else
          end = stbl_get_nearest_keyframe(stbl, end + 1) - 1;
#endif
        }
        //MP4_INFO("end=%u (zero based keyframe)\n", end);
        debug("end=%u (zero based keyframe)\n", end);
        trak_sample_end[i] = end;
        //MP4_INFO("endframe=%u, samples_size_=%u\n", end, trak->samples_size_);
        debug("endframe=%u, samples_size_=%u\n", end, trak->samples_size_);
...
mp4_split_options_t* mp4_split_options_init()
{
  ...
  options->file_offsets = 0;
  options->get_keyframe_flag = 0;
  return options;
}
...
{
  int result;
  unsigned int get_keyframe_flag = options->get_keyframe_flag;
...

  {
...
#if 1
	/*QQVIDEO_ON_DEMAND*/
	if(start > moov_duration)
		start = 0;
	
	debug("mvhd->duration:%lld\n", moov_duration);
	debug("mvhd->timescale:%ld\n", moov_time_scale);
	debug("mvhd->duration:%.2f(seconds)\n", moov_duration/(float)moov_time_scale);
	debug("mp4_split start :%d\n",start);
	debug("mp4_split   end :%d\n",end);
#endif
    debug("drag start value: %d\n" ,start);
    ...
    result = get_aligned_start_and_end(mp4_context, start, end,
                                       trak_sample_start, trak_sample_end,get_keyframe_flag);
  }
