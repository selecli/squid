...
#ifdef CC_FRAMEWORK
StoreEntry * clientCreateStoreEntry(clientHttpRequest * h, method_t m, request_flags flags);
MemObject *new_MemObject(const char *);
int httpCachableReply(HttpStateData * httpState, NOCACHE_REASON *ncr);
#endif

void aclCheckCleanup(aclCheck_t * checklist);
...
extern void configFreeMemory_r(void);
...
#ifdef CC_FRAMEWORK
extern int httpBuildRequestPrefix(HttpStateData* http,
    request_t * request,
...
extern HttpHeaderHashField** httpHeaderBuildHashFields(const HttpHeaderFieldAttrs * attrs, int count);
extern void httpHeaderDestroyFieldsInfo(HttpHeaderFieldInfo * info, int count);
extern int httpHeaderIdByName(const char *name, int name_len, const HttpHeaderFieldInfo * attrs, int end);
extern int httpHeaderIdByNameNew(const char *name, int name_len, const HttpHeaderFieldInfo * attrs, HttpHeaderHashField ** hash, int end);
...
extern HttpHeaderEntry *httpHeaderGetEntryPlus(const HttpHeader * hdr, HttpHeaderPos * pos);
...
#ifdef CC_FRAMEWORK
extern void storeDisDirWriteCleanLogs(void *);
#endif
...
extern int percent_64(squid_off_t, squid_off_t);
...
#endif/*CC_FRAMEWORK*/
#ifdef CC_FRAMEWORK
extern pid_t readRunningPidFile(void);
extern helper_server *GetFirstAvailable(helper * hlp);
#endif
...
extern void clientSendMoreData(void *data, char *buf, ssize_t size);
...
/*added by jaingbo.tian for acl_optimize */
extern void aclLookupProxyAuthStart(aclCheck_t * checklist);
extern void aclLookupDstIPDone(const ipcache_addrs * ia, void *data);
extern void aclLookupDstIPforASNDone(const ipcache_addrs * ia, void *data);
extern void aclLookupSrcFQDNDone(const char *fqdn, void *data);

extern void aclLookupDstFQDNDone(const char *fqdn, void *data);

extern void aclLookupProxyAuthDone(void *data, char *result);
extern void aclLookupExternalDone(void *data, void *result);
//added by wen.si for zcopy and tcp optimize
#ifdef CC_FRAMEWORK
extern void comm_z_write(int fd, int size, CWCB * handler, void *handler_data, FREE * free_func);
extern void comm_z_write_mbuf(int fd, MemBuf mb, CWCB * handler, void *handler_data);
extern void storeZCopy(storeIOState *, int, size_t, squid_off_t, STRCB *, void *);
extern void printIsZCopy(char * file, int line);
extern void commEnhanceSndBuf(int fd);
extern void commSetCork(int fd);
extern void commSetNoCork(int fd);
extern aclCheck_t *cc_clientAclCheckListCreate(char* token, const clientHttpRequest * http);
extern int httpHdrRangeOffsetLimit2(request_t* request, HttpHdrRange *range);
extern void storeHashDelete(StoreEntry * e);
extern void clientWriteComplete(int fd, char *bufnotused, size_t size, int errflag, void *data);
//used to cover RequestMethods with RequestMethods_r
extern void urlExtMethodAdd(const char *mstr);
extern void mainReconfigure(void);
extern char *storeAufsDirFullPath(SwapDir * SD, sfileno filn, char *fullpath);
extern ipcache_entry* ipcache_get(const char *name);
extern int ipcacheExpiredEntry(ipcache_entry * i);
extern void ipcacheRelease(ipcache_entry *);
extern void fwdConnectDone(int server_fd, int status, void *data);
...
#endif
#ifdef CC_FRAMEWORK
//added by chaosheng.huang for netease phone software download
