...
#ifdef CC_FRAMEWORK
	{"Webluker-Edge", HDR_X_WEBLUKER, ftStr},
	{"Webluker-Edge-Error", HDR_X_WEBLUKER_ERROR, ftStr},
	{"Powered-By-ChinaCache", HDR_X_CACHE, ftStr},
	{"FlexiCache-Error", HDR_X_SQUID_ERROR, ftStr},
	{"FlexiCache-Lookup", HDR_X_CACHE_LOOKUP, ftStr},
	{"X-Forwarded-For", HDR_X_FORWARDED_FOR, ftStr},
	{"X-Request-URI", HDR_X_REQUEST_URI, ftStr},
	{"X-ChinaCache-Geo", HDR_CLIENT_AREA, ftStr},
	//{"Webluker-Edge-Error", HDR_X_SQUID_ERROR, ftStr},
#else
...
#ifdef	CC_FRAMEWORK	
	{"X-Local-Object", HDR_LOCAL_OBJECT, ftStr},
	{"Server-Encoding", HDR_SERVER_ENCODING, ftStr},
#endif
	{"Other:", HDR_OTHER, ftStr}	/* ':' will not allow matches */
};
static HttpHeaderFieldInfo *Headers = NULL;
static HttpHeaderHashField **HeadersHash = NULL;
/*
...
httpHeaderInitModule(void)
{
	...
	if (!HeadersHash)
		HeadersHash = httpHeaderBuildHashFields(HeadersAttrs, HDR_ENUM_END);
	/* create masks */
...
httpHeaderInit(HttpHeader * hdr, http_hdr_owner_type owner)
{
	...
	//zhb:preAlloc space for all headers
	memset(hdr->pSearch, -1, 128 * 4);
	hdr->nEndSearch = HDR_ENUM_END;
}
...
httpHeaderRepack(HttpHeader * hdr)
{
	...
	while (dp < hdr->entries.count)
	{
		...
		if(HDR_OTHER == ((HttpHeaderEntry *)hdr->entries.items[pos])->id)
		{
			hdr->pSearch[hdr->nEndSearch] = pos;
			hdr->nEndSearch++;
		}
		else
		{
			if(-1 == hdr->pSearch[((HttpHeaderEntry *)hdr->entries.items[pos])->id])
				hdr->pSearch[((HttpHeaderEntry *)hdr->entries.items[pos])->id] = pos;
			else
			{
				hdr->pSearch[hdr->nEndSearch] = pos;
				hdr->nEndSearch++;
...
/* returns next valid entry */
HttpHeaderEntry *
httpHeaderGetEntryPlus(const HttpHeader * hdr, HttpHeaderPos * pos)
{
	static g_c1 = 0;
	static g_c2 = 0;
	g_c1 ++;
	assert(hdr && pos);
	assert(*pos >= HttpHeaderInitPos && *pos < hdr->nEndSearch);
	for ((*pos)++; *pos < hdr->nEndSearch; (*pos)++)
	{
		g_c2 ++;
		if (-1 != hdr->pSearch[*pos])
			return hdr->entries.items[hdr->pSearch[*pos]];
	}
	return NULL;
}
...
httpHeaderFindLastEntry(const HttpHeader * hdr, http_hdr_type id)
{
	...
	//if(id != HDR_OTHER)
	//{
	//	result = hdr->entries.items[hdr->pSearch[id]];
	//	if(!result || !result->nRepeat)
	//		return result;
	//}
	/* looks like we must have it, do linear search */
	while ((e = httpHeaderGetEntry(hdr, &pos)))
...
{
	//zhb:simplyfy this tackle
	int nID = httpHeaderIdByNameDef(name, strlen(name));
	if(-1 != nID)
		return httpHeaderDelById(hdr, nID);
	int count = 0;
	HttpHeaderPos pos = HttpHeaderInitPos + HDR_ENUM_END;
	HttpHeaderEntry *e;
	//zhb:just clean HDR_OTHER
	//httpHeaderMaskInit(&hdr->mask, 0);	/* temporal inconsistency */
	CBIT_CLR(hdr->mask, HDR_OTHER);
...
httpHeaderDelById(HttpHeader * hdr, http_hdr_type id)
{
	...
	if(-1 == hdr->pSearch[id])
		return 0;
	if(((HttpHeaderEntry*)(hdr->entries.items[hdr->pSearch[id]]))->nRepeat)
	{
		while ((e = httpHeaderGetEntryPlus(hdr, &pos)))
		{
			if (e->id == id)
	...
	httpHeaderDelAt(hdr, hdr->pSearch[id]);
	hdr->pSearch[id] = -1;
	count++;
...
httpHeaderAddEntry(HttpHeader * hdr, HttpHeaderEntry * e)
{
	...
	//the most number of header is 128
	if(hdr->nEndSearch >= 127)
	{
		debug(55, 7) ("%p not adding entry: %d at %d,because hdr number is on the 127\n",
	...
	//zhb:Set header to its own position
	if(e->id == HDR_OTHER)
	{
		hdr->pSearch[hdr->nEndSearch] = hdr->entries.count - 1;
		hdr->nEndSearch ++;
	}
	else
	{
		if(-1 == hdr->pSearch[e->id])
			hdr->pSearch[e->id] = hdr->entries.count - 1;
		else
		{
			((HttpHeaderEntry*)(hdr->entries.items[hdr->pSearch[e->id]]))->nRepeat = 1;
			hdr->pSearch[hdr->nEndSearch] = hdr->entries.count - 1;
			hdr->nEndSearch ++;
...
httpHeaderGetList(const HttpHeader * hdr, http_hdr_type id)
{
	...
	//zhb:quick the get list action
	if((e = (HttpHeaderEntry*)hdr->entries.items[hdr->pSearch[id]]))
	{
...
httpHeaderInsertTime(HttpHeader * hdr, int pos, http_hdr_type id, time_t htime)
{
	...
	assert(Headers[id].type == ftDate_1123);	/* must be of an appropriate type */
	assert(htime >= 0);
	//zhb: just don't care the sequence
...
httpHeaderEntryParseCreate(const char *field_start, const char *field_end)
{
	...
	/* is it a "known" field? */
	id = httpHeaderIdByNameNew(field_start, name_len, Headers, HeadersHash, HDR_ENUM_END);
	if (id < 0)
		id = HDR_OTHER;
	assert_eid(id);
	e->id = id;
...
int
httpHeaderIdByNameNew(const char *name, int name_len, const HttpHeaderFieldInfo * info, HttpHeaderHashField ** hash, int end)
{
	if(name_len < 2)
		return -1;
	...
	// fix bug: header like this Cache-Control & Cache-control

	//int nID = (name[0] + name[1] + name[name_len - 1] + name[name_len - 2] + name[name_len/2]) * name_len % 137;
	//int nID = (towlower(name[0]) + towlower(name[1]) + towlower(name[name_len - 1]) + towlower(name[name_len - 2]) + towlower(name[name_len/2])) * name_len % 137;
	int nID = (tolower(name[0]) + tolower(name[1]) + tolower(name[name_len - 1]) + tolower(name[name_len - 2]) + tolower(name[name_len/2])) * name_len % 137;
	
	// end add

	if(nID < 0)
		return -1;
	HttpHeaderHashField * p = (HttpHeaderHashField *)hash[nID];
	while(NULL != p)
	{
		if(name_len != strLen(info[p->id].name))
		{
			p = p->next;
			continue;
		}
		if (!strncasecmp(name, strBuf(info[p->id].name),
					name_len < 0 ? strLen(info[p->id].name) + 1 : name_len))
			return p->id;
		p = p->next;
	}
	return -1;
...
httpHeaderIdByNameDef(const char *name, int name_len)
{
	...
	if(!HeadersHash)
		HeadersHash = httpHeaderBuildHashFields(HeadersAttrs, HDR_ENUM_END);
	return httpHeaderIdByNameNew(name, name_len, Headers, HeadersHash, HDR_ENUM_END);
}
