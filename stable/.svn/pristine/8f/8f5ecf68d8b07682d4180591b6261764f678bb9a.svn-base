...
static void __eventDiffTime(const struct timeval *beg_tv, const struct timeval *end_tv, struct timeval *diff_tv)
{
    diff_tv->tv_sec  = (end_tv->tv_sec - beg_tv->tv_sec);
    diff_tv->tv_usec = (end_tv->tv_usec - beg_tv->tv_usec);
    
    while(0 > diff_tv->tv_usec)
    {
        diff_tv->tv_sec  --;
...
static int __eventRunTimeInMsec(const struct timeval *beg_tv)
{
    ...
    __eventDiffTime(beg_tv, &end_tv, &diff_tv);
...
eventRun(void)
{
	...
	{
		int valid = 1;
        if(0 < Config.evenRunMaxMsec && Config.evenRunMaxMsec <= __eventRunTimeInMsec(&beg_tv))
        {
            break;
        }
		...
		if (valid)
		{
            ...
            {
                struct timeval   __beg_tv;
                
                gettimeofday(&__beg_tv, NULL);
                
                func(arg);

    			debug(41, 0)("eventRun: handled '%s', id %d, elapsed %d micro seconds\n",
    						  event->name, event->id, __eventRunTimeInMsec(&__beg_tv));       
		   }
		   else
		   {
		        func(arg);
    ...
    if(Config.onoff.debug_80_log)
    {
	debug(41, 0)("eventRun: total elapsed %d micro seconds\n", __eventRunTimeInMsec(&beg_tv));	
	}
	return;
}
