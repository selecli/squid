...
#ifdef CC_FRAMEWORK
void httpReplyHdrCacheInit(HttpReply * rep);
void httpReplyHdrCacheClean(HttpReply * rep);
...
httpRedirectReply(HttpReply * reply, http_status status, const char *loc)
{
    ...
    if (status == HTTP_BAD_REQUEST
            ...
            || status == HTTP_METHOD_NOT_ALLOWED                // 405
            || status == HTTP_PROXY_AUTHENTICATION_REQUIRED    // 407
            || status == HTTP_UNAUTHORIZED)
        httpHeaderPutStr(hdr, HDR_SERVER, appname_string);
#else
...
#ifdef CC_FRAMEWORK
	httpHeaderDelById(&rep->header, HDR_EXPIRES);
#endif
...
httpReplyHdrExpirationTime(const HttpReply * rep)
{
	...
	if (rep->cache_control)
	{
		...
		else
		{
			...
			if (rep->cache_control->max_age >= 0)
				return squid_curtime;*/
			ret = cc_call_hook_func_reply_cannot_find_date(rep);
			if (ret)
			{
				if (rep->cache_control->s_maxage >= 0)
				{
					debug(105, 4)("httpReplyHdrExpirationTime: squid_curtime is [%ld]\n", squid_curtime);
					debug(105, 4)("httpReplyHdrExpirationTime: rep->cache_control->s_maxage is [%ld]\n", rep->cache_control->s_maxage);
					debug(105, 4)("httpReplyHdrExpirationTime: return time is [%ld]\n", squid_curtime + rep->cache_control->s_maxage);
					return squid_curtime + rep->cache_control->s_maxage;
				}
				if (rep->cache_control->max_age >= 0)
				{
					debug(105, 4)("httpReplyHdrExpirationTime: squid_curtime is [%ld]\n", squid_curtime);
					debug(105, 4)("httpReplyHdrExpirationTime: rep->cache_control->max_age is [%ld]\n", rep->cache_control->max_age);
					debug(105, 4)("httpReplyHdrExpirationTime: return time is [%ld]\n", squid_curtime + rep->cache_control->max_age);
					return squid_curtime + rep->cache_control->max_age;
				}
			}
			else
			{
	...
	if (can_find_vary && Config.onoff.vary_ignore_expire &&
