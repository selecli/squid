...
static DZCCB storeUfsZCopyDone;
...
void
storeUfsZCopy(SwapDir * SD, storeIOState * sio, int targetfd, size_t size, squid_off_t offset, STRCB * callback, void *callback_data)
{
    ufsstate_t *ufsstate = (ufsstate_t *) sio->fsstate;
    ...
    cbdataLock(callback_data);
    debug(79, 3) ("storeUfsZCopy: dirno %d, fileno %08X, FD %d\n",
	sio->swap_dirn, sio->swap_filen, ufsstate->fd);
    sio->offset = offset;
    //ufsstate->flags.zcopying = 1;
    /*
    file_zcopy(ufsstate->fd,
	buf,
	size,
	(off_t) offset,
	storeUfsZCopyDone,
	sio);
	*/
    int sfResult = sendfile(targetfd, ufsstate->fd, &offset, size);
    int errflag = 0;
    if(sfResult < 0)
    {
	    errflag = sfResult;
    }
    storeUfsZCopyDone(ufsstate->fd, targetfd, sfResult, errflag, sio);
...
static void
storeUfsZCopyDone(int fd, int targetfd, int len, int errflag, void *my_data)
{
    storeIOState *sio = my_data;
    ufsstate_t *ufsstate = (ufsstate_t *) sio->fsstate;
    STRCB *callback = sio->zcopy.callback;
    void *their_data = sio->zcopy.callback_data;
    ssize_t rlen;

    debug(79, 3) ("storeUfsZCopyDone: dirno %d, fileno %08X, FD %d, len %d\n",
	sio->swap_dirn, sio->swap_filen, fd, len);
    ufsstate->flags.reading = 0;
    if (errflag) {
	debug(79, 3) ("storeUfsZCopyDone: got failure (%d)\n", errflag);
	rlen = -1;
    } else {
	rlen = len;
	sio->offset += len;
    ...
    assert(callback);
    assert(their_data);
    sio->zcopy.callback = NULL;
    sio->zcopy.callback_data = NULL;
    if (cbdataValid(their_data))
	callback(their_data, NULL, rlen);
    cbdataUnlock(their_data);
}
