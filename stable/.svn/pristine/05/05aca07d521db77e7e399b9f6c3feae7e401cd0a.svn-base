 ...
 */
static int
rfc1035HeaderPack(char *buf, size_t sz, struct rfc1035_message_st * hdr)
{
	int off = 0;
	unsigned short s;
	unsigned short t;
 ...
 * of the struct rfc1035_message_st structure.
 *
 ...
 */
static int
rfc1035HeaderUnpack(const char *buf, size_t sz, int *off, struct rfc1035_message_st * h)
{
	unsigned short s;
	unsigned short t;
	xassert(*off == 0);
 ...
 */
static int
rfc1035RRUnpack(const char *buf, size_t sz, int *off, struct rfc1035_rr_st * RR)
{
	unsigned short s;
	unsigned int i;
...
static void
rfc1035RRDestroy(struct rfc1035_rr_st * rr, int n)
{
	if (rr == NULL)
		return;
	xassert(n > 0);
	...
	{
		if (rr[n].rdata)
			safe_process(free, rr[n].rdata);
	}
	safe_process(free, rr);
}
 ...
 */
static int
rfc1035QueryUnpack(const char *buf, size_t sz, int *off, struct rfc1035_query_st * query)
{
	unsigned short s;
...
void
rfc1035MessageDestroy(struct rfc1035_message_st * msg)
{
	if (!msg)
		return;
	if (msg->query)
		safe_process(free, msg->query);
	if (msg->answer)
		rfc1035RRDestroy(msg->answer, msg->ancount);
	safe_process(free, msg);
}

/*
 * rfc1035QueryCompare()
 *
 * Compares two struct rfc1035_query_st entries
 *
 * Returns 0 (equal) or !=0 (different)
 */
int
rfc1035QueryCompare(const struct rfc1035_query_st * a, const struct rfc1035_query_st * b)
{
	size_t la, lb;
	if (a->qtype != b->qtype)
		return 1;
	...
	printf("compare: %s; %s; %ld\n", a->name, b->name, (long)la);
...
rfc1035MessageUnpack(const char *buf,
					 size_t sz,
					 struct rfc1035_message_st ** answer)
{
	int off = 0;
	int i;
	int nr = 0;
	struct rfc1035_message_st *msg;
	struct rfc1035_rr_st *recs;
	struct rfc1035_query_st *querys;
 ...
 */
ssize_t
rfc1035BuildAQuery(const char *hostname, char *buf, size_t sz, unsigned short qid, struct rfc1035_query_st * query)
{
	static struct rfc1035_message_st h;
	size_t offset = 0;
	memset(&h, '\0', sizeof(h));
	h.id = qid;
	h.qr = 0;
 ...
 */
ssize_t
rfc1035BuildPTRQuery(const struct in_addr addr, char *buf, size_t sz, unsigned short qid, struct rfc1035_query_st * query)
{
	static struct rfc1035_message_st h;
	size_t offset = 0;
	static char rev[32];
...
main(int argc, char *argv[])
{
	...
	while (fgets(input, 512, stdin))
	{
		...
		rl = recv(s, rbuf, 512, 0);
		{
			...
			int i;
			int n;
			struct rfc1035_rr_st *answers = NULL;
