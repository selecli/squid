...
if (($fcext_win > 0))
then
/bin/rm -f /usr/local/squid/etc/fcexternal.conf
fi
...
#/bin/cp -f ./maint/squid.init /etc/init.d/squid
...
#/bin/cp -f ./ssr/fc.amr /etc/ChinaCache/app.d/ 
...
# -------------------- detectorig install --------------------
cd $Dir/detectorig
chmod +x ./detectorig_install.sh
./detectorig_install.sh
cd $Dir
#------------- For TCP settings  ------------------------------
