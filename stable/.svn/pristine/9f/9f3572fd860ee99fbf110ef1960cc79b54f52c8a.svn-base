...
#ifdef CC_FRAMEWORK
static void listen_fd_alive_ck(int num)
{
	static time_t last_appear = 0;
	int time_spend = 0;
	int i;
	if (listen_80_fd < 0)
		return;
	if (!last_appear)
		last_appear = squid_curtime;
	for (i = 0; i < num; i++)
	{
		if (events[i].data.fd == listen_80_fd)	
		{
			last_appear = squid_curtime;
			break;
		}
	}
	if ((time_spend = squid_curtime - last_appear) > 60)
	{
		debug(1, 0)("Warning: listen_fd %d of port 80 is not returned by epoll_wait in last %ds, all listen_fds will be reopened to prevent FC from rejecting new incoming requests\n",listen_80_fd, time_spend);
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_set_epoll_load(num, events);
	listen_fd_alive_ck(num);
#endif
	if (num < 0)
	{
		getCurrentTime();
