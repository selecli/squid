...
#ifdef CC_FRAMEWORK
#define cc_sleep(nsec)  do{\
    if(Config.onoff.debug_80_log) {\
        debug(1, 1) ("[DEBUG] sleep %u seconds at %s:%d\n", (nsec), __func__, __LINE__);\
    }\
    sleep(nsec);\
}while(0)

#define cc_usleep(msec)  do{\
    if(Config.onoff.debug_80_log) {\
        debug(1, 1) ("[DEBUG] usleep %u microseconds at %s:%d\n", (msec), __func__, __LINE__);\
    }\
    usleep(msec);\
}while(0)
