...
#ifdef CC_FRAMEWORK
		if(Config.cacheSwap.n_configured - i == 1)
			store_dirs_rebuilding--;
#endif
		sd->init(sd);
	}
...
#ifdef CC_FRAMEWORK
	eventAdd("storeDirLimitObj", storeDirLimitObj, NULL, 10.0, 1);
#endif
}
...
#ifdef CC_FRAMEWORK
static void storeDirLimitObj(void* data)
{
	SwapDir* sd = NULL;
	int i, total = 0;
	int mark = 0;
	...
	mark = cc_call_hook_func_limit_obj(total);
	debug(47, 3)("storeDirLimitObj: This starts...mark[%d]\n", mark);
	if(-1 == mark)
		goto finish;
	else if(0 == mark)
		goto end;
	else if(1 == mark)
	{
		for (i = 0; i < Config.cacheSwap.n_configured; i++)
		{
			sd = &Config.cacheSwap.swapDirs[i];
			sd->limitobj(sd);
			debug(47, 3)("storeDirLimitObj_all: [%s]snp[%d]\n", sd->path, sd->snapshot);
		}
	}
	else if(2 == mark)
	{	
	...
	{	
		...
		{
			sd = &Config.cacheSwap.swapDirs[i];
			if(!strcmp(sd->type, "coss") || !strcmp(sd->type, "aufs"))
				sd->limitobj(sd);
		}
		...
		fatalf("Limit Obj: mark not valid\n");
end:
	eventAdd("storeDirLimitObj", storeDirLimitObj, NULL, 2.0, 1);
finish:
	return;
}
#endif
...
storeDirSelectSwapDirRoundRobin(const StoreEntry * e)
{
...
#ifdef CC_FRAMEWORK
    char *forbid = NULL;
    int ret = cc_call_hook_func_private_assign_dir(e, &forbid);
    if (ret >= 0  && ret < Config.cacheSwap.n_configured) 
    {   
        return ret;
    } 
#endif
	for (i = 0; i <= Config.cacheSwap.n_configured; i++)
    {
        if (++dirn >= Config.cacheSwap.n_configured)
            dirn = 0;
        /********************added by jiangbo.tian ****************************/
#ifdef CC_FRAMEWORK
        if (forbid && forbid[dirn])
        {   
            debug(20,2)("%s : private disk\n", __func__);
            continue;
        }
        ret = cc_call_hook_func_private_select_swap_dir_round_robin(e);
...
/********************added by jiangbo.tian end****************************/
		...
		{
			debug(20,2)("storeDirSelectSwapDirRoundRobin: sd read_only\n");
			continue;
		}
		if (sd->cur_size > sd->max_size)
		{
			debug(20,2)("storeDirSelectSwapDirRoundRobin: sd cur_size > sd->max_size\n");
			continue;
		}
		if (!storeDirValidSwapDirSize(dirn, objsize))
		{
			debug(20,2)("storeDirSelectSwapDirRoundRobin: not valid store dir\n");
			continue;
		}
		/* check for error or overload condition */
		if (sd->checkobj(sd, e) == 0)
		{
			debug(20,2)("storeDirSelectSwapDirRoundRobin: checkobj(sd,e) == 0\n");
			continue;
		}
		...
		debug(20,3)("max_load is changed to %d\n", max_load);
...
#ifdef CC_FRAMEWORK
    char *forbid = NULL;
    dirn = cc_call_hook_func_private_assign_dir(e, &forbid);
...
storeDirUpdateSwapSize(SwapDir * SD, squid_off_t size, int sign)
{
	...
	SD->cur_size += k;
#ifdef CC_FRAMEWORK
	SD->cur_obj_cnt += (sign > 0 ? 1 : -1);
#endif
	debug(47, 2)("[%s],cur=[%d],snp=[%d]\n", SD->path, SD->cur_obj_cnt, SD->snapshot);
	store_swap_size += k;
	if (sign > 0)
		n_disk_objects++;
	else if (sign < 0)
...
#ifdef CC_FRAMEWORK
	storeAppendPrintf(sentry, "Maximum Swap Size      : %8lld KB\n",
					  Config.Swap.maxSize);
#else
	storeAppendPrintf(sentry, "Maximum Swap Size      : %8ld KB\n",
					  (long int) Config.Swap.maxSize);
#endif
	storeAppendPrintf(sentry, "Current Store Swap Size: %8llu KB\n",
					  store_swap_size);
#ifdef CC_FRAMEWORK
	storeAppendPrintf(sentry, "Current Capacity       : %d%% used, %d%% free\n",
			percent_64(store_swap_size, Config.Swap.maxSize),
			percent_64((Config.Swap.maxSize - store_swap_size), Config.Swap.maxSize));
#else	
	storeAppendPrintf(sentry, "Current Capacity       : %d%% used, %d%% free\n",
...
storeDirOpenSwapLogs(void)
{
	...
	SwapDir *sd;
        ...
        if(uid == 0)
        {
            debug(20, 3)("WARNING: storeDirOpenSwapLogs effective user is root!"); 
            leave_suid();
        }
...
storeDirWriteCleanLogs(int reopen)
{
	...
	if (store_dirs_rebuilding)
	{
		debug(20, 1) ("storeDirWriteCleanLogs: Not currently OK to rewrite swap log.\n");
        ...
        if(uid == 0)
        {
            debug(20, 3)("WARNING: storeDirWriteCleanLogs effective user is root!"); 
            leave_suid();
        }
...
#ifdef CC_FRAMEWORK
void
storeDisDirWriteCleanLogs(void * pj)
{
	const StoreEntry *e = NULL;
	int n = 0;
	struct timeval start;
	...
	if (store_dirs_rebuilding)
	{
		debug(20, 1) ("storeDisDirWriteCleanLogs: Not currently OK to rewrite swap log.\n");
		debug(20, 1) ("storeDisDirWriteCleanLogs: Operation aborted.\n");
		return;
	}

	if(eventFind(storeDisDirWriteCleanLogs, NULL))
		return;
	
        if(uid == 0)
        {
            debug(20, 3)("WARNING: storeDisDirWriteCleanLogs effective user is root!"); 
            leave_suid();
        }
	...
	while (notdone)
	{
		...
		for (dirn = 0; dirn < Config.cacheSwap.n_configured; dirn++)
		{
			...
			{
				getCurrentTime();
				debug(20, 1) ("disWirteSwapfile: %7d entries written. Let's have a break.\n", n);
				eventAdd("disWriteLogs", storeDisDirWriteCleanLogs, NULL, 3.0, 1);
	...
	storeDirOpenSwapLogs();
	debug(20, 1) ("disWirteSwapfile: Finished Writing a CLEAN swap file\n");
...
storeDirCallback(void)
{
	...
	static int ndir = 0;
#ifdef CC_FRAMEWORK
	cc_call_hook_func_clear_dir_load();
#endif
	do
	{
		j = 0;
