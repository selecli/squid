...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_error_page(err, request);
#endif
...
errorBuildReply(ErrorState * err)
{
...
#ifdef CC_FRAMEWORK
	int ret_i = cc_call_hook_func_is_webluker_set();
	if(ret_i)
		httpHeaderPutStrf(&rep->header, HDR_X_WEBLUKER_ERROR, "%d %s", err->http_status, "Access Denied");
	else
#endif
	...
	int ret_i = cc_call_hook_func_is_webluker_set();
	if(ret_i)
		httpHeaderPutStrf(&rep->header, HDR_X_WEBLUKER_ERROR, "%s %d",
						  name, err->xerrno);
	else
...
errorBuildContent(ErrorState * err)
{
	...
	#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_set_error_text(err,&m);
	#endif 
	//added end
	assert(m);
	while ((p = strchr(m, '%')))
