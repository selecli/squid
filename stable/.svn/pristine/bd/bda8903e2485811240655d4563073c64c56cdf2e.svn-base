 ...
 */
void md5_init(struct md5_context_st *context)
{
...
/* MD5 block update operation. Continues an MD5 message-digest
   ...
   context.
 */
void md5_update (struct md5_context_st *context, unsigned char *input, unsigned int inputLen)
{
 ...
 */
void md5_final (unsigned char* digest, struct md5_context_st *context)
{
...
static struct md5_context_st g_stMD5_CTX;
...
{
	md5_init(&g_stMD5_CTX);
	md5_update(&g_stMD5_CTX, (unsigned char *)buffer, (unsigned int)length);
	md5_final(digest, &g_stMD5_CTX);
}
static char g_szDigest[16];
/*得到一个字符串的md5摘要，用32个byte表示
 *输入：
 *    buffer----字符串
 *    length----字符串长度
 *输出：
...
{
	getMd5Digest(buffer, length, (unsigned char *)g_szDigest);
	int ret;
	int i;
	for(i=0;i<16;i++)
	{
		ret = ((g_szDigest[i]&0xf0)>>4);
		if(ret <= 9)
		{
			digest[2*i] = ret + '0';
		}
