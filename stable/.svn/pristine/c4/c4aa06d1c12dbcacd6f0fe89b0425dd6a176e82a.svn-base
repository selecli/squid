...
storeSwapOutStart(StoreEntry * e)
{
	...
	{
#ifdef CC_FRAMEWORK
		cc_call_hook_func_store_no_cache_reason(e, NCR_DISK);
#endif
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_store_no_cache_reason(e, NCR_OFF_T_OVERFLOW);
#endif  
		storeAbort(e);
		return;
	}
...
storeSwapOutFileClosed(void *data, int errflag, storeIOState * sio)
{
	...
	if (errflag)
	{
		...
		{
#ifdef CC_FRAMEWORK
			cc_call_hook_func_private_setDiskErrorFlag(e, errflag);
			debug(20, 1)("need real unlink when a disk error occoured during swaping \"%s\" out\n", mem->url);
#endif
			storeUnlink(e);
		}
		e->swap_filen = -1;
