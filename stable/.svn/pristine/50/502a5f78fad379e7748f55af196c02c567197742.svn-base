...
#ifdef CC_FRAMEWORK
static void storeZCopySelectWritableStart(store_client *sc);
static CWCB storeZCopySelectWritableDone;
#endif
...
#ifdef CC_FRAMEWORK
	STZCCB *zc_callback = sc->zc_callback;
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_store_client_copy(e, sc);
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_store_client_copy3(e, sc);
#endif
	/*
...
static void storeZCopySelectWritableStart(store_client *sc)
{
	assert(sc->is_zcopy);
	cbdataLock(sc);
		comm_z_write(sc->target_fd,
			STORE_CLIENT_BUF_SZ,
			storeZCopySelectWritableDone,
			sc,
			NULL);
}

static void storeZCopySelectWritableDone(int fd, char *buf, size_t size, int errflag, void *data)
{
	store_client *sc = data;
	if(errflag)
	{
		debug(200, 2)("storeZCopySelectWritableDone got an errorflag=%d\n", errflag);
		cbdataUnlock(sc);
		comm_close(fd);
		return;
	}
	...
	storeZCopy(sc->swapin_sio,
			sc->target_fd,
			sc->copy_size,
			sc->copy_offset + sc->entry->mem_obj->swap_hdr_sz,
			storeClientReadBody,
			sc);
...
#ifdef CC_FRAMEWORK
		//int is_zcopy_disabled = cc_call_hook_func_store_client_file_read();
		if(sc->zc_callback && sc->is_zcopyable && 
				Config.cacheSwap.swapDirs[sc->swapin_sio->swap_dirn].obj.zcopy  && 
				((!sc->range_flag) || ((sc->range->specs.count == 1) && (sc->range_iter->prefix_size > 0)) ))
		{
			debug(200, 3)("zcopy is enabled, mem->swap_hdr_sz=%d\n", mem->swap_hdr_sz);
			sc->is_zcopy = 1;
			memFree(sc->copy_buf, MEM_STORE_CLIENT_BUF);
			sc->copy_buf = NULL;
			storeZCopySelectWritableStart(sc);
		}
		else
		{
			debug(200, 3)("zcopy is disabled\n");
			sc->is_zcopy = 0;
#endif
			storeRead(sc->swapin_sio,
					sc->copy_buf,
...
storeClientReadHeader(void *data, const char *buf, ssize_t len)
{
	...
	do
	{
		...
		if ((!new_store_url) && mem->url && (strcasecmp(mem->url, new_url) != 0) && cc_call_hook_func_private_store_meta_url_for_redirect_location(http))
#else
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_store_meta_url(sc,new_url);
#endif
	xfree(new_url);
...
#ifdef CC_FRAMEWORK
	int shouldReturn = 0;
	squid_off_t hookResult = cc_call_hook_func_private_storeLowestMemReaderOffset(entry, &shouldReturn);
	if(shouldReturn)
	{
		return hookResult;
	}
...
#ifdef CC_FRAMEWORK
        int ret = cc_call_hook_func_invoke_handlers(e);
	if(ret == 1)
		return;
#endif
...
#ifdef CC_FRAMEWORK
    int abort  = 1;
    cc_call_hook_func_quick_abort_check(entry, &abort);
    if(abort ==  0)
        return 0;
#endif
	if (curlen > expectlen)
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_store_no_cache_reason(entry, NCR_CLIENT_ABORT);
#endif  
	storeAbort(entry);
}
