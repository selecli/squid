...
#ifdef CC_FRAMEWORK
STCB clientSendMoreData;
#else
static STCB clientSendMoreData;
#endif
#ifdef CC_FRAMEWORK
static STZCCB clientSendMoreDataZ;
static CWCB clientZCopyBodyComplete;
#endif
static STHCB clientSendHeaders;
#ifdef CC_FRAMEWORK  
STHCB clientCacheHit;
#else
...
#endif
#ifdef CC_FRAMEWORK
void clientProcessExpired(clientHttpRequest *);
#else
...
storeClientCopyHeaders(store_client * sc, StoreEntry * e, STHCB * callback, void *callback_data)
{
	...
	http->header_entry = e;
#ifdef CC_FRAMEWORK
	//http->sc->zc_callback = clientSendMoreDataZ;
	//http->sc->target_fd = http->conn->fd;
	//cc_call_hook_func_client_set_zcopyable(sc, http->conn->fd);
#endif
...
#ifdef CC_FRAMEWORK
		http->acl_checklist = cc_clientAclCheckListCreate("follow_x_forwarded_for",http);
#else
		http->acl_checklist = clientAclChecklistCreate( Config.accessList.followXFF, http);
#endif
...
clientCheckFollowXForwardedFor(void *data)
{
    clientHttpRequest *http = data;
#if FOLLOW_X_FORWARDED_FOR
#ifdef CC_FRAMEWORK     // added by chenqi, case 3097
    if (httpHeaderHas(&http->request->header, HDR_X_FORWARDED_FOR) && cc_get_acl_access_by_token("follow_x_forwarded_for", http))
#else
...
clientAccessCheck(void *data)
{
	clientHttpRequest *http = data;
#ifdef CC_FRAMEWORK
	http->acl_checklist = cc_clientAclCheckListCreate("http_access",http);
#else
...
clientAccessCheck2(void *data)
{
	clientHttpRequest *http = data;
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token("http_access2",http);
	if (acl != NULL &&  http->redirect.status)
#else
		if (Config.accessList.http2 && !http->redirect.status)
#endif
		{
#ifdef CC_FRAMEWORK
			http->acl_checklist = cc_clientAclCheckListCreate("http_access2",http);
#else
...
#ifdef CC_FRAMEWORK
	if((h->sc->range_flag = h->request->flags.range))
	{
		h->sc->range = h->request->range;
		h->sc->range_iter = &h->range_iter;
	}
#endif
...
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_http_client_create_store_entry(h, e);
	if(ret == 1)
		return e;
#endif
...
#ifdef CC_FRAMEWORK
                if(cc_call_hook_func_client_access_check_done(http) > 0)
                {
                        return;
                }
#endif
...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token("no_cache",http);
	if (acl != NULL &&  http->request->flags.cachable)
#else
		if (Config.accessList.noCache && http->request->flags.cachable)
#endif
		{
#ifdef CC_FRAMEWORK
			http->acl_checklist = cc_clientAclCheckListCreate("no_cache",http);
#else
...
#ifdef CC_FRAMEWORK
	if (1 != answer)
		cc_call_hook_func_httpreq_no_cache_reason(http->conn->fd, http, NCR_NOT_ENTRY_CACHABLE);
#endif
	http->acl_checklist = NULL;
...
#ifdef CC_FRAMEWORK
			int can_find_vary = cc_call_hook_func_can_find_vary();
			int has_vary = 0;
			if(can_find_vary)
			{
				has_vary = httpHeaderHas(&rep->header, HDR_VARY);
			}
#else
...
clientProcessExpired(clientHttpRequest * http)
{
	...
	const char *etag;
#ifdef CC_FRAMEWORK
	const int can_revalidate = (http->entry->mem_obj->reply->sline.status == HTTP_OK || http->entry->mem_obj->reply->sline.status ==HTTP_PARTIAL_CONTENT);
...
clientHandleIMSReply(void *data, HttpReply * rep)
{
...
#ifdef CC_FRAMEWORK
	int ret = 0; 
	if (NULL != entry && NULL != entry->mem_obj && NULL != entry->mem_obj->reply)
		ret = cc_call_hook_func_get_content_orig_down(http, entry->mem_obj->reply->sline.status);
	if (!rep || ret) 
#else
	if (!rep)
#endif
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_private_getReplyUpdateByIMSRep(http);
#endif
			storeTimestampsSet(oldentry);
			storeUpdate(oldentry, http->request);
			http->log_type = LOG_TCP_REFRESH_HIT;
			debug(22,2)("XIAOSI304, timestamp=%d,expires=%d,reply->expires=%d\n",oldentry->timestamp, oldentry->expires,oldentry->mem_obj->reply->expires);
		}
		else
		{
			http->log_type = LOG_TCP_REFRESH_FAIL_HIT;
	...
	else
	{
		...
		{
			cc_call_hook_func_client_handle_ims_reply(http);
		}
		//	storeRelease(http->old_entry);
#endif
...
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_process_modified_since(entry,request);
	if(ret == 1){
		debug(33, 3) ("-->  YES: entry older than client\n");
		return 1;
	}
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_change_accesslog_info(http);
#endif
...
#ifdef CC_FRAMEWORK
		http->al.cache.hit = http->flags.hit;
#endif
		if (request)
		{
...
#ifdef CC_FRAMEWORK
		acl_access * acl = cc_get_acl_access_by_token_and_host("log_access",request->host);
		if (!acl || aclCheckFast(acl, http->acl_checklist))
#else
...
#ifdef CC_FRAMEWORK
				cc_call_hook_func_private_record_trend_access_log(http);
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_req_free(http);
	cc_call_hook_func_http_req_debug_log(http);
#endif
...
#ifdef CC_FRAMEWORK
	cc_free_clientHttpRequest_private_data(http);
#endif
	cbdataFree(http);
}
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_conn_state_free(connState);
	cc_free_connStateData_private_data(connState);
#endif
...
clientInterpretRequestHeaders(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_add_http_orig_range(http);
#endif
	/* added end */
	...
	if (clientCachable(http))
		...
		cc_call_hook_func_httpreq_no_cache_reason(http->conn->fd, http, NCR_NOT_METHOD_CACHABLE);
#endif
...
clientBuildRangeHeader(clientHttpRequest * http, HttpReply * rep)
{
	...
	int ret = 0;
	ret = cc_call_hook_func_private_clientBuildRangeHeader(http);
	if (!rep)
		range_err = "no [parse-able] reply";
#ifdef CC_FRAMEWORK
	else if (rep->sline.status != HTTP_OK && rep->sline.status !=HTTP_PARTIAL_CONTENT)
		range_err = "wrong status code";
	else if(rep->sline.status == HTTP_PARTIAL_CONTENT && ret == 0)
		range_err = "wrong status code";
	else if (httpHeaderHas(hdr, HDR_CONTENT_RANGE) && ret == 0)
#else
...
#ifdef CC_FRAMEWORK
	else if (0 == cc_call_hook_func_support_complex_range(http) && httpHdrRangeIsComplex(http->request->range))
#else
	...
	if (range_err)
	{
		...
		if(strcmp(range_err,"canonization failed"))
	...
	else
	{
		...
		if (spec_count == 1)
		{
			...
			if( rep->content_range)
				httpHeaderAddContRange(hdr, *spec, rep->content_range->elength);
			else
...
clientBuildReplyHeader(clientHttpRequest * http, HttpReply * rep)
{
	...
	if (request->range)
	{
		HttpHdrRangePos pos = HttpHdrRangeInitPos;
		const HttpHdrRangeSpec *spec;
		spec = httpHdrRangeGetSpec(request->range, &pos);
		squid_off_t orig_length = spec->length;
		debug(33,2)("clientBuildReplyHeader :request->range start from   %" PRINTF_OFF_T ", and orig_length is%" PRINTF_OFF_T "  \n",spec->offset,spec->length);
	...
	int ret2 = cc_call_hook_func_is_webluker_set();
	int ret3 = cc_call_hook_func_fc_reply_header(http,hdr);
    ...
    if (0 == ret3)
    {
        ...
        {	
            if(http->entry)
            {    // 304 should not show PENDING
                if (STORE_PENDING == http->entry->store_status && LOG_TCP_IMS_HIT != http->log_type)   
                {
                    //if(ret2)
                        //httpHeaderPutStrf(hdr, HDR_X_WEBLUKER, "PENDING via %s", getMyHostname());	
                    //else
                        httpHeaderPutStrf(hdr, HDR_X_CACHE, "PENDING from %s", getMyHostname());	

                }
                //else if(ret2)
                    //httpHeaderPutStrf(hdr, HDR_X_WEBLUKER, "HIT via %s", getMyHostname());	
                else
                    httpHeaderPutStrf(hdr, HDR_X_CACHE, "HIT from %s", getMyHostname());	
            }
        }
        else
        {
            //if (ret2)
                //httpHeaderPutStrf(hdr, HDR_X_WEBLUKER, "MISS via %s", getMyHostname());
            //else
                httpHeaderPutStrf(hdr, HDR_X_CACHE, "MISS from %s", getMyHostname());
        }
    }
#else
	...
	cc_call_hook_func_private_client_build_reply_header(http,rep);
...
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_private_set_client_keepalive(http);
	if(0 == ret)
	{
#endif
...
clientCloneReply(clientHttpRequest * http, HttpReply * orig_rep)
{
	...
	if (rep->pstate == psParsed)
	{
		...
		#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_process_error_page(http,rep);
		cc_call_hook_func_private_set_status_line(http,rep);
		               //cc_call_hook_private_process_error_page(http,rep);
		#endif
			               /*added by jiangbo.tian end*/
	}
	else
	{
...
clientCacheHit(void *data, HttpReply * rep)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_client_cache_hit_start(http->conn->fd, e->mem_obj->reply, e);
#endif
...
#ifdef CC_FRAMEWORK
		if (mem->store_url == NULL && mem->url && strcmp(r->store_url, mem->url) != 0 && cc_call_hook_func_private_client_cache_hit_for_redirect_location(http))
#else
...
#ifdef CC_FRAMEWORK            
		if ((strcmp(mem->url, urlCanonical(r)) != 0) && cc_call_hook_func_private_client_cache_hit_for_redirect_location(http))
		{
	...
	if (httpHeaderHas(&r->header, HDR_IF_NONE_MATCH))
	{
		...
		int has_etag;
#ifdef CC_FRAMEWORK
		if (cc_call_hook_func_private_checkModPartitionStoreEnable(http)
		    && mem->reply->sline.status != HTTP_OK
		    && mem->reply->sline.status != HTTP_PARTIAL_CONTENT)
		{
		...
		if (rep_etag)
		{
			...
			else
			{
				...
				is_modified = 1;
#ifdef CC_FRAMEWORK
				//this is used to meet the needs of trend 
				int return_v = 0;
				return_v = cc_call_hook_func_private_clientCacheHit_etag_mismatch(http);
				if ( 1 == return_v)
					return;
#endif
			}
...
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_private_client_cache_hit_start(http,e);
	if (ret ==1)
	{
		clientProcessMiss(http);
		return;
...
#ifdef CC_FRAMEWORK
	stale = cc_call_hook_func_http_client_cache_hit(http, stale);
#endif
	if (stale)
	{
...
{
#ifdef CC_FRAMEWORK
	cc_call_hook_func_client_process_hit_start(http);
#endif
	...
	if (is_modified == 0)
	{
		...
		storeComplete(e);
#ifdef CC_FRAMEWORK
		//cc_call_hook_func_client_process_hit_after_304_send(http);
#endif
		return;
	}
	/*
...
#ifdef CC_FRAMEWORK
	//cc_call_hook_func_client_process_hit_end(http);
#endif
}
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_getMultiRangeOneCopySize(http,copy_sz);
#endif
	/*
	 * update offsets
	 */
...
#ifdef CC_FRAMEWORK
static void
clientPackRangeZ(clientHttpRequest * http,
		HttpHdrRangeIter * i,
		size_t * size)
{
	const size_t copy_sz = i->debt_size <= *size ? i->debt_size : *size;
...
clientCanPackMoreRanges(const clientHttpRequest * http, HttpHdrRangeIter * i, size_t size)
{
	...
	debug(33,3)("clientCanPackMoreRanges: i->spec = %p, and size == %ld\n", i->spec, (long int) size);
...
#ifdef CC_FRAMEWORK
		if (body_off > start &&  cc_call_hook_func_support_complex_range(http))
			break;
		else
#else
...
#ifdef CC_FRAMEWORK
			cc_call_hook_func_private_clientPackMoreRanges(http, (char *)buf, size);
#endif
...
#ifdef CC_FRAMEWORK
static int
clientPackMoreRangesZ(clientHttpRequest * http, size_t size)
{
	HttpHdrRangeIter *i = &http->range_iter;
	/* offset in range specs does not count the prefix of an http msg */
	squid_off_t body_off = http->out.offset - i->prefix_size;
...
#ifdef CC_FRAMEWORK
    if (cc_call_hook_func_send_header_clone_before_reply(http,rep))
		return;
#endif
...
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token("location_rewrite_access",http);
	if (acl != NULL )
#else
		...
		{
#ifdef CC_FRAMEWORK
			ch = cc_clientAclCheckListCreate("location_rewrite_access",http);
#else
...
clientHttpReplyAccessCheck(clientHttpRequest * http)
{
	aclCheck_t *ch;
#ifdef CC_FRAMEWORK
	acl_access * acl = cc_get_acl_access_by_token("http_reply_access",http);
	if (acl != NULL &&  http->log_type != LOG_TCP_DENIED && !clientAlwaysAllowResponse(http->reply->sline.status))
#else
		if (Config.accessList.reply && http->log_type != LOG_TCP_DENIED && !clientAlwaysAllowResponse(http->reply->sline.status))
#endif
		{
#ifdef CC_FRAMEWORK
			ch = cc_clientAclCheckListCreate("http_reply_access",http);
#else
...
#ifdef CC_FRAMEWORK
	/* mod acl check fc to client */
	cc_client_acl_check(http, 1);	
#endif
...
clientCheckHeaderDone(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_repl_send_start(http);
	int ret=0;
	cc_call_hook_func_private_repl_send_start(http,&ret);
	if(ret ==1)
	{
		send_header = 0;
		return;
	...
	int ret2 = cc_call_hook_func_private_before_send_header(http,&mb); // added for parition store and 302location:w
	if(ret2 == 1)
	{
		send_header = 0;
	}
	...
	if (http->request->method == METHOD_HEAD)
	{
		...
		debug(33,3)("clientCheckHeaderDone: !clientPackMoreRanges(http, "", 0, &mb) set http->flags.done_copying = 1\n");
	...
	{
#ifdef CC_FRAMEWORK
		acl_access * acl = cc_get_acl_access_by_token("upgrade_http0.9",http);
		if (send_header && acl != NULL)
#else
			if (send_header && Config.accessList.upgrade_http09)
#endif
			{
#ifdef CC_FRAMEWORK
				aclCheck_t *checklist = cc_clientAclCheckListCreate("upgrade_http0.9",http);
#else
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_process_send_header(http);
#endif
	...
	debug(33,3)("clientCheckHeaderDone: http->out.offset == %" PRINTF_OFF_T " ,http->out.size == %" PRINTF_OFF_T"",http->out.offset, http->out.size);
#ifdef CC_FRAMEWORK
	//commSetCork(http->conn->fd);
	//commEnhanceSndBuf(http->conn->fd);
	cc_call_hook_func_reply_header_mb(http, &mb);
#endif
	...
	if (mb.size > 0)
	{
		...
		int ret = cc_call_hook_func_private_mp4_send_more_data(http,&mb,mb.buf,mb.size);
		if(ret == 1)
		{
			return;
		}
		/*
		   int ret2 = cc_call_hook_func_private_before_send_header(http,&mb); // added for parition store
		   if(ret2)
		   {
		   return;
		   }
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_after_write_header(http);
#endif
	}
	else
	{
		...
		http->sc->zc_callback = clientSendMoreDataZ;
		http->sc->target_fd = http->conn->fd;
		cc_call_hook_func_client_set_zcopyable(http->sc, http->conn->fd);
		if(http->request->flags.chunked_response ||
				http->request->protocol != PROTO_HTTP)
		{
			//to avoid assert(0) error
			http->sc->is_zcopyable = 0;
		}
		memBufClean(&mb);
#endif
...
clientSendMoreData(void *data, char *buf, ssize_t size)
{
	...
	assert(buf);
#ifdef CC_FRAMEWORK
	cc_call_hook_func_get_http_body_buf(http, buf, size);
#endif
	if (DLINK_HEAD(conn->reqs) != http)
	{       
		/* there is another object in progress, defer this one */
...
#ifdef CC_FRAMEWORK
        cc_call_hook_func_private_getOnceSendBodySize(http,size);
#endif
	if (!http->request->range && !http->request->flags.chunked_response)
	{
		//#ifdef CC_FRAMEWORK
		//	memBufDefInit(&mb);
		//      cc_call_hook_func_private_client_send_more_data(http,&mb,buf,size);
		//#endif  
	...
	if (http->request->method == METHOD_HEAD)
	{
		 ...
		 * continue...
		 */
		debug(33,3)("clientSendMoreData: http->request->method == METHOD_HEAD! set http->flags.done_copying = 1\n");
		http->flags.done_copying = 1;
		/*
	...
	if (http->request->range)
	{
		...
		{
			debug(33,3)("clientSendMoreData: !clientPackMoreRanges(http, buf, size, &mb)\n");
			http->flags.done_copying = 1;
		}
	}
	else
 ...
 */
static void
clientSendMoreDataZ(void *data, ssize_t size)
{
	clientHttpRequest *http = data;
	StoreEntry *entry = http->entry;
	ConnStateData *conn = http->conn;
	...
	if (!http->request->range && !http->request->flags.chunked_response)
	{
		...
		http->out.offset += size;
		//comm_write(fd, buf, size, clientWriteBodyComplete, http, NULL);
		//comm_z_write(fd, size, clientZCopyBodyComplete, http, NULL);
		clientZCopyBodyComplete(fd, NULL, size, 0, http);
		/* NULL because clientWriteBodyComplete frees it */
		return;
	}
	if (http->request->method == METHOD_HEAD)
	{
		 ...
		 * continue...
		 */
		debug(33,3)("clientSendMoreDataZ: http->request->method == METHOD_HEAD! set http->flags.done_copying = 1\n");
		http->flags.done_copying = 1;
		/*
		 * And as this is a malformed HTTP reply we cannot keep
		 * the connection persistent
	...
	if (http->request->range)
	{
		/* Only GET requests should have ranges */
		assert(http->request->method == METHOD_GET);
		/* clientPackMoreRangesZ() updates http->out.offset */
		/* force the end of the transfer if we are done */
		if (!clientPackMoreRangesZ(http, size))
		{
			debug(33,3)("clientSendMoreDataZ: !clientPackMoreRanges(http,  size)\n");
			http->flags.done_copying = 1;
		}
	}
	else
	...
	if (http->request->flags.chunked_response)
	{
		//FIXME:not supported squid make chunk case
	...
	{
		//comm_z_write_mbuf(fd, mb, clientWriteComplete, http);
		clientWriteComplete(fd, NULL, size, 0, http);
	}
	//memFree(buf, MEM_STORE_CLIENT_BUF);
}
#endif
...
#ifdef CC_FRAMEWORK
static void clientZCopyBodyComplete(int fd, char *buf, size_t size, int errflag, void *data)
{
	/*
	 * NOTE: clientWriteComplete doesn't currently use its "buf"
	 * (second) argument, so we pass in NULL.
...
clientKeepaliveNextRequest(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_private_client_keepalive_next_request(conn->fd);
	...
	if (http == NULL)
	{
		 ...
		 */
#ifdef CC_FRAMEWORK
		int ret = cc_call_hook_func_private_client_keepalive_set_timeout(conn->fd);
		if (ret > 0)
			commSetTimeout(conn->fd, ret, requestTimeout, conn);
		else
#endif
			commSetTimeout(conn->fd, Config.Timeout.persistent_request, requestTimeout, conn);
#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_set_sublayer_persist_connection_timeout(conn->fd);
#endif 
...
#ifdef CC_FRAMEWORK
	int ret;
	ret = cc_call_hook_func_private_client_write_complete(fd,bufnotused,size,errflag,data);
	if(ret == 1)
	{
		return ;
	}
...
#endif
#if SIZEOF_SQUID_OFF_T <= 4
	else if (http->out.offset > 0x7FFF0000)
	{
		debug(33, 1) ("WARNING: closing FD %d to prevent counter overflow\n", fd);
		debug(33, 1) ("\tclient %s\n", inet_ntoa(http->conn->peer.sin_addr));
...
#ifdef CC_FRAMEWORK
	{
		int transferDonePrecondition = 1;
		cc_call_hook_func_transfer_done_precondition(http, &transferDonePrecondition);

		//int transferDonePrecondition = (http->request->flags.chunked_response == 0);
#endif 
		if (errflag)
		{
			/*
...
#ifdef CC_FRAMEWORK
		else if (( (done = clientCheckTransferDone(http)) != 0 && transferDonePrecondition ) || size == 0)
#else
...
#endif
		{
#ifdef CC_FRAMEWORK
			//commSetNoCork(fd);
			/* added for partition store by jiangbo.tian*/ /*also for redirect_location*/
			int ret2 = cc_call_hook_func_private_http_repl_send_end(http);
			if(ret2)
			{
				debug(153,3)("cc_call_hook_func_private_http_repl_send_end return\n");
				return;
			}
			int ret;
			ret = cc_call_hook_func_http_repl_send_end(http);
			/*added end */
#endif
			debug(33, 5) ("clientWriteComplete: FD %d transfer is DONE, and http->flags.done_copying == %d\n", fd,http->flags.done_copying);
			/* We're finished case */
			if (!done)
			{
...
#ifdef CC_FRAMEWORK
			else if (clientGotNotEnough(http) &&  Config.onoff.pipeline_prefetch ==0)
#else
			...
			else if (http->request->flags.chunked_response)
			{
				...
				debug(33, 5) ("clientWriteComplete: http->request->flags.chunked_responese\n");
			...
			{
				debug(33, 5) ("clientWriteComplete: http->flags.done_copying ==%d \n", http->flags.done_copying);
				//if(http->flags.done_copying == 1)
				//						sleep(2);
#ifdef CC_FRAMEWORK
				int ret = cc_call_hook_func_private_should_close_fd(http);
				if(ret)
#endif
					comm_close(fd);
			}
...
clientProcessRequest2(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
	{
		int ret = cc_call_hook_func_private_http_req_process2(http);
		if (ret)
			return ret;	
	}	
#endif
	...
	supp = cc_call_hook_func_support_complex_range(http);
    if (http->log_type == LOG_TCP_HIT || http->log_type == LOG_TCP_MISS)
    {
        if (http->log_type == LOG_TCP_MISS)
            http->entry = NULL;
        return http->log_type;
    }
#endif
...
#ifdef CC_FRAMEWORK
		r->flags.cachable = 0;				/* add by yandong.nian */
#endif
		http->entry = NULL;
		return LOG_TCP_MISS;
	}
...
clientProcessRequest(clientHttpRequest * http)
{
	...
	{
#ifdef CC_FRAMEWORK
		cc_call_hook_func_before_clientProcessRequest2(http);
#endif
	...
	{
		int ret;
		ret = cc_call_hook_func_private_before_http_req_process(http);
		if (ret)
			return;
	}	
#endif
	...
	{
		int ret;
		ret = cc_call_hook_func_http_req_process(http);
		if (ret)
			return;
	}	
#endif
...
#ifdef CC_FRAMEWORK
                int ret = cc_call_hook_func_private_sc_unregister(http->entry, http);
                if(ret == 0){
                        http->sc = storeClientRegister(http->entry, http);
                }
#else
...
#ifdef CC_FRAMEWORK
		if((http->sc->range_flag = http->request->flags.range))
		{
			http->sc->range = http->request->range;
			http->sc->range_iter = &http->range_iter;
		}
#endif
...
#ifdef CC_FRAMEWORK
		{
			int ret = cc_call_hook_func_private_store_client_copy_hit(http);
			if(ret)
				return;
		}	
#endif
...
clientProcessMiss(clientHttpRequest * http)
{
...
#ifdef CC_FRAMEWORK
        cc_call_hook_func_get_client_area_use_ip(http);
#endif
	/*
	 ...
	 * Deny double loops
	 */
	//added by jiangbo.tian for customized_server_error_page 
#ifdef CC_FRAMEWORK
	int ret =0;
	cc_call_hook_func_private_process_miss_in_error_page(http, &ret);
	if(ret ==1)
		return;
#endif
	...
	if (http->redirect.status)
	{
		if(HTTP_BAD_REQUEST == http->redirect.status)
		{
			err = errorCon(ERR_INVALID_REQ, HTTP_BAD_REQUEST, http->request);
			http->log_type = LOG_TCP_DENIED;
			http->entry = clientCreateStoreEntry(http, http->request->method, null_request_flags);
			errorAppendEntry(http->entry, err);
			return;
		}
		else if(HTTP_UNAUTHORIZED == http->redirect.status)
		{
			err = errorCon(ERR_ACCESS_DENIED, HTTP_UNAUTHORIZED, http->request);
			http->log_type = LOG_TCP_DENIED;
			http->entry = clientCreateStoreEntry(http, http->request->method, null_request_flags);
			errorAppendEntry(http->entry, err);
			return;
		}
		else if(HTTP_FORBIDDEN == http->redirect.status)
		{
			err = errorCon(ERR_ACCESS_DENIED, HTTP_FORBIDDEN, http->request);
			http->log_type = LOG_TCP_DENIED;
			http->entry = clientCreateStoreEntry(http, http->request->method, null_request_flags);
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_http_created(http);
#endif
...
clientReadDefer(int fd, void *data)
{
	...
	ConnStateData *conn = data;
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_is_defer_read_req();
#endif
	if (conn->body.size_left && !F->flags.socket_eof)
	{
#ifdef CC_FRAMEWORK
		if (conn->in.offset >= conn->in.size - 1 || 1 == ret)
#else
	...
	{
#ifdef CC_FRAMEWORK
		if (conn->defer.until > squid_curtime || 1 == ret)
#else
...
#ifdef CC_FRAMEWORK
		/* mod acl check client to fc */
		cc_call_hook_func_before_client_acl_check(http);
		cc_client_acl_check(http, 0);	

		int retval;	
		retval = cc_call_hook_func_http_req_parsed(http);
		if (2 == retval)
		{
...
#ifdef CC_FRAMEWORK
	int ret = cc_call_hook_func_is_defer_accept_req();
	if (fdNFree() >= RESERVED_FD && 0 == ret)
#else
		if (fdNFree() >= RESERVED_FD)
#endif
			return 0;
#ifdef CC_FRAMEWORK
	if (last_warn + 15 < squid_curtime && 0 == ret)
#else
...
#ifdef CC_FRAMEWORK
		cc_call_hook_func_private_set_sublayer_tcp_keepalive(fd);
		cc_call_hook_func_private_set_sublayer_persist_request_timeout(fd);
#endif
...
clientCheckTransferDone(clientHttpRequest * http)
{
	...
	{
		debug(33,3)("clientCheckTransferDone: http->flags.done_copying == 1\n");
		return 1;
	}
	/*
	...
	if (entry->store_status == STORE_OK)
	{
		if (http->out.offset >= objectLen(entry))
		{
			...
			debug(33,3)("clientCheckTransferDone: http->out.offset >= objectLen(entry)\n");
			return 1;
		}
		else
			return 0;
	 ...
	 */
	debug(33,3)("clientCheckTransferDone: http->out.offset == %" PRINTF_OFF_T " , sendlen == %" PRINTF_OFF_T "\n",http->out.offset, sendlen);
...
clientGotNotEnough(clientHttpRequest * http)
{
#ifdef CC_FRAMEWORK
	squid_off_t cl = 0; 
	if(http->entry->mem_obj && http->entry->mem_obj->reply)
		cl = httpReplyBodySize(http->request->method, http->entry->mem_obj->reply);
	else    
		return 1;
#else
	...
	debug(33,5)("clientGotNotEnough: cl ==%" PRINTF_OFF_T " , and hs == %d , and http->out.offset == %" PRINTF_OFF_T " \n",cl,hs,http->out.offset);
/* add by xin.yao, fixed: In the long connection mode HIT cases Range request problem */
#ifdef CC_FRAMEWORK
	debug(33,5)("clientGotNotEnough: http->request->flags.range=%d\n",http->request->flags.range);
...
clientHttpConnectionsOpen(void)
{
	...
	for (s = Config.Sockaddr.http; s; s = s->next)
	{
		...
		if(comm_listen(fd)<0)
		{
			fd_close(fd);
			close(fd);
			debug(1, 1) ("comm_listen  fail: listen(%d, %s):",fd, xstrerror());
...
{
#ifdef CC_FRAMEWORK
	if ((-1 == opt_squid_id) && (1 == opt_total_squids))
	{
#endif
...
#ifdef CC_FRAMEWORK
	cc_call_hook_func_clientLscsConnectionsOpen();
#endif
}
void
...
varyEvaluateMatch(StoreEntry * entry, request_t * request)
{
	...
	if(can_find_vary)
	{
		has_vary = httpHeaderHas(&entry->mem_obj->reply->header, HDR_VARY);
	}
#else
