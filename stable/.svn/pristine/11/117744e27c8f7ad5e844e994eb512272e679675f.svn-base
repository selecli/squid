...
static int __mem_need_check(MemPool *pool)
{
    static const char *to_check_lable[] = {
        "acl",
        "acl_deny_info_list",
        "acl_ip_data",
        "acl_list",
    ...
    static int to_check_lable_size = sizeof(to_check_lable)/sizeof(to_check_lable[0]);

    int idx;

    for(idx = 0; idx < to_check_lable_size; idx ++)
    {
        if(0 == strcmp(pool->label, to_check_lable[ idx ]))
        {
            return 1; /*need to check*/
        }
    }
    return 0; /*NOT need to check*/
}

void mem_dump_debug(const char *info)
{
    int i;

    debug(3,0)("%s: %s", __func__, info); 
	for (i = 0; i < Pools.count; i++)
	{
		MemPool *pool = Pools.items[i];

		if (memPoolWasUsed(pool) && __mem_need_check(pool))
		{
			debug(3,0)("%s: %32s: alloc %8d, inuse %8d, idle %8d\n", __func__,
			           pool->label, 
			           pool->meter.alloc.level,
			           pool->meter.inuse.level,
			           pool->meter.idle.level);
		}
	}    
	//debug(3,0)("%s: dump end\n", __func__); 
