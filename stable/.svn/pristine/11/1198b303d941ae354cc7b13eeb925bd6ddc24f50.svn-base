...
static CossMemBuf *storeCossCreateMemOnlyBuf2(SwapDir * SD, int stripe);
...
static void memonly_debug(CossInfo *cs);
static void stripe_debug(CossInfo *cs);
...
#ifdef CC_FRAMEWORK
void storeCossNewPendingRelocate(CossInfo * cs, storeIOState * sio, sfileno original_filen, sfileno new_filen, int sd_index);
...
#ifdef CC_FRAMEWORK
struct _CossRemoveState {
    SwapDir* sd;
    int mark;
};
CBDATA_TYPE(CossRemoveState);
#endif
...
static sfileno
storeCossMemOnlyAllocate2(SwapDir * SD, const StoreEntry * e)
{
    CossInfo *cs = (CossInfo *) SD->fsdata;
    CossMemBuf *newmb;
    off_t retofs;
    size_t allocsize;
    sfileno f;
    int nummemstripes = cs->nummemstripes;
    int curmemstripe = cs->curmemstripe;

    coss_stats.alloc.memalloc++;
    allocsize = e->swap_file_sz;

    if (cs->memstripes[curmemstripe].membuf == NULL) {
        newmb = storeCossCreateMemOnlyBuf2(SD, curmemstripe);
        if (newmb == NULL) {
            return -1;
        }
        cs->current_memonly_membuf = newmb;
        cs->current_memonly_offset = cs->current_memonly_membuf->diskstart;
    } else if ((cs->memstripes[0].membuf != NULL) 
            && (curmemstripe == nummemstripes-1) 
            && ((cs->current_memonly_offset + allocsize) >= cs->current_memonly_membuf->diskend)) {
        cs->current_memonly_membuf->flags.full = 1;
        //debug(79, 4) ("storeCossMemOnlyAllocate2: move to the first %d memonly (%p)\n", cs->curmemstripe, cs->current_memonly_membuf);
        newmb = storeCossCreateMemOnlyBuf2(SD, 0);
        if (newmb == NULL) {
            return -1;
        }
        cs->current_memonly_membuf = newmb;
        ...
        cs->current_memonly_membuf->flags.full = 1;
        //debug(79, 4) ("storeCossMemOnlyAllocate2: move to the next %d memonly (%p)\n", cs->curmemstripe, cs->current_memonly_membuf);
        newmb = storeCossCreateMemOnlyBuf2(SD, curmemstripe+1);
        if (newmb == NULL) {
            return -1;
        }
        cs->current_memonly_membuf = newmb;
    ...
    f = storeCossDiskOffsetToFileno(retofs, cs);
    assert(f >= 0 && f <= 0xfffffff);
    debug(79, 3) ("storeCossMemOnlyAllocate2: offset %" PRId64 ", filen: %d\n", (int64_t) retofs, f);
    return f;
}

static int
storeCossRemoveObjFromTo(SwapDir* sd)
{
    CossInfo *cs = (CossInfo *) sd->fsdata;
    CossStripe *cstripe;
    dlink_node *n;
    debug(79, 4)("Reducing: cs->rmark = %d, cs->curstripe = %d\n", cs->rmark, cs->curstripe);
    if(cs->rmark <= cs->curstripe)
        return;
    int step = (int)((sd->cur_obj_cnt - sd->snapshot)/20 > 1000 ? 1000 : (sd->cur_obj_cnt - sd->snapshot)/20);
    StoreEntry* e;
    ...
    debug(79, 4)("Reducing: Step = %d\n", step);
    for(; rNum < step && cs->rmark > cs->curstripe; cs->rmark--)
    {
        cstripe = &cs->stripes[cs->rmark];
        debug(79, 4)("Reducing: cs->rmark = %d\n", cs->rmark);
        while(cstripe->objlist.head)
        {
            e = (StoreEntry *) cstripe->objlist.head->data;
    ...
    debug(79, 4)("Reducing: rNum = %d\n", rNum);
    return rNum;

}

static void
storeCossRemoveObj(void* data)
{
    CossRemoveState* crs = data;
    SwapDir* sd = crs->sd;
    CossInfo* cs = sd->fsdata;

    /* If no need to do, means no need do this too often*/
    if(/*0 == sd->snapshot || */store_dirs_rebuilding)  //Notes by nian.yandong 
    {
        return;
    }
    /* Here we must start Remove Useless Objs*/

    //if(sd->cur_obj_cnt > sd->snapshot)
    if(sd->snapshot > 0 && sd->cur_obj_cnt > sd->snapshot)      //modify by nian.yandong
    {
        storeCossRemoveObjFromTo(sd); /* do Remove*/
        eventAdd("storeCossRemoveObj", storeCossRemoveObj, crs, 6.0, 0);
    }
    else
    {
        eventAdd("storeCossRemoveObj", storeCossRemoveObj, crs, 12.0, 0);
    }
}
#endif
...
/*
#ifdef CC_FRAMEWORK
    } else if ((cs->stripes[0].membuf == NULL && cs->mark && cs->current_offset + allocsize > cs->mark) 
            || (cs->current_offset + allocsize) > ((off_t) SD->max_size << 10)) {
        if(cs->stripes[0].membuf == NULL && cs->mark && cs->current_offset + allocsize > cs->mark)
        {
            SD->cur_obj_cnt = 0;
            if(SD->snapshot == 0)
            {
                CBDATA_INIT_TYPE(CossRemoveState);
                CossRemoveState* crs;
                crs = cbdataAlloc(CossRemoveState);
                SD->snapshot = SD->cur_obj_cnt;
                crs->sd = SD;
                cs->LOend = SD->cur_obj_cnt; // This is the end of romove point 
                cs->rmark = cs->curstripe;	
                eventAdd("storeCossRemoveObj", storeCossRemoveObj, crs, 1.0, 0);
            }
        }
#else 
*/
...
//#endif
        debug(79, 2)("cs->mark[%d], cs->current_offset[%d]\n",(int) (cs->mark), (int)(cs->current_offset));
        /*
        ...
        cs->current_membuf = newmb;
        if (newmb == NULL) {
            debug(79, 2) ("storeCossAllocate: storeCossCreateMemBuf fail %s:\n", stripePath(SD));
            cs->sizerange_max = SD->max_objsize;
            return -1;
        }
#ifdef CC_FRAMEWORK
...
//        cs->current_membuf->diskend = cs->current_offset;
        ...
        debug(79, 3) ("storeCossAllocate: curstripe: %u, numstripes: %u\n", cs->curstripe, cs->numstripes);
        ...
        STIOCB * callback, void *callback_data)
{
    ...
    debug(79, 4) ("storeCossOpen: %p: offset %d\n", sio, f);
    coss_stats.open.ops++;
    ...
    int stripe;
    CossMemBuf *t;
    if (storeCossRelocateRequired(cs, f)) {
        e->mem_swap_filen = -1;
    } else if (e->mem_swap_filen != -1) {
        stripe = storeCossFilenoToStripe(cs, e->mem_swap_filen);
        t = cs->memstripes[stripe-cs->numstripes].membuf;
        if (e->when == t->when) {
            sio->swap_filen =  e->mem_swap_filen;       // MemHit
        } else {
            e->mem_swap_filen = -1;
        }
    }
    ...
    if (p) {
...
#ifdef CC_FRAMEWORK
            nf = storeCossMemOnlyAllocate2(SD, e);
#else
        ...
        if (nf < cs->max_disk_nf) {
            ...
            storeCossRemove(SD, e);
#ifdef CC_FRAMEWORK
            storeCossNewPendingRelocate(cs, sio, sio->swap_filen, nf, SD->index);
#else
...
storeCossWrite(SwapDir * SD, storeIOState * sio, char *buf, size_t size, squid_off_t offset, FREE * free_func)
{
    ...
    if(NULL == dest)
    {
        if (free_func)
            (free_func) (buf);
        sio->offset += size;
        if(sio->e) storeRelease(sio->e);
        debug(79, 1) ("storeCossWrite: filen %lld offset %lld match dest is NULL, storeRelease %s\n", (long long)sio->swap_filen, (long long)sio->offset, storeUrl(sio->e));
        return;
    }
#endif
    assert(dest != NULL);
...
storeCossMemPointerFromDiskOffset(CossInfo * cs, off_t offset, CossMemBuf ** mb)
{
    ...
    if (offset >= storeCossFilenoToDiskOffset(cs->max_disk_nf, cs)) {
        //filen = storeCossDiskOffsetToFileno(offset, cs);
        //avoid assert fail [assert(0 == (o & cs->blksz_mask))];
        filen = offset >> cs->blksz_bits;
        stripe = storeCossFilenoToStripe(cs, filen);
        t = cs->memstripes[stripe-cs->numstripes].membuf;
        if (mb)
            *mb = t;
        if (t) {
...
storeCossFilenoToMembuf(SwapDir * SD, sfileno s)
{
    ...
    if (s >= cs->max_disk_nf) {
        stripe = storeCossFilenoToStripe(cs, s);
        t = cs->memstripes[stripe - cs->numstripes].membuf;
        return t;
    }
#endif /* CC_FRAMEWORK */
...
#ifdef CC_FRAMEWORK
        aioWrite(cs->fd, t->diskstart, &(t->buffer[0]), COSS_MEMBUF_SZ, storeCossWriteMemBufDone, t, NULL, SD->index);
#else
...
storeCossCreateMemOnlyBuf(SwapDir * SD)
{
    ...
    debug(79, 3) ("storeCossCreateMemOnlyBuf: create new strip %d!\n", stripe);
...
#ifdef CC_FRAMEWORK
static CossMemBuf *
storeCossCreateMemOnlyBuf2(SwapDir * SD, int stripe)
{
    int i;
    int newstripe;
    CossInfo *cs = (CossInfo *) SD->fsdata;
    ...
    for (i=0; i<cs->nummemstripes; i++) {
        newstripe = (stripe+i) % cs->nummemstripes;
        mb = cs->memstripes[newstripe].membuf;
        if (mb == NULL) {
            mb = cbdataAlloc(CossMemBuf);
            //dlinkAdd(mb, &mb->node, &cs->membufs);
            coss_stats.stripes++;
            cs->memstripes[newstripe].membuf = mb;
            mb->diskstart = ((off_t) SD->max_size << 10) + start;
            mb->stripe = newstripe;
            mb->diskend = mb->diskstart + COSS_MEMBUF_SZ;
            mb->flags.memonly = 1;
            mb->SD = SD;
            break;
        } else if (!mb->lockcount) {
            break;
        }
    }
    if (i == cs->nummemstripes) {
        debug(79, 3) ("storeCossCreateMemOnlyBuf2: failure!\n");
        return NULL;
    }

    getCurrentTime();
    mb->when = current_dtime;
    debug(79, 3) ("storeCossCreateMemOnlyBuf2: newstripe %d on the %s cache_dir\n", newstripe, stripePath(SD));
    cs->curmemstripe = newstripe;
    mb->flags.full = 0;
    mb->flags.writing = 0;
    mb->lockcount = 0;
    mb->numobjs = 0;
    cs->memstripes[newstripe].objlist.head = cs->memstripes[newstripe].objlist.tail = NULL;
...
storeCossCreateMemBuf(SwapDir * SD, int stripe, sfileno curfn, int *collision)
{
    ...
    for (i=0; i<cs->numstripes; i++) {
        if (cs->stripes[stripe].membuf == NULL) {
            start = (off_t) stripe * COSS_MEMBUF_SZ;
            break;
        }
        stripe = (stripe+1) % cs->numstripes;
    }
    if (i == cs->numstripes)
        return NULL;
#else
...
#ifdef CC_FRAMEWORK
void
storeCossStartMembuf2(SwapDir * sd)
{
    CossInfo *cs = (CossInfo *) sd->fsdata;
    CossMemBuf *newmb;
    ...
    newmb = storeCossCreateMemOnlyBuf2(sd, cs->curmemstripe);
...
#ifdef CC_FRAMEWORK
    aioRead(cs->fd, (off_t) disk_offset, pr->len, storeCossCompletePendingReloc, pr, sd_index);
#else
...
storeCossGetPendingReloc(CossInfo * cs, sfileno new_filen)
{
    ...
    while (n != NULL) {
        ...
        if (pr->new_filen == new_filen) {
            debug(79, 3) ("storeCossGetPendingReloc: orig_filen: %u  new_filen: %u\n", pr->original_filen, new_filen);
            return pr;
        }
        n = n->next;
    }
...
#ifdef CC_FRAMEWORK
            StoreEntry *e = sio->e;
            if (sio->swap_filen >= cs->max_disk_nf) {
                e->mem_swap_filen = sio->swap_filen;
                int s = storeCossFilenoToStripe(cs, e->mem_swap_filen);     
                CossMemBuf *t = cs->memstripes[s - cs->numstripes].membuf;
                e->when = t->when;  // case 3390
            }
#endif /* CC_FRAMEWORK */
        }
