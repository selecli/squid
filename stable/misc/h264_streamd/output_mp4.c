    ...
    if(destLen + extra_space < sourceLen)
    {
      ...
      if(zstatus == Z_OK)
      {
        ...
        else
        {
          ...
          errlog("2nd pass compress overflow\n");
        }
      }
    }
  }
...
//      }
    ...
    if(stts_get_samples(stts) != end - start)
    {
      ...
      errlog("ERROR: stts_get_samples=%d, should be %d\n",
             stts_get_samples(stts), end - start);
    }
  }
  ...
  // ctts = [entries * [sample_count, sample_offset]
  {
    ...
    if(ctts)
    {
      ...
      if(ctts_get_samples(ctts) != end - start)
      {
        ...
        errlog("ERROR: ctts_get_samples=%d, should be %d\n",
               ctts_get_samples(ctts), end - start);
      }
    }
  }
                      ...
                      struct mp4_split_options_t* options)
{
  ...
  //MP4_INFO("mdat_start%"PRIu64"\n", mdat_start);
  //MP4_INFO("mdat_size%"PRIu64"\n", mdat_size);

  debug("mdat_start = %"PRIu64"\n", mdat_start);
  debug("mdat_size = %"PRIu64"\n", mdat_size);
  ...
  uint64_t new_mdat_start = 0;
  {
    ...
    if(mp4_context->ftyp_atom.size_)
    {
      ...
      if(fread(buffer, (off_t)mp4_context->ftyp_atom.size_, 1, mp4_context->infile) != 1)
      {
        ...
        errlog("Error reading ftyp atom\n");
        free(buffer);
        return 0;
      }
    }
...
//  new_mdat_start += mp4_context->moov_atom.size_;
  ...
  //MP4_INFO("new mdat_start%"PRIu64"\n", new_mdat_start);
  debug("new mdat_start = %"PRIu64"\n", new_mdat_start);
  offset = new_mdat_start - mp4_context->mdat_atom.start_ + options->file_offsets;
...
//  offset -= mp4_context->moov_atom.size_;
  ...
  for(i = 0; i != moov->tracks_; ++i)
  {
    ...
    if(trak->samples_size_ == 0)
    {
      ...
      warn("Trak %u contains no samples. Maybe a fragmented file?", i);
      return 1;
    }

    {
      ...
      debug("Trak can skip %"PRIu64" bytes\n", skip);
      ...
      if(end_sample != trak->samples_size_)
      {
        ...
        debug("Trak can skip %"PRIu64" bytes at end\n",
...

    {
      ...
      long trak_time_scale = trak->mdia_->mdhd_->timescale_;
      {
        ...
        debug("trak: new_duration=%"PRIu64"\n", duration);
...
//      MP4_INFO("stsc.samples=%d\n", stsc_get_samples(stbl->stsc_));
  ...
  // add new moov size
  offset += moov_size ;
  //offset += options->file_offsets ;//zds add

  //MP4_INFO("shifting offsets by %"PRId64"\n", offset);
  debug("shifting offsets by %"PRId64"\n", offset);
