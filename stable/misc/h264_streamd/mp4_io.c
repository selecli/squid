                                ...
                                FILE* infile, mp4_atom_t* atom)
{
  ...
  if(fread(atom_header, 8, 1, infile) != 1)
  {
    ...
    errlog("Error reading atom header\n");
    return 0;
  }
  ...
  if(atom->short_size_ == 1)
  {
    if(fread(atom_header, 8, 1, infile) != 1)
    {
      ...
      errlog("Error reading extended atom header\n");
      return 0;
    }
                               ...
                               FILE* infile, struct mp4_atom_t* atom)
{
  ...
  if(fread(box_data, (off_t)atom->size_, 1, infile) != 1)
  {
    ...
    errlog("Error reading %c%c%c%c atom\n",
           atom->type_ >> 24, atom->type_ >> 16,
           atom->type_ >> 8, atom->type_);
    free(box_data);
    fclose(infile);
...
extern mp4_context_t* mp4_open(const char* filename, int64_t filesize,
                               mp4_open_flags flags, int verbose,int64_t file_offset)
{
  ...
  //zds add code for mp4 drag
  if(file_offset > 0){
	
      fseeko(mp4_context->infile, file_offset, SEEK_SET);
  }
  else {
	file_offset = 0;
  }
  ...
  if(flags == MP4_OPEN_MFRA)
  {
    ...
    if(fread(mfro, 16, 1, mp4_context->infile) != 1)
    {
      ...
      errlog("Error reading mfro header\n");
    ...
    else
    {
      if(read_32(mfro + 4) != FOURCC('m', 'f', 'r', 'o'))
      {
        ...
        errlog("%s", "Error parsing mfro header\n");
        //fseeko(mp4_context->infile, 0, SEEK_SET);
        fseeko(mp4_context->infile, file_offset, SEEK_SET);
      }
      else
  ...
  while(ftello(mp4_context->infile) < filesize)
  {
    ...
    case FOURCC('m', 'o', 'o', 'v'):
	 if(hasmoov == 1)
      ...
      if(mp4_context->moov == 0 || mp4_context->moov->mvhd_ == 0)
      {
        ...
        errlog("Error parsing moov header\n");
        mp4_context_exit(mp4_context);
        return 0;
      }
      break;
	...
	hasmoof = 1;
           ...
           mp4_context->moov->traks_[0]->mdia_->minf_->stbl_->stco_ == 0)
        {
          ...
          errlog("Fixing invalid offsets for Expression Encoder file\n");
        ...
        if(moof == 0)
        {
          ...
          errlog("Error parsing moof header\n");
          mp4_context_exit(mp4_context);
          return 0;
        }
   ...
   /* if(leaf_atom.end_ > (uint64_t)filesize)
    {
     // MP4_ERROR("%s", "Reached end of file prematurely\n");
	  errlog("Reached end of file prematurely\n");
      mp4_context_exit(mp4_context);
      return 0;
    }*/
  ...
  if(mp4_context->moov == 0)
  {
    ...
    errlog("Error: moov atom not found\n");
    mp4_context_exit(mp4_context);
    return 0;
  }
  ...
  if(mp4_context->mdat_atom.size_ == 0)
  {
    ...
    errlog("Error: mdat atom not found\n");
    mp4_context_exit(mp4_context);
    return 0;
  }
...
extern unsigned int stbl_get_nearest_keyframe_prev(struct stbl_t const* stbl,
                                              unsigned int sample, unsigned int *next_keyframe)
{
  // If the sync atom is not present, all samples are implicit sync samples.
  if(!stbl->stss_)
    return sample;

  return stss_get_nearest_keyframe_prev(stbl->stss_, sample, next_keyframe);
}

extern unsigned int stbl_get_nearest_keyframe_next(struct stbl_t const* stbl,
                                              unsigned int sample)
{
  // If the sync atom is not present, all samples are implicit sync samples.
  if(!stbl->stss_)
  ...
  return stss_get_nearest_keyframe_next(stbl->stss_, sample);
}
...
/* save next_keyframe for processing start == end */
extern unsigned int stss_get_nearest_keyframe_prev(struct stss_t const* stss,
                                              unsigned int sample, unsigned int *next_keyframe)
{
  // scan the sync samples to find the key frame that precedes the sample number
  unsigned int i;
  ...
  //if start == end ,end should be point next keyframe
  if ((*next_keyframe == stss->sample_numbers_[i - 1]) && (i < stss->entries_))
  {
	  return stss->sample_numbers_[i];
  }
  *next_keyframe = stss->sample_numbers_[i - 1];

  return stss->sample_numbers_[i - 1];
}

extern unsigned int stss_get_nearest_keyframe_next(struct stss_t const* stss,
                                              unsigned int sample)
{
  // scan the sync samples to find the key frame that precedes the sample number
  unsigned int i;
  ...
  if(table_sample >= sample || i >= stss->entries_)
    return table_sample;
  else
    return stss->sample_numbers_[i];
}
